...
void SplitDomain()
{
	...
	if (!parameters::self_gravity) {
		if (debug)
			logging::print_master(LOG_INFO "SplitDomain: Doing standard domain decomposition.\n");
		...
		if (size_low < 2*CPUOVERLAP) {
			logging::print_master(LOG_ERROR "The number of processes is too large or the mesh is radially too narrow.\n");
			PersonalExit(1);
		}
		...
		Max_or_active = NRadial-CPUOVERLAP*(CPU_Rank < CPU_Number-1 ? 1 : 0);
		MaxMO_or_active = NRadial-1-(CPUOVERLAP-1)*(CPU_Rank < CPU_Number-1 ? 1 : 0);
		...
		if (debug)
			logging::print_master(LOG_INFO "SplitDomain: Doing fftw-mesh implied domain decomposition.\n");

		logging::print_master(LOG_INFO "SplitDomain: Doing some FFT test measures to optimize FFT calculations. This may take a few seconds.\n");

		// calculate fft mesh sizes
		total_local_size = fftw_mpi_local_size_2d_transposed(2*GlobalNRadial, NAzimuthal, MPI_COMM_WORLD, &local_Nx, &local_i_start, &local_Ny_after_transpose, &local_j_start_after_transpose);

		if (CPU_Number%2 == 0) {
			if ( (CPU_Rank == CPU_Number/2-1) && (local_i_start+local_Nx-1 < GlobalNRadial-1) ) {
				logging::print(LOG_ERROR "ERROR: Bad choice of number of processes for current value of GLOBALNRAD.\n");
				PersonalExit(1);
			}
		}

		if (local_Nx < 2*CPUOVERLAP ) {
			logging::print(LOG_ERROR "ERROR: At least one of the processes has a ring number (%u) less than the total number of ghosts (%u): I must exit.\n", local_Nx, 2*CPUOVERLAP);
			PersonalExit(1);
		}
		...
		/* Each CPU communicates with its 'CPU_Friend' the info from the hydro mesh to the fftw mesh, and vice versa */
		...
		/* Each CPU communicates boundaries with CPU_Prev and CPU_Next, except CPU 0 and CPU_Highest */
		...
		if ( CPU_Rank < (CPU_Number + one_if_odd)/2 ) {
			...
			if ( (CPU_Number%2 == 0) && (CPU_Rank == CPU_Number/2 - 1) ) {
				transfer_size = 2*(GlobalNRadial - local_i_start - ifront - 1 + CPUOVERLAP) * 2*(NAzimuthal/2+1);
			} else {
				transfer_size = 2*(local_Nx - ifront - 1 + CPUOVERLAP) * 2*(NAzimuthal/2 + 1);
			}

			if ( CPU_Rank != CPU_NoFriend ) {
				ffttohydro_transfer = (double *) malloc(sizeof(double) * transfer_size);
				if (ffttohydro_transfer == NULL) {
					logging::print(LOG_ERROR "No enougth memory for ffttohydro_transfer tab in split.c ...\n");
					PersonalExit(1);
				}
			}
			...
			SGP_buffft_Accr_friend = (double*)malloc(sizeof(double)*total_local_size_friend);
			SGP_buffft_Acct_friend = (double*)malloc(sizeof(double)*total_local_size_friend);
			if ( (SGP_buffft_Accr_friend == NULL) || (SGP_buffft_Acct_friend == NULL) ) {
				logging::print(LOG_ERROR "No enougth memory for SGP_buffft_Acc_friend in split.c ...\n");
				PersonalExit(1);
			}

			ffttohydro_transfer_friend = (double *) malloc(sizeof(double) * transfer_size_friend);
			if ( ffttohydro_transfer_friend == NULL ) {
				logging::print(LOG_ERROR "No enougth memory for ffttohydro_transfer_friend tab in split.c ...\n");
				PersonalExit(1);
			}
		...
		if ( NRadial < 2*CPUOVERLAP ) {
			...
			logging::print(LOG_ERROR "ERROR: One of the processes has a ring number less than the total number of ghosts: I must exit.\n");
			PersonalExit(1);
		}

		Zero_or_active = CPUOVERLAP * (CPU_Rank > 0 ? 1 : 0);
		One_or_active = 1+(CPUOVERLAP-1) * (CPU_Rank > 0 ? 1 : 0);
		Max_or_active = NRadial-CPUOVERLAP*(CPU_Rank != CPU_Highest ? 1 : 0);
		MaxMO_or_active = NRadial-1-(CPUOVERLAP-1)*(CPU_Rank != CPU_Highest ? 1 : 0);
		hydro_totalsize = NRadial * NAzimuthal;
		active_hydro_totalsize = (Max_or_active - Zero_or_active + 1)*NAzimuthal;
