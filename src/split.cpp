...
void SplitDomain()
{
	...
	if (debug)
	    logging::print_master(LOG_INFO "SplitDomain: Doing standard "
	...
	if (size_low < 2 * CPUOVERLAP) {
	    logging::print_master(LOG_ERROR
				  "The number of processes is too large or "
	...
	if (debug)
	    logging::print_master(LOG_INFO
				  "SplitDomain: Doing fftw-mesh implied "
				  "domain decomposition.\n");
	...
	if (CPU_Rank < (CPU_Number + one_if_odd) / 2) {
		...
		if (ffttohydro_transfer == NULL) {
		    logging::print(LOG_ERROR "No enougth memory for "
					     "ffttohydro_transfer "
					     "tab in split.c ...\n");
		    PersonalExit(1);
		}
	...
	if (NRadial < 2 * CPUOVERLAP) {
	    logging::print(LOG_ERROR "CPU %d: local_Nx = %d\n", CPU_Rank,
			   NRadial);
	    logging::print(LOG_ERROR "ERROR: One of the processes has a ring "
