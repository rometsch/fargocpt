...
void SplitDomain()
{
	...
	if (!parameters::self_gravity) {
		...
		if (size_low < 2 * CPUOVERLAP) {
			logging::print_master(
			    LOG_ERROR "The number of processes is too large or "
				      "the mesh is radially too narrow.\n");
			PersonalExit(1);
		}
		...
		if (CPU_Rank < remainder) {
			...
			IMIN = size_high * remainder +
			       (CPU_Rank - remainder) * size_low;
		...
		if (debug)
			logging::print_master(
			    LOG_INFO "SplitDomain: Doing fftw-mesh implied "
				     "domain decomposition.\n");

		logging::print_master(
		    LOG_INFO
		    "SplitDomain: Doing some FFT test measures to optimize FFT "
		...
		if (CPU_Number % 2 == 0) {
			if ((CPU_Rank == CPU_Number / 2 - 1) &&
			    (local_i_start + local_Nx - 1 <
			     GlobalNRadial - 1)) {
				logging::print(
				    LOG_ERROR
				    "ERROR: Bad choice of number of processes "
				    "for current value of GLOBALNRAD.\n");
				PersonalExit(1);
			}
		}
		...
		if (local_Nx < 2 * CPUOVERLAP) {
			logging::print(
			    LOG_ERROR
			    "ERROR: At least one of the processes has a ring "
			    "number (%u) less than the total number of ghosts "
			    "(%u): I must exit.\n",
		...
		if (CPU_Number % 2 == 1) {
			...
			CPU_NoFriend = CPU_Number; // does not exist, on
						   // purpose. Do not erase!!
			CPU_Highest = CPU_Number - 1;
		}
		...
		if (CPU_Rank < (CPU_Number + one_if_odd) / 2) {
			...
			if ((CPU_Number % 2 == 0) &&
			    (CPU_Rank == CPU_Number / 2 - 1)) {
				transfer_size = 2 *
						(GlobalNRadial - local_i_start -
						 ifront - 1 + CPUOVERLAP) *
						2 * (NAzimuthal / 2 + 1);
			} else {
				transfer_size =
				    2 * (local_Nx - ifront - 1 + CPUOVERLAP) *
				    2 * (NAzimuthal / 2 + 1);
			}

			if (CPU_Rank != CPU_NoFriend) {
				ffttohydro_transfer = (double *)malloc(
				    sizeof(double) * transfer_size);
				if (ffttohydro_transfer == NULL) {
					logging::print(LOG_ERROR
						       "No enougth memory for "
						       "ffttohydro_transfer "
						       "tab in split.c ...\n");
					PersonalExit(1);
				}
			...
			if (CPU_Rank != CPU_NoFriend)
				MPI_Send(sg_split, 5, MPI_AINT, CPU_Friend, 10,
					 MPI_COMM_WORLD);
		} else {
			MPI_Recv(hydro_split, 5, MPI_AINT, CPU_Friend, 10,
			...
			SGP_buffft_Accr_friend = (double *)malloc(
			    sizeof(double) * total_local_size_friend);
			SGP_buffft_Acct_friend = (double *)malloc(
			    sizeof(double) * total_local_size_friend);
			if ((SGP_buffft_Accr_friend == NULL) ||
				...
				logging::print(
				    LOG_ERROR
				    "No enougth memory for "
				    "SGP_buffft_Acc_friend in split.c ...\n");
				PersonalExit(1);
			}

			ffttohydro_transfer_friend = (double *)malloc(
			    sizeof(double) * transfer_size_friend);
			if (ffttohydro_transfer_friend == NULL) {
				logging::print(LOG_ERROR
					       "No enougth memory for "
					       "ffttohydro_transfer_friend tab "
					       "in split.c ...\n");
				PersonalExit(1);
			}
		...
		if (NRadial < 2 * CPUOVERLAP) {
			logging::print(LOG_ERROR "CPU %d: local_Nx = %d\n",
				       CPU_Rank, NRadial);
			logging::print(LOG_ERROR
				       "ERROR: One of the processes has a ring "
				       "number less than the total number of "
				       "ghosts: I must exit.\n");
			PersonalExit(1);
		}
		...
		MaxMO_or_active =
		    NRadial - 1 -
		    (CPUOVERLAP - 1) * (CPU_Rank != CPU_Highest ? 1 : 0);
		...
		if ((CPU_Number % 2 == 0) || (CPU_Rank != CPU_NoFriend)) {
			if (CPU_Rank >= (CPU_Number + one_if_odd) / 2) {
				MPI_Ssend(&active_hydro_totalsize, 1, MPI_INT,
					  CPU_Friend, 20, MPI_COMM_WORLD);
				MPI_Ssend(&Zero_or_active, 1, MPI_INT,
					  CPU_Friend, 22, MPI_COMM_WORLD);
			} else {
				MPI_Recv(&active_hydro_totalsize_friend, 1,
					 MPI_INT, CPU_Friend, 20,
					 MPI_COMM_WORLD, &global_MPI_Status);
				MPI_Recv(&Zero_or_active_friend, 1, MPI_INT,
					 CPU_Friend, 22, MPI_COMM_WORLD,
					 &global_MPI_Status);
				dens_friend = (double *)malloc(
				    sizeof(double) *
				    active_hydro_totalsize_friend);
				if (dens_friend == NULL) {
					logging::print(
					    LOG_ERROR
					    "No enougth memory for dens_friend "
					    "in split.c ...\n");
					PersonalExit(1);
				}
			}
	...
	// Initializes Radial sizes of non vector arrays and required
	// displacements
	if (CPU_Master) {
		...
		for (int i = 0; i < CPU_Number; i++) {
			RootNradialDisplacements[i] =
			    (i > 0) ? (RootNradialDisplacements[i - 1] +
				    ...
				    : 0;
			RootIMIN[i] = RootNradialDisplacements[i] +
				      ((i == 0) ? GHOSTCELLS_B : 0);
	...
	if (debug) {
		logging::print(LOG_DEBUG
			       "SplitDomain: DomainSplit Information:\n");
		if (!parameters::self_gravity)
			logging::print_master(
			    LOG_DEBUG "SplitDomain: %d = %d * %d + %d\n",
		...
		if (parameters::self_gravity) {
			logging::print(LOG_DEBUG "SplitDomain: LocalNx: %ld\n",
				       local_Nx);
			logging::print(LOG_DEBUG
				       "SplitDomain: LocalIStart: %ld\n",
				       local_i_start);
			logging::print(LOG_DEBUG
				       "SplitDomain: total_local_size: %ld\n",
				       total_local_size);

			logging::print(LOG_DEBUG "Friend: %ld\n", CPU_Friend);
			logging::print(LOG_DEBUG
				       "SplitDomain: LocalNx_friend: %ld\n",
				       local_Nx_friend);
			logging::print(LOG_DEBUG
				       "SplitDomain: LocalIStart_friend: %ld\n",
