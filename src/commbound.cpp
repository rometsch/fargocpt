...
void AllocateBoundaryCommunicationBuffers() {
	/* number of polar grids to communicate; standard is 3 (density, vrad, vtheta) */
	bufferSize = 3;
	...
	if ((SendInnerBoundary == NULL) || (SendOuterBoundary == NULL) || (RecvInnerBoundary == NULL) || (RecvOuterBoundary == NULL)) {
		logging::print(LOG_ERROR "CPU %d had not enough memory to allocate communicators.\n", CPU_Rank);
		PersonalExit(0);
	}
...
void CommunicateBoundaries(t_polargrid* Density, t_polargrid* Vrad, t_polargrid* Vtheta, t_polargrid* Energy)
{
	...
	/* Note that boundary exchange is independant from chosen domain decomposition */
	/* send / receive data */
	if (CPU_Rank%2 == 0) {
		if (CPU_Rank != 0) {
			MPI_Isend(SendInnerBoundary, bufferSize, MPI_DOUBLE, CPU_Prev, 0, MPI_COMM_WORLD, &req1);
			MPI_Irecv(RecvInnerBoundary, bufferSize, MPI_DOUBLE, CPU_Prev, 0, MPI_COMM_WORLD, &req2);
		}

		if (CPU_Rank != CPU_Highest) {
			MPI_Isend(SendOuterBoundary, bufferSize, MPI_DOUBLE, CPU_Next, 0, MPI_COMM_WORLD, &req3);
			MPI_Irecv(RecvOuterBoundary, bufferSize, MPI_DOUBLE, CPU_Next, 0, MPI_COMM_WORLD, &req4);
		}
	} else {
		if (CPU_Rank != CPU_Highest) {
			MPI_Irecv(RecvOuterBoundary, bufferSize, MPI_DOUBLE, CPU_Next, 0, MPI_COMM_WORLD, &req3);
			MPI_Isend(SendOuterBoundary, bufferSize, MPI_DOUBLE, CPU_Next, 0, MPI_COMM_WORLD, &req4);
		}

		if (CPU_Rank != 0) {
			MPI_Irecv(RecvInnerBoundary, bufferSize, MPI_DOUBLE, CPU_Prev, 0, MPI_COMM_WORLD, &req1);
			MPI_Isend(SendInnerBoundary, bufferSize, MPI_DOUBLE, CPU_Prev, 0, MPI_COMM_WORLD, &req2);
		}
	}
