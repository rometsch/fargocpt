...
void AlgoGas(unsigned int nTimeStep, Force *force, t_data &data)
{
    ...
    while (dtemp < DT) {
	logging::print_master(LOG_VERBOSE "AlgoGas: Total: %*i/%i (%5.2f %%) - "
					  "Timestep: %#7f/%#7f (%5.2f %%)\n",
			      (int)ceil(log10(NTOT)), nTimeStep, NTOT,
			      (double)nTimeStep / (double)NTOT * 100.0, dtemp,
			      DT, dtemp / DT * 100.0);
	...
	if (Corotating == YES) {
	    double distance_new =
		sqrt(pow2(data.get_planetary_system().get_planet(0).get_x()) +
		     pow2(data.get_planetary_system().get_planet(0).get_y()));
	    double distance_old =
	...
	if (parameters::calculate_disk) {
		...
		if (assure_minimum_temperature(
		    ...
		    logging::print(LOG_DEBUG "Found temperature < %g "
					     "%s after SubStep3.\n",
				   parameters::minimum_temperature,
		...
		if (parameters::radiative_diffusion_enabled) {
		    ...
		    if (assure_minimum_temperature(
			    ...
			    parameters::minimum_temperature *
				units::temperature.get_inverse_cgs_factor())) {
			logging::print(LOG_DEBUG "Found temperature < %g %s "
						 "after "
						 "radiative_diffusion.\n",
				       parameters::minimum_temperature,
				       units::temperature.get_cgs_symbol());
...
void calculate_qminus(t_data &data)
{
	...
	for (unsigned int n_radial = 0;
		...
		if (t_ramp_up > 0.0) {
		    ...
		    double t_ramp_factor = 1 - exp(-pow(2 * t / t_ramp_up, 2));
...
void compute_sound_speed(t_data &data, bool force_update)
{
	...
	for (unsigned int n_azimuthal = 0;
		...
		data[t_data::SOUNDSPEED](n_radial, n_azimuthal) =
		    ASPECTRATIO_REF * sqrt(constants::G * M / Rb[n_radial]) *
