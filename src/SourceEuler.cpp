		...
		for (unsigned int n_azimuthal = 0;
			...
			if (dst(n_radial, n_azimuthal) < minimum_value) {
				if (is_dens) {
					double mass_delta =
					    (minimum_value -
					     dst(n_radial, n_azimuthal)) *
					    Surf[n_radial];
					sum_without_ghost_cells(
...
#ifndef NDEBUG
				logging::print(
				    LOG_DEBUG "assure_minimum_temperature: "
					      "(%u,%u)=%g<%g\n",
...
#ifndef NDEBUG
				logging::print(
				    LOG_DEBUG "assure_maximum_temperature: "
					      "(%u,%u)=%g>%g\n",
				    n_radial, n_azimuthal,
				    energy(n_radial, n_azimuthal) *
					units::temperature.get_cgs_factor() /
...
void AlgoGas(unsigned int nTimeStep, Force *force, t_data &data)
{
	...
	// recalculate timestep, even for no_disk = true, so that particle drag
	// has reasonable timestep size
	...
	while (dtemp < DT) {
		logging::print_master(LOG_VERBOSE
				      "AlgoGas: Total: %*i/%i (%5.2f %%) - "
				      "Timestep: %#7f/%#7f (%5.2f %%)\n",
				      (int)ceil(log10(NTOT)), nTimeStep, NTOT,
				      (double)nTimeStep / (double)NTOT * 100.0,
				      dtemp, DT, dtemp / DT * 100.0);
		...
		if (parameters::calculate_disk) {
			...
			if (parameters::disk_feedback)
				DiskOnPrimaryAcceleration =
				    ComputeAccel(force, data, 0.0, 0.0, 0.0);

			/* Gravitational potential from star and planet(s) is
			 * computed and stored here*/
			FillForcesArrays(data);
			/* Planets' velocities are updated here from
			 * gravitationnal interaction with disk */
		...
		/* Below we correct v_azimuthal, planet's position and
		 * velocities if we work in a frame non-centered on the star */
		if (Corotating == YES) {
			double distance_new =
			    sqrt(pow2(data.get_planetary_system()
				 ...
				 pow2(data.get_planetary_system()
					  ...
					  .get_y()));
			double distance_old =
			    sqrt(pow2(planet0_old_x) + pow2(planet0_old_y));
			double cross =
			    planet0_old_x * data.get_planetary_system()
						.get_planet(0)
						.get_y() -
			    data.get_planetary_system().get_planet(0).get_x() *
				planet0_old_y;

			// new = r_new x r_old = distance_new * distance_old *
			// sin(alpha*dt)
			OmegaNew =
			...
			if (parameters::calculate_disk)
				correct_v_azimuthal(data[t_data::V_AZIMUTHAL],
						    domega);
		...
		if (parameters::calculate_disk) {
			...
			copy_polargrid(data[t_data::V_RADIAL],
				       data[t_data::V_RADIAL_NEW]);
			copy_polargrid(data[t_data::V_AZIMUTHAL],
				       data[t_data::V_AZIMUTHAL_NEW]);

			boundary_conditions::apply_boundary_condition(data, dt,
								      false);
			...
			if (parameters::Adiabatic) {
				if ((parameters::artificial_viscosity ==
					...
					viscosity::compute_viscous_terms(data,
									 true);
				} else {
					viscosity::compute_viscous_terms(data,
									 false);
				...
				copy_polargrid(data[t_data::ENERGY],
					       data[t_data::ENERGY_NEW]);
			...
			Transport(data, &data[t_data::DENSITY],
				  &data[t_data::V_RADIAL],
				  &data[t_data::V_AZIMUTHAL],
				  &data[t_data::ENERGY], dt);
			...
			if (parameters::Adiabatic) {
				// assure minimum temperature after all substeps
				// & transport. it is crucial the check minimum
				// density before!
				if (assure_minimum_temperature(
					data[t_data::ENERGY],
					data[t_data::DENSITY],
			...
			if (data[t_data::ALPHA_GRAV_MEAN].get_write()) {
				quantities::calculate_alpha_grav_mean_sumup(
				    data, nTimeStep, dt);
			}
			if (data[t_data::ALPHA_GRAV_MEAN_1D].get_write()) {
				quantities::
				    calculate_radial_alpha_grav_mean_sumup(
					data, nTimeStep, dt);
			}
			if (data[t_data::ALPHA_REYNOLDS_MEAN].get_write()) {
				quantities::calculate_alpha_reynolds_mean_sumup(
				    data, nTimeStep, dt);
			}
			if (data[t_data::ALPHA_REYNOLDS_MEAN_1D].get_write()) {
				quantities::
				    calculate_radial_alpha_reynolds_mean_sumup(
					data, nTimeStep, dt);
			}
		}
	...
	if (data[t_data::ALPHA_REYNOLDS_MEAN_1D].get_write()) {
		quantities::calculate_radial_alpha_reynolds_mean_finalize(data,
									  DT);
	}
}
...
void SubStep1(t_data &data, double dt)
{
	...
	for (unsigned int n_radial = 1;
	     n_radial <=
	     data[t_data::V_RADIAL_SOURCETERMS].get_max_radial() - 1;
	     ++n_radial) {
		for (unsigned int n_azimuthal = 0;
		     n_azimuthal <=
		     data[t_data::V_RADIAL_SOURCETERMS].get_max_azimuthal();
		     ++n_azimuthal) {
			// 1/Sigma * dP/dr : Sigma is calculated as a mean value
			...
			gradp =
			    ...
			    (data[t_data::PRESSURE](n_radial, n_azimuthal) -
			     data[t_data::PRESSURE](n_radial - 1,
						    n_azimuthal)) *
			...
			gradphi =
			    (data[t_data::POTENTIAL](n_radial, n_azimuthal) -
			     data[t_data::POTENTIAL](n_radial - 1,
						     n_azimuthal)) *
			    InvDiffRmed[n_radial];

			// v_phi^2/r : v_phi^2 is calculated by a mean in both
			// directions
			vt2 = data[t_data::V_AZIMUTHAL](n_radial, n_azimuthal) +
			      data[t_data::V_AZIMUTHAL](
				  n_radial,
				  n_azimuthal == data[t_data::V_AZIMUTHAL]
				      ...
				      ? 0
				      : n_azimuthal + 1) +
			      data[t_data::V_AZIMUTHAL](n_radial - 1,
							n_azimuthal) +
			      data[t_data::V_AZIMUTHAL](
				  n_radial - 1,
				  n_azimuthal == data[t_data::V_AZIMUTHAL]
			...
			// add all terms to new v_radial: v_radial_new =
			// v_radial + dt*(source terms)
			data[t_data::V_RADIAL_SOURCETERMS](n_radial,
							   n_azimuthal) =
	...
	for (unsigned int n_radial = 0;
		...
		supp_torque = IMPOSEDDISKDRIFT * 0.5 *
			      pow(Rmed[n_radial], -2.5 + SIGMASLOPE);
		invdxtheta =
		    1.0 /
		    (2.0 * PI /
		     (double)data[t_data::V_AZIMUTHAL].get_size_azimuthal() *
		     Rmed[n_radial]);
		...
		for (unsigned int n_azimuthal = 0;
			...
			// add all terms to new v_azimuthal: v_azimuthal_new =
			// v_azimuthal + dt*(source terms)
			...
			data[t_data::V_AZIMUTHAL_SOURCETERMS](
			    n_radial, n_azimuthal) += dt * supp_torque;
		}
	}
...
void SubStep2(t_data &data, double dt)
{
	if (parameters::artificial_viscosity ==
		...
		for (unsigned int n_radial = 0;
		     n_radial <= data[t_data::Q_R].get_max_radial();
		     ++n_radial) {
			for (unsigned int n_azimuthal = 0;
			     n_azimuthal <=
			     data[t_data::Q_R].get_max_azimuthal();
			     ++n_azimuthal) {
				double dv_r =
		...
		for (unsigned int n_radial = 1;
		     n_radial <=
		     data[t_data::V_RADIAL_NEW].get_max_radial() - 1;
		     ++n_radial) {
			for (unsigned int n_azimuthal = 0;
			     n_azimuthal <=
			     data[t_data::V_RADIAL_NEW].get_max_azimuthal();
			     ++n_azimuthal) {
				// 1/Sigma dq_r/dr : Sigma is calculated as a
				// mean value between the neightbour cells
				data[t_data::V_RADIAL_NEW](n_radial,
							   n_azimuthal) =
				    data[t_data::V_RADIAL_SOURCETERMS](
					n_radial, n_azimuthal) -
				    dt * 2.0 /
					(data[t_data::DENSITY](n_radial,
							       n_azimuthal) +
					 data[t_data::DENSITY](n_radial - 1,
							       n_azimuthal)) *
					(data[t_data::Q_R](n_radial,
							   n_azimuthal) -
					 data[t_data::Q_R](n_radial - 1,
							   n_azimuthal)) *
					InvDiffRmed[n_radial];
			}
		}
		...
		for (unsigned int n_radial = 0;
			...
			for (unsigned int n_azimuthal = 0;
			     ...
			     ++n_azimuthal) {
				// 1/Sigma 1/r dq_phi/dphi : Sigma is calculated
				// as a mean value between the neightbour cells
				data[t_data::V_AZIMUTHAL_NEW](n_radial,
							      n_azimuthal) =
				    data[t_data::V_AZIMUTHAL_SOURCETERMS](
					n_radial, n_azimuthal) -
				    dt * 2.0 /
		...
		// If gas disk is adiabatic, we add artificial viscosity as a
		// source term for advection of thermal energy polargrid
		if (parameters::Adiabatic) {
			if (parameters::artificial_viscosity_dissipation) {
				for (unsigned int n_radial = 0;
				     n_radial <=
				     data[t_data::ENERGY_INT].get_max_radial();
				     ++n_radial) {
					dxtheta = 2.0 * PI /
						  (double)data[t_data::DENSITY]
						      .get_size_azimuthal() *
						  Rmed[n_radial];
					invdxtheta = 1.0 / dxtheta;
					for (unsigned int n_azimuthal = 0;
					     n_azimuthal <=
					     data[t_data::ENERGY_INT]
						 .get_max_azimuthal();
					     ++n_azimuthal) {
						data[t_data::ENERGY_INT](
						    n_radial, n_azimuthal) =
				...
				copy_polargrid(data[t_data::ENERGY_INT],
					       data[t_data::ENERGY]);
			}
		}
	} else {
...
void calculate_qplus(t_data &data)
{
	...
	if (parameters::heating_viscous_enabled) {
		...
		for (unsigned int n_radial = 1;
			...
			for (unsigned int n_azimuthal = 0;
			     ...
			     ++n_azimuthal) {
				if (data[t_data::VISCOSITY](
					n_radial, n_azimuthal) != 0.0) {
					...
					double qplus =
					    1.0 /
					    (2.0 *
					     data[t_data::VISCOSITY](
						 n_radial, n_azimuthal) *
					     data[t_data::DENSITY](
						 n_radial, n_azimuthal)) *
					    (pow2(data[t_data::TAU_R_R](
						 n_radial, n_azimuthal)) +
					     2 * pow2(tau_r_phi) +
					     pow2(data[t_data::TAU_PHI_PHI](
						 n_radial, n_azimuthal)));
					qplus += (2.0 / 9.0) *
						 data[t_data::VISCOSITY](
						     n_radial, n_azimuthal) *
						 data[t_data::DENSITY](
						     n_radial, n_azimuthal) *
						 pow2(data[t_data::DIV_V](
						     n_radial, n_azimuthal));

					qplus *=
					    parameters::heating_viscous_factor;
					data[t_data::QPLUS](
					    n_radial, n_azimuthal) += qplus;
				}
			}
		}
				...
				double qplus =
				    ...
				    exp(log(data[t_data::QPLUS](
						...
						data[t_data::QPLUS]
						    ...
						    2,
						n_azimuthal)) *
					log(Rmed[data[t_data::QPLUS]
						     .get_max_radial()] /
					    Rmed[data[t_data::QPLUS]
						     .get_max_radial() -
						 1]) /
					log(Rmed[data[t_data::QPLUS]
						     .get_max_radial() -
	...
	if (parameters::heating_star_enabled) {
		...
		if (parameters::heating_star_simple) {
			...
			// Simple star heating (see Masterthesis Alexandros
			// Ziampras)
			for (unsigned int n_radial = 1;
			     n_radial <=
				...
				for (unsigned int n_azimuthal = 0;
					...
					const double HoverR =
					    data[t_data::ASPECTRATIO](
						n_radial, n_azimuthal);
					const double sigma =
					    constants::sigma.get_code_value();
					const double T_star =
					    parameters::star_temperature;
					const double R_star =
					    parameters::star_radius;
					const double tau_eff =
					    data[t_data::TAU_EFF](n_radial,
								  n_azimuthal);
					const double eps =
					    0.5; // TODO: add a parameter
					// choose according to Chiang &
					// Goldreich (1997)
					const double dlogH_dlogr = 9.0 / 7.0;
					// use eq. 7 from Menou & Goodman (2004)
					// (rearranged), Qirr =
					// 2*(1-eps)*L_star/(4 pi
					// r^2)*(dlogH/dlogr - 1) * H/r *
					// 1/Tau_eff here we use (1-eps) =
					// parameters::heating_star_factor
					// L_star = 4 pi R_star^2 sigma_sb
					// T_star^4
					double qplus =
					    2 * (1 - eps); // 2*(1-eps)
					qplus *=
					    ...
					    pow2(
						R_star /
						distance); // *L_star/(4 pi r^2)
					qplus *= dlogH_dlogr -
						 1;       // *(dlogH/dlogr - 1)
					qplus *= HoverR;  // * H/r
					qplus /= tau_eff; // * 1/Tau_eff
					data[t_data::QPLUS](n_radial,
							    n_azimuthal) +=
					    ramping * qplus;
				}
			}
			...
			for (unsigned int n_azimuthal = 0;
				...
				for (int n_radial =
					 data[t_data::QPLUS].get_max_radial() -
					 (CPU_Rank == CPU_Highest ? GHOSTCELLS_B
								  : CPUOVERLAP);
				     n_radial >= (int)((CPU_Rank == 0)
							   ? GHOSTCELLS_B
							   : CPUOVERLAP);
				     n_radial--) {
					for (unsigned int n_theta = 0;
			...
			for (unsigned int n_radial = 1;
				...
				for (unsigned int n_azimuthal = 0;
					...
					// see Günther et. al (2004) or Phil
					// Armitage "Astrophysics of Planet
					// Format" p. 46
					double alpha =
					    (data[t_data::ASPECTRATIO](
						 n_radial, n_azimuthal) *
	...
	if (parameters::cooling_radiative_enabled) {
		...
		for (unsigned int n_radial = 0;
			...
			for (unsigned int n_azimuthal = 0;
			     ...
			     data[t_data::QMINUS].get_max_azimuthal();
			     ++n_azimuthal) {
				// calculate Rosseland mean opacity kappa.
				// opaclin needs values in cgs units
				temperatureCGS = data[t_data::TEMPERATURE](
						     n_radial, n_azimuthal) *
						 units::temperature;
				...
				// mean vertical optical depth: tau = 1/2 kappa
				// Sigma
				...
				//  tau_eff = 3/8 tau + sqrt(3)/4 +
				//  1/(4*tau+tau_min)
				data[t_data::TAU_EFF](n_radial, n_azimuthal) =
				    3.0 / 8.0 *
					data[t_data::TAU](n_radial,
							  n_azimuthal) +
				    sqrt(3.0) / 4.0 +
				    1.0 / (4.0 * data[t_data::TAU](
						     n_radial, n_azimuthal) +
					   0.01);
...
void SubStep3(t_data &data, double dt)
{
	...
	if (data[t_data::TAU_COOL].get_write_1D() ||
		...
		for (unsigned int n_radial = 0;
		     n_radial <= data[t_data::TAU_COOL].get_max_radial();
		     ++n_radial) {
			for (unsigned int n_azimuthal = 0;
			     n_azimuthal <=
			     data[t_data::TAU_COOL].get_max_azimuthal();
			     ++n_azimuthal) {
				data[t_data::TAU_COOL](n_radial, n_azimuthal) =
				    data[t_data::ENERGY](n_radial,
							 n_azimuthal) /
				    data[t_data::QMINUS](n_radial, n_azimuthal);
			}
		}
	...
	if (data[t_data::P_DIVV].get_write_1D() ||
		...
		for (unsigned int n_radial = 0;
			...
			for (unsigned int n_azimuthal = 0;
			     n_azimuthal <=
			     data[t_data::ENERGY_NEW].get_max_azimuthal();
			     ++n_azimuthal) {
				double pdivv =
				    (ADIABATICINDEX - 1.0) * dt *
				    data[t_data::DIV_V](n_radial, n_azimuthal) *
				    data[t_data::ENERGY](n_radial, n_azimuthal);
				data[t_data::P_DIVV](n_radial, n_azimuthal) =
				    pdivv;

				sum_without_ghost_cells(data.pdivv_total, pdivv,
							n_radial);
			}
		}
	}
	...
	for (unsigned int n_radial = 0;
		...
		for (unsigned int n_azimuthal = 0;
			...
			den
			= 1.0+(ADIABATICINDEX-1.0)*dt*data[t_data::DIV_V](n_radial,
			n_azimuthal);
			*/
			// ZEUS2D like
			/*num =
			...
			n_azimuthal)-dt*data[t_data::QMINUS](n_radial,
			n_azimuthal); den =
			(1.0+(dt/2.0)*(ADIABATICINDEX-1.0)*data[t_data::DIV_V](n_radial,
			n_azimuthal));
			*/
			...
			// n_azimuthal))
			// 	*pow3(data[t_data::ENERGY_INT](n_radial,
			// n_azimuthal));
			...
			const double inv_pow4 =
			    pow4((parameters::MU * (ADIABATICINDEX - 1.0)) /
				 (constants::R * data[t_data::DENSITY](
						     n_radial, n_azimuthal)));
			const double pow3_eint = pow3(
			    data[t_data::ENERGY_INT](n_radial, n_azimuthal));
			double alpha = 1.0 + 2.0 * H * 4.0 * constants::sigma /
						 constants::c * inv_pow4 *
						 pow3_eint;

			num = dt * data[t_data::QPLUS](n_radial, n_azimuthal) -
			      dt * data[t_data::QMINUS](n_radial, n_azimuthal) +
			      alpha * data[t_data::ENERGY_INT](n_radial,
							       n_azimuthal);
			den = alpha +
			      (ADIABATICINDEX - 1.0) * dt *
				  data[t_data::DIV_V](n_radial, n_azimuthal);

			data[t_data::ENERGY_NEW](n_radial, n_azimuthal) =
			    num / den;
		}
	}
}
...
void radiative_diffusion(t_data &data, double dt)
{
	...
	static double *SendInnerBoundary, *SendOuterBoundary,
	    *RecvInnerBoundary, *RecvOuterBoundary;
	...
	for (unsigned int n_radial = 1; n_radial <= Ka.get_max_radial() - 1;
		...
		for (unsigned int n_azimuthal = 0;
			...
			unsigned int n_azimuthal_plus =
			    (n_azimuthal == Ka.get_max_azimuthal()
				 ? 0
				 : n_azimuthal + 1);
			unsigned int n_azimuthal_minus =
			    (n_azimuthal == 0 ? Ka.get_max_azimuthal()
					      : n_azimuthal - 1);
			...
			double aspectratio =
			    0.5 *
			    (data[t_data::ASPECTRATIO](n_radial - 1,
						       n_azimuthal) +
			     data[t_data::ASPECTRATIO](n_radial, n_azimuthal));
			...
			double dT_dphi =
			    InvRinf[n_radial] *
			    (0.5 * (data[t_data::TEMPERATURE](
					n_radial - 1, n_azimuthal_plus) +
				    data[t_data::TEMPERATURE](
					n_radial, n_azimuthal_plus)) -
			     0.5 * (data[t_data::TEMPERATURE](
					n_radial - 1, n_azimuthal_minus) +
				    data[t_data::TEMPERATURE](
			...
			if ((n_radial > 1) &&
				...
				double temperature =
				    0.5 *
				    (data[t_data::TEMPERATURE](
					 n_radial_adjusted - 1, n_azimuthal) +
				     data[t_data::TEMPERATURE](
					 n_radial_adjusted, n_azimuthal));
				double density =
				    0.5 *
				    (data[t_data::DENSITY](
					 n_radial_adjusted - 1, n_azimuthal) +
				     data[t_data::DENSITY](n_radial_adjusted,
							   n_azimuthal));
				double aspectratio =
				    0.5 *
				    (data[t_data::ASPECTRATIO](
				...
				double densityCGS =
				    density / (parameters::density_factor * H) *
				    units::density;

				double kappaCGS = opacity::opacity(
				    densityCGS, temperatureCGS);
				double kappa =
				    parameters::kappa_factor * kappaCGS *
				    units::opacity.get_inverse_cgs_factor();
			...
			Ka(n_radial, n_azimuthal) =
			    8.0 * 4.0 * constants::sigma.get_code_value() *
	...
	for (unsigned int n_radial = 1; n_radial <= Kb.get_max_radial() - 1;
		...
		for (unsigned int n_azimuthal = 0;
			...
			unsigned int n_azimuthal_minus =
			    (n_azimuthal == 0 ? Kb.get_max_azimuthal()
					      : n_azimuthal - 1);
			...
			double aspectratio =
			    0.5 *
			    (data[t_data::ASPECTRATIO](n_radial,
						       n_azimuthal_minus) +
			     data[t_data::ASPECTRATIO](n_radial, n_azimuthal));
			...
			dphi*n_azimuthal, R,
			lambda,dT_dphi,dT_dr,nabla_T,temperature,H);
			}*/

			Kb(n_radial, n_azimuthal) =
			    8 * 4 * constants::sigma.get_code_value() * lambda *
			    H * pow3(temperature) * denom;
			// Kb(n_radial, n_azimuthal)
			// = 16.0*parameters::density_factor*constants::sigma.get_code_value()*lambda*H*pow3(temperature)*denom;
		}
	}
	...
	for (unsigned int n_radial = 1;
		...
		for (unsigned int n_azimuthal = 0;
			...
			A(n_radial, n_azimuthal) =
			    common_AC * Ka(n_radial, n_azimuthal) *
			    Ra[n_radial] * InvDiffRmed[n_radial];
			C(n_radial, n_azimuthal) =
			    common_AC * Ka(n_radial + 1, n_azimuthal) *
			    Ra[n_radial + 1] * InvDiffRmed[n_radial + 1];

			// 1/(r^2 dphi^2)
			double common_DE =
			    common_factor / (pow2(Rb[n_radial]) * pow2(dphi));
			D(n_radial, n_azimuthal) =
			    common_DE * Kb(n_radial, n_azimuthal);
			E(n_radial, n_azimuthal) =
			    common_DE *
			    Kb(n_radial, n_azimuthal == Kb.get_max_azimuthal()
					     ? 0
					     : n_azimuthal + 1);

			B(n_radial, n_azimuthal) = -A(n_radial, n_azimuthal) -
						   C(n_radial, n_azimuthal) -
						   D(n_radial, n_azimuthal) -
						   E(n_radial, n_azimuthal) +
						   1.0;
	...
	while ((norm_change > 1e-12) &&
		...
		for (unsigned int n_radial = 1;
			...
			for (unsigned int n_azimuthal = 0;
			     ...
			     data[t_data::TEMPERATURE].get_max_azimuthal();
			     ++n_azimuthal) {
				double old_value = data[t_data::TEMPERATURE](
				    n_radial, n_azimuthal);
				unsigned int n_azimuthal_plus =
				    (n_azimuthal == data[t_data::TEMPERATURE]
							.get_max_azimuthal()
					 ? 0
					 : n_azimuthal + 1);
				unsigned int n_azimuthal_minus =
				...
				data[t_data::TEMPERATURE](n_radial,
							  n_azimuthal) =
				    (1.0 - omega) * data[t_data::TEMPERATURE](
							n_radial, n_azimuthal) -
				...
				// only non ghostcells to norm and don't count
				// overlap cell's twice
				bool isnot_ghostcell_rank_0 =
				    n_radial > ((CPU_Rank == 0) ? GHOSTCELLS_B
								: CPUOVERLAP);
				bool isnot_ghostcell_rank_highest =
				    (n_radial < (data[t_data::TEMPERATURE]
						     .get_max_radial() -
						 ((CPU_Rank == CPU_Highest)
						      ? GHOSTCELLS_B
						      : CPUOVERLAP)));

				if (isnot_ghostcell_rank_0 &&
				    isnot_ghostcell_rank_highest) {
					absolute_norm +=
					    pow2(old_value -
						 data[t_data::TEMPERATURE](
						     n_radial, n_azimuthal));
		...
		MPI_Allreduce(&tmp, &absolute_norm, 1, MPI_DOUBLE, MPI_SUM,
			      MPI_COMM_WORLD);
		absolute_norm =
		    sqrt(absolute_norm) / (GlobalNRadial * NAzimuthal);
		...
		if (CPU_Rank % 2 == 0) {
			if (CPU_Rank != 0) {
				MPI_Isend(SendInnerBoundary,
					  NAzimuthal * CPUOVERLAP, MPI_DOUBLE,
					  CPU_Prev, 0, MPI_COMM_WORLD, &req1);
				MPI_Irecv(RecvInnerBoundary,
					  NAzimuthal * CPUOVERLAP, MPI_DOUBLE,
					  CPU_Prev, 0, MPI_COMM_WORLD, &req2);
			}
			if (CPU_Rank != CPU_Highest) {
				MPI_Isend(SendOuterBoundary,
					  NAzimuthal * CPUOVERLAP, MPI_DOUBLE,
					  CPU_Next, 0, MPI_COMM_WORLD, &req3);
				MPI_Irecv(RecvOuterBoundary,
					  NAzimuthal * CPUOVERLAP, MPI_DOUBLE,
					  CPU_Next, 0, MPI_COMM_WORLD, &req4);
			}
		} else {
			if (CPU_Rank != CPU_Highest) {
				MPI_Irecv(RecvOuterBoundary,
		...
		if (CPU_Rank != 0) {
			...
			memcpy(data[t_data::TEMPERATURE].Field,
			       RecvInnerBoundary, l * sizeof(double));
		}

		if (CPU_Rank != CPU_Highest) {
			MPI_Wait(&req3, &global_MPI_Status);
			MPI_Wait(&req4, &global_MPI_Status);
			memcpy(data[t_data::TEMPERATURE].Field + oo,
			       RecvOuterBoundary, l * sizeof(double));
		}
	}
	...
	if (iterations == parameters::radiative_diffusion_max_iterations) {
		logging::print_master(
		    LOG_WARNING
		    "Maximum iterations (%u) reached in radiative_diffusion "
		    "(omega = %lg). Norm is %lg with a last change of %lg.\n",
	...
	for (unsigned int n_radial = 0;
		...
		for (unsigned int n_azimuthal = 0;
			...
			data[t_data::ENERGY](n_radial, n_azimuthal) =
			    data[t_data::TEMPERATURE](n_radial, n_azimuthal) *
			    data[t_data::DENSITY](n_radial, n_azimuthal) /
			    (ADIABATICINDEX - 1.0) / parameters::MU *
			    constants::R;
		}
	}
}
		     ...
		     double deltaT)
{
	...
	for (unsigned int n_radial = 0;
	     n_radial <= v_azimuthal.get_max_radial(); ++n_radial) {
		v_mean[n_radial] = 0.0;
		for (unsigned int n_azimuthal = 0;
	...
	for (unsigned int n_radial = One_or_active; n_radial < Max_or_active;
		...
		double dxAzimuthal = Rmed[n_radial] * 2.0 * PI /
		...
		for (unsigned int n_azimuthal = 0;
			...
			// radial motion limit
			invdt2 =
			    fabs(v_radial(n_radial, n_azimuthal)) / dxRadial;
			...
			if (parameters::artificial_viscosity ==
			    parameters::artificial_viscosity_SN) {
				// TODO: Change to sizes defined by constants of
				// compiler
				if (dvRadial >= 0.0) {
					dvRadial = 1e-10;
				} else {
				...
				invdt4 = 4.0 *
					 pow2(parameters::
						  artificial_viscosity_factor) *
					 max(dvRadial / dxRadial,
					     dvAzimuthal / dxAzimuthal);
			} else {
				invdt4 = 0.0;
			}
	...
	// for (unsigned int n_radial = Zero_or_active; n_radial <
	// MaxMO_or_active; ++n_radial) {
	for (unsigned int n_radial =
		 1 + (CPUOVERLAP - 1) * (CPU_Rank > 0 ? 1 : 0);
	     n_radial <
	     NRadial - 2 -
	...
	if (debug) {
		...
		logging::print(LOG_DEBUG
			       ...
			       Rmed[n_radial_debug]);
		logging::print(
		    LOG_DEBUG "Sound speed limit              : %g\n", itdbg1);
		logging::print(
		    LOG_DEBUG "Radial motion limit            : %g\n", itdbg2);
		logging::print(
		    LOG_DEBUG "Residual circular motion limit : %g\n", itdbg3);
		...
		logging::print(
		    LOG_DEBUG "Kinematic viscosity limit      : %g\n", itdbg5);
		logging::print(
		    LOG_DEBUG "Limit time step for this cell  : %g\n", mdtdbg);
		logging::print(LOG_DEBUG
			       "Limit time step adopted        : %g\n",
			       dtGlobal);
		if (dtGlobal < mdtdbg) {
			logging::print(
			    LOG_DEBUG "Discrepancy arise either from shear.\n");
			logging::print(LOG_DEBUG
...
void compute_sound_speed(t_data &data, bool force_update)
{
	...
	for (unsigned int n_radial = 0;
	     n_radial <= data[t_data::SOUNDSPEED].get_max_radial();
	     ++n_radial) {
		for (unsigned int n_azimuthal = 0;
		     n_azimuthal <=
		     data[t_data::SOUNDSPEED].get_max_azimuthal();
		     ++n_azimuthal) {
			if (parameters::Adiabatic) {
				/*if
				((ADIABATICINDEX*(ADIABATICINDEX-1.0)*data[t_data::ENERGY](n_radial,
				n_azimuthal)/data[t_data::DENSITY](n_radial,
				n_azimuthal)) < 0) { logging::print("SoundSpeed
				calc: %g < 0! density=%g energy=%g in cell
				(%u,%u)\n",(ADIABATICINDEX*(ADIABATICINDEX-1.0)*data[t_data::ENERGY](n_radial,
				n_azimuthal)/data[t_data::DENSITY](n_radial,
				n_azimuthal)), data[t_data::DENSITY](n_radial,
				n_azimuthal), data[t_data::ENERGY](n_radial,
				n_azimuthal), n_radial, n_azimuthal);
				}*/
				data[t_data::SOUNDSPEED](n_radial,
							 n_azimuthal) =
				    sqrt(ADIABATICINDEX *
				...
				data[t_data::SOUNDSPEED](n_radial,
							 n_azimuthal) =
				    sqrt(ADIABATICINDEX * constants::R /
					 parameters::MU *
					 data[t_data::TEMPERATURE](
					     n_radial, n_azimuthal));
			} else { // isothermal
				...
				data[t_data::SOUNDSPEED](n_radial,
							 n_azimuthal) =
				    ASPECTRATIO_REF *
				    sqrt(constants::G * M / Rb[n_radial]) *
...
void compute_aspect_ratio(t_data &data, bool force_update)
{
	...
	for (unsigned int n_radial = 0;
		...
		for (unsigned int n_azimuthal = 0;
		     ...
		     ++n_azimuthal) {
			if (parameters::Adiabatic || parameters::Polytropic) {
				// h = H/r = c_s,iso / v_k = c_s/sqrt(gamma) /
				// v_k
				data[t_data::ASPECTRATIO](n_radial,
							  n_azimuthal) =
				    data[t_data::SOUNDSPEED](n_radial,
		...
		for (unsigned int n_azimuthal = 0;
			...
			if (parameters::Adiabatic) {
				...
				data[t_data::PRESSURE](n_radial, n_azimuthal) =
				    data[t_data::DENSITY](n_radial,
							  n_azimuthal) *
				    pow2(data[t_data::SOUNDSPEED](
					n_radial, n_azimuthal)) /
				    ADIABATICINDEX;
			} else { // Isothermal
				// since SoundSpeed is not update from
				// initialization, cs remains axisymmetric
				data[t_data::PRESSURE](n_radial, n_azimuthal) =
				    data[t_data::DENSITY](n_radial,
							  n_azimuthal) *
				    pow2(data[t_data::SOUNDSPEED](n_radial,
								  n_azimuthal));
			}
		}
	}
...
void compute_temperature(t_data &data, bool force_update)
{
	...
	for (unsigned int n_radial = 0;
	     n_radial <= data[t_data::TEMPERATURE].get_max_radial();
	     ++n_radial) {
		for (unsigned int n_azimuthal = 0;
		     n_azimuthal <=
		     data[t_data::TEMPERATURE].get_max_azimuthal();
			...
			if (parameters::Adiabatic) {
				...
				data[t_data::TEMPERATURE](n_radial,
				    ...
				    parameters::MU / constants::R *
				    POLYTROPIC_CONSTANT *
				    pow(data[t_data::DENSITY](n_radial,
							      n_azimuthal),
					ADIABATICINDEX - 1.0);
			} else { // Isothermal
				data[t_data::TEMPERATURE](n_radial,
...
void compute_rho(t_data &data, bool force_update)
{
	...
	for (unsigned int n_radial = 0;
		...
		for (unsigned int n_azimuthal = 0;
		     ...
		     ++n_azimuthal) {
			if (parameters::Adiabatic) {
				H = data[t_data::ASPECTRATIO](n_radial,
							      n_azimuthal) *
				    Rb[n_radial];
			} else {
				H = ASPECTRATIO_REF *
				    pow(Rb[n_radial], 1.0 + FLARINGINDEX);
			}
