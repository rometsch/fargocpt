...
std::string t_unit::get_cgs_factor_symbol()
{
	...
	std::stringstream us;
	us.precision(16);
	us << std::scientific << m_cgs_factor << " "
	   << std::string(m_cgs_symbol);
	return us.str();
}
...
void print_code_units()
{
	...
	logging::print_master(
	    LOG_VERBOSE "                     length:       l0 = %.16g %s\n",
	    length.get_cgs_factor(), length.get_cgs_symbol());
	logging::print_master(
	    LOG_VERBOSE "                       mass:       m0 = %.16g %s\n",
	...
	logging::print_master(
	    LOG_VERBOSE "                    density:     rho0 = %.16g %s\n",
...
void write_code_unit_file()
{
	...
	if (CPU_Master) {
		if (asprintf(&fd_filename, "%s%s", OUTPUTDIR, "units.dat") ==
		    -1) {
			logging::print_master(
		...
		fprintf(fd,
			...
			time.get_cgs_factor() / (24 * 60 * 60 * 365.2425));
		fprintf(
		    fd, "#               temperature:       T0 = %.16g %s\n",
		    temperature.get_cgs_factor(), temperature.get_cgs_symbol());
		fprintf(fd,
			"#                    energy:       E0 = %.16g %s\n",
			energy.get_cgs_factor(), energy.get_cgs_symbol());
		fprintf(fd,
			"#            energy density:       e0 = %.16g %s\n",
		...
		fprintf(fd,
			...
			density.get_cgs_factor(), density.get_cgs_symbol());
		fprintf(fd,
			"#           surface density:   Sigma0 = %.16g %s\n",
			surface_density.get_cgs_factor(),
			surface_density.get_cgs_symbol());
		fprintf(
		    fd, "#               energy flux:       S0 = %.16g %s\n",
		    energy_flux.get_cgs_factor(), energy_flux.get_cgs_symbol());
		fprintf(fd,
			"#                  velocity:       v0 = %.16g %s\n",
			velocity.get_cgs_factor(), velocity.get_cgs_symbol());
		fprintf(fd,
			"#              acceleration:       a0 = %.16g %s\n",
			acceleration.get_cgs_factor(),
			acceleration.get_cgs_symbol());
		fprintf(fd,
		...
		fprintf(fd,
			...
			pressure.get_cgs_factor(), pressure.get_cgs_symbol());
		fprintf(fd,
			"# Syntax: base unit <tab> value <tab> unit name\n");
