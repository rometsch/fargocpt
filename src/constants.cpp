...
{
    logging::verbose_master("Code constants:\n");
#ifndef NDEBUG
    logging::verbose_master("     gravitational constant: %8s = %15g = %15g %s\n",
			  G.get_symbol(), G.get_code_value(), G.get_cgs_value(),
			  G.get_cgs_unit_symbol());
#else
    logging::verbose_master("     gravitational constant: 1 (hardcoded, compile without NDEBUG to calculate it dynamically)\n");
#endif
    logging::verbose_master("         Boltzmann constant: %8s = %15g = %15g %s\n",
			  k_B.get_symbol(), k_B.get_code_value(),
			  k_B.get_cgs_value(), k_B.get_cgs_unit_symbol());
    logging::verbose_master("             molecular mass: %8s = %15g = %15g %s\n",
			  m_u.get_symbol(), m_u.get_code_value(),
			  m_u.get_cgs_value(), m_u.get_cgs_unit_symbol());
    logging::verbose_master("            Planck constant: %8s = %15g = %15g %s\n",
			  h.get_symbol(), h.get_code_value(), h.get_cgs_value(),
			  h.get_cgs_unit_symbol());
    logging::verbose_master("             speed of light: %8s = %15g = %15g %s\n",
			  c.get_symbol(), c.get_code_value(), c.get_cgs_value(),
			  c.get_cgs_unit_symbol());
#ifndef NDEBUG
    logging::verbose_master("      specific gas constant: %8s = %15g = %15g %s\n",
			  R.get_symbol(), R.get_code_value(), R.get_cgs_value(),
			  R.get_cgs_unit_symbol());
#else
    logging::verbose_master("      specific gas constant: 1 (hardcoded, compile without NDEBUG to calculate it dynamically)\n");
#endif
    logging::verbose_master("  Stefan-Boltzmann constant: %8s = %15g = %15g %s\n",
