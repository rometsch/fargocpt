				 ...
				 std::string description)
{
	...
	case damping_none:
		...
		description_outer = "Damping " + description +
				    " is disabled at outer boundary.";
		break;
	case damping_mean:
		damping_type.outer_damping_function =
		    &boundary_conditions::damping_single_outer_mean;
		description_outer = "Damping " + description +
				    " to mean value at outer boundary.";
		break;
	case damping_initial:
		damping_type.outer_damping_function =
		    &boundary_conditions::damping_single_outer;
		description_outer = "Damping " + description +
				    " to initial value at outer boundary.";
		break;
	case damping_zero:
		damping_type.outer_damping_function =
...
void read(char *filename, t_data &data)
{
	...
	data[t_data::ALPHA_REYNOLDS_MEAN_1D].set_write(
	    config::value_as_bool_default("WriteRadialAlphaReynoldsMean",
					  false));
	...
	log_after_steps =
	    config::value_as_unsigned_int_default("LogAfterSteps", 0);
	...
	if (config::key_exists("WriteLightCurvesRadii")) {
		...
		char *lightcurves_radii_string = new char[strlen(
	...
	if (config::key_exists("DampingVRadial"))
		die("DampingVRadial flag is decrepated used "
		    "DampingVRadialInner and DampingVRadialOuter instead!");

	tmp_damping_inner = config::value_as_boudary_damping_default(
	    "DampingVRadialInner", "None");
	tmp_damping_outer = config::value_as_boudary_damping_default(
	    "DampingVRadialOuter", "None");
	...
	if (config::key_exists("DampingVAzimuthal"))
		die("DampingVRadial flag is decrepated used "
		    "DampingVAzimuthalInner and DampingVAzimuthalOuter "
		    "instead!");
	...
	if (config::key_exists("DampingSurfaceDensity"))
		die("DampingSurfaceDensity flag is decrepated used "
		    "DampingSurfaceDensityInner and DampingSurfaceDensityOuter "
		    "instead!");
	...
	damping_vector.push_back(write_damping_type(
	    tmp_damping_inner, tmp_damping_outer, t_data::DENSITY,
	    t_data::DENSITY0, "SurfaceDensity"));

	tmp_damping_inner =
	    config::value_as_boudary_damping_default("DampingEnergy", "None");
	if (config::key_exists("DampingEnergy"))
		die("DampingEnergy flag is decrepated used DampingEnergyInner "
		    "and DampingEnergyOuter instead!");

	tmp_damping_inner = config::value_as_boudary_damping_default(
	    "DampingEnergyInner", "None");
	tmp_damping_outer = config::value_as_boudary_damping_default(
	    "DampingEnergyOuter", "None");

	damping_vector.push_back(
	    write_damping_type(tmp_damping_inner, tmp_damping_outer,
			       t_data::ENERGY, t_data::ENERGY0, "Energy"));
	...
	heating_star_enabled =
	    config::value_as_bool_default("HeatingStar", false);
	heating_star_factor =
	...
	radiative_diffusion_max_iterations =
	    config::value_as_unsigned_int_default(
		"RadiativeDiffusionMaxIterations", 50000);

	zbuffer_size =
	    config::value_as_unsigned_int_default("zbufferSize", 100);
	zbuffer_maxangle = config::value_as_double_default("zbufferMaxAngle",
							   10.0 / 180.0 * PI);
	...
	default:
		die("Invalid setting for SigmaCondition: %s",
		    config::value_as_string_default("SigmaCondition",
						    "Profile"));
	}

	if (config::key_exists("SigmaFilename")) {
		if (strlen(config::value_as_string_default("SigmaFilename",
							   "")) > 0) {
			sigma_filename =
			    new char[strlen(config::value_as_string_default(
				"SigmaFilename", ""))];
			strcpy(sigma_filename, config::value_as_string_default(
						   "SigmaFilename", ""));
		}
	}
	...
	default:
		die("Invalid setting for EnergyCondition: %s",
		    config::value_as_string_default("EnergyCondition",
						    "Profile"));
	}

	if (config::key_exists("EnergyFilename")) {
		if (strlen(config::value_as_string_default("EnergyFilename",
							   "")) > 0) {
			energy_filename =
			    new char[strlen(config::value_as_string_default(
				"EnergyFilename", ""))];
			strcpy(energy_filename, config::value_as_string_default(
						    "EnergyFilename", ""));
		}
	}
	...
	sigma_random_factor =
	    config::value_as_double_default("RandomFactor", 0.1);
	sigma_feature_size =
	...
	default:
		die("Invalid setting for ArtificialViscosity: %s",
		    config::value_as_string_default("ArtificialViscosity",
						    "SN"));
	}
	artificial_viscosity_dissipation = config::value_as_bool_default(
	...
	if (config::key_exists("CVNR")) {
		die("Parameter CVNR has been renamed to "
	...
	integrate_planets =
	    config::value_as_bool_default("IntegratePlanets", true);
	...
	exitOnDeprecatedSetting(
	    "FeelsDisk",
	    "Replaced by parameter DiskFeedback for clarification since it "
	    "affects more than just the star.",
	...
	case 'c': // Constant
		...
		kappa_const =
		    config::value_as_double_default("KappaConst", 1.0);
		break;
	default:
	...
	// star parameters
	star_temperature =
	    config::value_as_double_default("StarTemperature", 5778);
	star_radius =
	    config::value_as_double_default("StarRadius", 0.009304813);

	// boundary layer parameters
	radial_viscosity_factor =
	    config::value_as_double_default("RadialViscosityFactor", 1.);
	vrad_fraction_of_kepler =
	    config::value_as_double_default("VRadIn", 1.6e-3);
	...
	particle_radius =
	    config::value_as_double_default("ParticleRadius", 100.0);
	particle_eccentricity =
	    config::value_as_double_default("ParticleEccentricity", 0.0);
	particle_density =
	    config::value_as_double_default("ParticleDensity", 2.65);
	...
	particle_minimum_escape_radius = config::value_as_double_default(
	    "ParticleMinimumEscapeRadius", RMIN);
	particle_maximum_escape_radius = config::value_as_double_default(
	    "ParticleMaximumEscapeRadius", RMAX);
	...
	case 's': // Semi-implicit
		...
		if (!particle_gas_drag_enabled) {
			logging::print_master(
			    LOG_ERROR "Do not use semi-implicit particle "
				      "integrator without gas drag, use the "
				      "explicit integrator instead.\n");
		}
	...
	case 'i': // Implicit
		...
		if (!particle_gas_drag_enabled) {
			logging::print_master(
			    LOG_ERROR
			    "Do not use implicit particle integrator without "
			    "gas drag, use the explicit integrator instead.\n");
		}

		break;
	default:
		die("Invalid setting for Particle Integrator: %s	with "
		    "key %s",
		    config::value_as_string_default("ParticleIntegrator", "s"),
		    tolower(*config::value_as_string_default(
			"ParticleIntegrator", "s")));
	}

	if (CartesianParticles && ((integrator == integrator_implicit) ||
				   integrator == integrator_semiimplicit)) {
		// implicit and semiimplicit integrator only implemented in
		// polar coordiantes, but forces can be calculated in cartesian
		// coordinates
	...
	if (particle_disk_gravity_enabled && (!self_gravity)) {
		logging::print_master(
		    LOG_ERROR "Cannot enable particle_disk_gravity_enabled "
			      "while self_gravity is off!\n");
		PersonalExit(1);
	}
	...
	if (particle_minimum_escape_radius < RMIN) {
		logging::print_master(
		    LOG_WARNING "particle_minimum_escape_radius can't be "
				"smaller than the inner radius of the domain. "
				"Setting particle_minimum_escape_radius to "
	...
	if (particle_maximum_escape_radius > RMAX) {
		logging::print_master(
		    LOG_WARNING "particle_maximum_escape_radius can't be "
				"larger than the outer radius of the domain. "
				"Setting particle_maximum_escape_radius to "
				"outer radius of the domain.\n");
		particle_maximum_escape_radius = RMAX;
	}

	particle_maximum_escape_radius_sq =
	    pow2(particle_maximum_escape_radius) -
...
void summarize_parameters()
{
	...
	case artificial_viscosity_none:
		logging::print_master(LOG_INFO
				      "Using no artificial viscosity.\n");
		break;
	case artificial_viscosity_TW:
		logging::print_master(LOG_INFO
				      "Using Tscharnuter-Winkler (1979) "
				      "artificial viscosity with C = %lf.\n",
				      parameters::artificial_viscosity_factor);
		logging::print_master(
		    LOG_INFO "Artificial viscosity is %s for dissipation.\n",
		    parameters::artificial_viscosity_dissipation ? "used"
								 : "not used");
		break;
	case artificial_viscosity_SN:
		logging::print_master(LOG_INFO
				      "Using Stone-Norman (1991, ZEUS-2D) "
				      "artificial viscosity with C = %lf.\n",
				      parameters::artificial_viscosity_factor);
		logging::print_master(
		    LOG_INFO "Artificial viscosity is %s for dissipation.\n",
	...
	case boundary_condition_reflecting:
		logging::print_master(LOG_INFO
				      "Using 'reflecting boundary condition' "
				      "at inner boundary.\n");
		break;
	case boundary_condition_nonreflecting:
		logging::print_master(LOG_INFO
				      "Using 'nonreflecting boundary "
				      "condition' at inner boundary.\n");
		break;
	case boundary_condition_evanescent:
		logging::print_master(LOG_INFO
				      "Using 'evanescent boundary condition' "
				      "at inner boundary.\n");
		break;
	case boundary_condition_viscous_outflow:
		logging::print_master(LOG_INFO
				      "Using 'viscous outflow boundary "
				      "condition' at inner boundary.\n");
		break;
	case boundary_condition_boundary_layer:
		logging::print_master(LOG_INFO
				      "Using 'boundary layer boundary "
				      "conditions' at inner boundary.\n");
		break;
	case boundary_condition_keplerian:
		logging::print_master(LOG_INFO
				      "Using 'keplarian boundary conditions' "
				      "at inner boundary.\n");
		break;
	}
	...
	if (parameters::massoverflow) {
		logging::print_master(
		    LOG_INFO "Mass Transfer of %g M_0/orbit will be spread on "
			     "%i gridcells (sigma = %g).\n",
	...
	if (boundary_inner == boundary_condition_boundary_layer) {
		logging::print_master(LOG_INFO
				      "Boundary Layer: Radial velocity at "
				      "inner boundary is %e * V_Kepler.\n",
				      vrad_fraction_of_kepler);
		logging::print_master(LOG_INFO
				      "Boundary Layer: Stellar rotation rate "
				      "is %f * Om_Kepler.\n",
				      stellar_rotation_rate);
	}
	if (boundary_outer == boundary_condition_boundary_layer) {
		logging::print_master(
		    LOG_INFO "Boundary Layer: Mass Accretion Rate is %g Solar "
			     "Masses per Year.\n",
		    mass_accretion_rate * units::mass.get_cgs_factor() /
			units::time.get_cgs_factor() * units::cgs_Year /
			units::cgs_Msol);
	}
	logging::print_master(LOG_INFO "Boundary Layer: Radial Viscosity is "
				       "multiplied by a factor of %f.\n",
			      radial_viscosity_factor);
	...
	if (damping) {
		...
		logging::print_master(LOG_INFO
				      "Damping at boundaries is disabled.\n");
	}
	...
	logging::print_master(LOG_INFO
			      "Heating from star is %s. Using %s model with "
			      "ramping time of %g and a total factor %g.\n",
	...
	logging::print_master(LOG_INFO "Heating from viscous dissipation is "
				       "%s. Using a total factor of %g.\n",
			      heating_viscous_enabled ? "enabled" : "disabled",
			      heating_viscous_factor);
	logging::print_master(
	    LOG_INFO "Cooling (beta) is %s. Using beta = %g.\n",
	...
	logging::print_master(
	    ...
	    cooling_radiative_factor);
	logging::print_master(
	    LOG_INFO "Radiative diffusion is %s. Using %s omega = %lf with a "
		     "maximum %u interations.\n",
	...
	case opacity_bell:
		logging::print_master(
		    LOG_INFO "Opacity uses tables from Bell & Lin, 1994\n");
		break;
	case opacity_zhu:
		logging::print_master(
		    LOG_INFO "Opacity uses tables from Zhu et al., 2012\n");
		break;
	case opacity_kramers:
		logging::print_master(LOG_INFO
				      "Kramers opacity and constant electron "
				      "scattering (Thomson) used.\n");
		break;
	case opacity_const_op:
		logging::print_master(LOG_INFO
				      "Using constant opacity kappa_R = %e.\n",
				      kappa_const);
		break;
	}
	...
	if (write_lightcurves) {
		...
		for (unsigned int i = 0; i < lightcurves_radii.size(); ++i) {
			int size =
			    asprintf(&temp, "%lf, ", lightcurves_radii[i]);
			buffer = (char *)realloc(buffer, (pos + size + 1) *
							     sizeof(char));
	...
	if (integrate_particles) {
		logging::print_master(
		    LOG_INFO "Using %u particles with a radius of %g and a "
			     "density of %g.\n",
		    number_of_particles, particle_radius, particle_density);
		logging::print_master(
		    LOG_INFO
		    "Distributing particles with a r^%.2g profile from %g to "
		    "%g with a eccentricity from 0.0 to %g.\n",
		    particle_slope, particle_minimum_radius,
		    particle_maximum_radius, particle_eccentricity);
		logging::print_master(
		    LOG_INFO "Particles are considered escaped from the system "
			     "when they reach a distance of %g or %g.\n",
		...
		logging::print_master(LOG_INFO "Particles gas drag is %s.\n",
				      particle_gas_drag_enabled ? "enabled"
								: "disabled");
		logging::print_master(
		    LOG_INFO "Particles disk gravity is %s.\n",
		    particle_disk_gravity_enabled ? "enabled" : "disabled");
		switch (integrator) {
		case integrator_explicit:
			logging::print_master(
			    LOG_INFO "Particles use the explicit integrator\n");
			break;
		case integrator_adaptive:
			logging::print_master(LOG_INFO
					      "Particles use the (explicit) "
					      "adaptive integrator\n");
			break;
		case integrator_semiimplicit: // Semi-implicit
			logging::print_master(
			    LOG_INFO
			    "Particles use the semiimplicit integrator\n");
			break;
		case integrator_implicit: // Implicit
			logging::print_master(
			    LOG_INFO "Particles use the implicit integrator\n");
			break;
		default:
			die("Invalid setting for Particle Integrator: %s",
			    config::value_as_string_default(
				"ParticleIntegrator", "s"));
		}
	}
}
...
void write_grid_data_to_file()
{
	...
	if (CPU_Master) {
		if (asprintf(&fd_filename, "%s%s", OUTPUTDIR,
			     "dimensions.dat") == -1) {
			logging::print_master(
			    LOG_ERROR "Not enough memory for string buffer.\n");
			PersonalExit(1);
		}
		...
		fprintf(fd, "#RMIN\tRMAX\tPHIMIN\tPHIMAX          "
			    "\tNRAD\tNAZ\tNGHRAD\tNGHAZ\tRadial_spacing\n");
