...
void check_free_space(t_data &data)
{
	...
	// go thru all polargrids in output_polargrids and check if they are
	// going to be put out
	for (unsigned int i = 0; i < t_data::N_POLARGRID_TYPES; ++i) {
		if (data[(t_data::t_polargrid_type)i].get_write_1D()) {
			space_needed +=
			    data[(t_data::t_polargrid_type)i].bytes_needed_1D();
			number_of_files += 1;
		}

		if (data[(t_data::t_polargrid_type)i].get_write_2D()) {
			space_needed +=
			    data[(t_data::t_polargrid_type)i].bytes_needed_2D();
			number_of_files += 1;
		}
	}
	...
	if ((statvfs(directory_name, &fiData)) < 0) {
		logging::print_master(
		    LOG_WARNING "Couldn't stat filesystem. You have to check "
				"for enough free space manually!\n");
	} else {
		// free space of device, more precisely number of space
		// available to non-priv processes (like us)
		unsigned long int free_space =
		    fiData.f_bavail * fiData.f_frsize;

		logging::print_master(LOG_INFO "    Space Available: %.2f GB\n",
				      (double)free_space / 1024.0 / 1024.0 /
					  1024.0);

		if (space_needed > free_space) {
			logging::print_master(
			    LOG_WARNING
			    "There is not enough space for all outputs! The "
			    "program will fail at same point!\n");
		}
	}
		      ...
		      unsigned int nTimeStep, bool force_update)
{
	...
	if (CPU_Master) {
		...
		if (fd == NULL) {
			logging::print_master(
			    LOG_ERROR
			    "Can't write 'Quantities.dat' file. Aborting.\n");
			PersonalExit(1);
		}
		...
		if (!fd_created) {
			...
			fprintf(fd, "%s",
				text_file_variable_description(
				    quantities_file_column,
	...
	auto disk_quantities =
	    reduce_disk_quantities(data, timestep, force_update);
	double disk_eccentricity = disk_quantities[0];
	double disk_periastron = disk_quantities[1];

	// computate absolute deviation from start values (this has to be done
	// on all nodes!)
	...
	MPI_Reduce(&data.pdivv_total, &pdivv_total, 1, MPI_DOUBLE, MPI_SUM, 0,
		   MPI_COMM_WORLD);
	MPI_Reduce(&MassDelta.InnerPositive, &InnerPositive, 1, MPI_DOUBLE,
		   MPI_SUM, 0, MPI_COMM_WORLD);
	MPI_Reduce(&MassDelta.InnerNegative, &InnerNegative, 1, MPI_DOUBLE,
		   MPI_SUM, 0, MPI_COMM_WORLD);
	MPI_Reduce(&MassDelta.OuterPositive, &OuterPositive, 1, MPI_DOUBLE,
		   MPI_SUM, 0, MPI_COMM_WORLD);
	MPI_Reduce(&MassDelta.OuterNegative, &OuterNegative, 1, MPI_DOUBLE,
		   MPI_SUM, 0, MPI_COMM_WORLD);
	...
	MPI_Reduce(&MassDelta.WaveDampingNegative, &WaveDampingNegative, 1,
		   MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	MPI_Reduce(&MassDelta.FloorPositive, &FloorPositive, 1, MPI_DOUBLE,
		   MPI_SUM, 0, MPI_COMM_WORLD);

	if (CPU_Master) {
		// print to logfile
		fprintf(fd,
			"%u\t%u\t%#.16e\t%#.16e\t%#.16e\t%#.16e\t%#.16e\t%#."
			"16e\t%#.16e\t%#.16e\t%#.16e\t%#.16e\t%#.16e\t%#.16e\t%"
			"#.16e\t%#.16e\t%#.16e\t%#.16e\t%#.16e\t%#.16e\t%#."
			"16e\t%#.16e\t%#.16e\t%#.16e\t%#.16e\n",
			timestep, nTimeStep, PhysicalTime, totalMass,
			diskRadius, totalAngularMomentum, totalEnergy,
			internalEnergy, kinematicEnergy, gravitationalEnergy,
			radialKinematicEnergy, azimuthalKinematicEnergy,
			disk_eccentricity, disk_periastron, qplus, qminus,
			pdivv_total, InnerPositive, InnerNegative,
			OuterPositive, OuterNegative, WaveDampingPositive,
			WaveDampingNegative, FloorPositive, scale_height);
...
void write_misc(unsigned int timestep)
{
	...
	if (CPU_Master) {
		if (asprintf(&fd_filename, "%s%s", OUTPUTDIR, "misc.dat") ==
		    -1) {
			logging::print_master(
			    LOG_ERROR "Not enough memory for string buffer.\n");
			PersonalExit(1);
		...
		if (!fd_created) {
			...
			fprintf(fd, "%s",
				text_file_variable_description(
				    misc_file_columns, misc_file_variables)
				    .c_str());
			fd_created = true;
		}
	}
	...
	if (CPU_Master) {
		...
		fprintf(fd, "%u\t%#.18g\t%#.18g\t%#.18g\n", timestep,
			PhysicalTime, OmegaFrame, FrameAngle);
    ...
    const std::map<const std::string, const std::string> &units)
{
	...
	std::map<std::string, std::string> unit_descriptors = {
	    ...
	    {"angular_momentum",
	     units::angular_momentum.get_cgs_factor_symbol()},
	    ...
	    {"specific power",
	     unit_descriptor(units::power.get_cgs_factor() /
				 units::length.get_cgs_factor() *
				 units::length.get_cgs_factor(),
			     "erg cm2/s/g")},
	    {"pressure per time",
	     unit_descriptor(units::pressure.get_cgs_factor() /
...
void write_torques(t_data &data, unsigned int timestep, bool force_update)
{
	...
	for (unsigned int n_planet = 0;
		...
		t_planet &planet =
		    data.get_planetary_system().get_planet(n_planet);
		double smooth = parameters::thickness_smoothing *
				ASPECTRATIO_REF *
				pow(planet.get_distance(), 1.0 + FLARINGINDEX);
		...
		for (unsigned int n_radial = 0;
			...
			for (unsigned int n_azimuthal = 0;
			     n_azimuthal <=
			     data[t_data::TORQUE].get_max_azimuthal();
			     ++n_azimuthal) {
				double phi = (double)n_azimuthal /
					     (double)data[t_data::TORQUE]
						 .get_size_azimuthal() *
					     2.0 * PI;
				double dx =
				    planet.get_x() - Rmed[n_radial] * cos(phi);
				double dy =
				    planet.get_y() - Rmed[n_radial] * sin(phi);
				double m = data[t_data::DENSITY](n_radial,
								 n_azimuthal) *
					   Surf[n_radial];

				double distance2 =
				    pow2(dx) + pow2(dy) + pow2(smooth);
				...
				data[t_data::TORQUE_1D](n_radial) +=
				    data[t_data::TORQUE](n_radial, n_azimuthal);
				if ((n_radial >= ((CPU_Rank == 0)
						      ? GHOSTCELLS_B
						      : CPUOVERLAP)) &&
				    (n_radial <=
				     data[t_data::TORQUE].get_max_radial() -
					 (CPU_Rank == CPU_Highest
					      ? GHOSTCELLS_B
					      : CPUOVERLAP))) {
					local_torques[n_planet + 1] +=
					    data[t_data::TORQUE](n_radial,
								 n_azimuthal);
				}
			}
		}
	...
	if (CPU_Master) {
		...
		if (asprintf(&fd_filename, "%s%s", OUTPUTDIR, "torques.dat") ==
		    -1) {
			logging::print_master(
			    LOG_ERROR "Not enough memory for string buffer.\n");
			PersonalExit(1);
		...
		if (fd == NULL) {
			logging::print_master(
			    LOG_ERROR
			    "Can't write 'torques.dat' file. Aborting.\n");
			PersonalExit(1);
		}
		...
		for (unsigned int i = 0;
		     i <= data.get_planetary_system().get_number_of_planets();
		     ++i) {
			fprintf(fd, "\t%.20e",
				global_torques[i] * units::torque);
		}
		fprintf(fd, "\n");
	...
	for (int i = 0; i < t_data::N_POLARGRID_TYPES; ++i) {
		if (data[t_data::t_polargrid_type(i)].get_write_1D()) {
			...
			info_ofs
			    << "# "
			    << data[t_data::t_polargrid_type(i)].get_name()
			    << " 1d radial, in first line alternating: radii | "
			...
			std::string unit;
			if (data[t_data::t_polargrid_type(i)].get_unit() !=
			    NULL) {
				unit = std::string(
			...
			info_ofs << "bigendian = " << is_big_endian()
				 << std::endl;
			info_ofs.close();
		}
	}
...
void write_massflow_info(t_data &data)
{
	...
	std::ofstream info_ofs(filename_info);
	info_ofs << "# Mass flow 1d radial, first line radii, from second line "
		    "on, values at time in Quantities.dat"
		 << std::endl;
	...
	const std::string filename =
	    std::string(OUTPUTDIR) + "/gasMassFlow1D.dat";
	std::ofstream ofs(filename, std::ios::binary);
	ofs.write((char *)Radii.array,
		  sizeof(*Radii.array) * (GlobalNRadial + 1));
}
					   ...
					   bool force_update)
{
	...
	// Loop thru all cells excluding GHOSTCELLS & CPUOVERLAP cells
	// (otherwise they would be included twice!)
	for (unsigned int n_radial = (CPU_Rank == 0) ? GHOSTCELLS_B
						     : CPUOVERLAP;
	     n_radial <=
		...
		for (unsigned int n_azimuthal = 0;
		     ...
		     ++n_azimuthal) {
			// eccentricity and semi major axis weighted with
			// cellmass
			local_mass =
	...
	// synchronize threads
	MPI_Reduce(&local_eccentricity, &disk_eccentricity, 1, MPI_DOUBLE,
		   MPI_SUM, 0, MPI_COMM_WORLD);
	// MPI_Allreduce(&local_semi_major_axis, &semi_major_axis, 1,
	// MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
...
void write_lightcurves(t_data &data, unsigned int timestep, bool force_update)
{
	...
	for (unsigned int i = 0; i < parameters::lightcurves_radii.size();
	     ++i) {
	...
	if (CPU_Rank > 0) {
		MPI_Recv(luminosity_values,
			 parameters::lightcurves_radii.size(), MPI_DOUBLE,
			 CPU_Prev, 0, MPI_COMM_WORLD, NULL);
		MPI_Recv(dissipation_values,
			 parameters::lightcurves_radii.size(), MPI_DOUBLE,
			 CPU_Prev, 0, MPI_COMM_WORLD, NULL);
	}
	...
	for (unsigned int n_radial = (CPU_Rank == 0) ? GHOSTCELLS_B
						     : CPUOVERLAP;
	     n_radial <=
	     data[t_data::LUMINOSITY_1D].get_max_radial() -
		 (CPU_Rank == CPU_Highest ? GHOSTCELLS_B : CPUOVERLAP);
	     ++n_radial) {
	...
	if (CPU_Rank < CPU_Highest) {
		MPI_Send(luminosity_values,
			 parameters::lightcurves_radii.size(), MPI_DOUBLE,
			 CPU_Next, 0, MPI_COMM_WORLD);
		MPI_Send(dissipation_values,
			 parameters::lightcurves_radii.size(), MPI_DOUBLE,
			 CPU_Next, 0, MPI_COMM_WORLD);
	}
	...
	if (CPU_Rank == CPU_Highest) {
		...
		if (fd == NULL) {
			logging::print_master(
			    LOG_ERROR
			    "Can't write 'luminosity.dat' file. Aborting.\n");
			PersonalExit(1);
		}
		...
		for (unsigned int i = 1;
		     i < parameters::lightcurves_radii.size(); ++i) {
			fprintf(fd, "%.20e\t",
				luminosity_values[i] *
				    units::power.get_cgs_factor());
		}
		fprintf(fd, "\n");
		...
		if (fd == NULL) {
			logging::print_master(
			    LOG_ERROR
			    "Can't write 'dissipation.dat' file. Aborting.\n");
			PersonalExit(1);
		}
		       ...
		       unsigned int fineOutputNumber)
{
	...
	if (CPU_Master) {
		...
		if (fd == NULL) {
			logging::print_master(
			    LOG_ERROR
			    "Can't write 'timeCoarse.dat' file. Aborting.\n");
			PersonalExit(1);
		}
		...
		if (!fd_created) {
			...
			fprintf(fd, "# Time log for course output.\n# Syntax: "
				    "coarse output step <tab> fine output step "
				    "<tab> physical time (cgs)\n");
			fd_created = true;
		}
	}
	...
	if (CPU_Master) {
		fprintf(fd, "%u\t%u\t%#.16e\n", coarseOutputNumber,
			fineOutputNumber, PhysicalTime * units::time);
		fclose(fd);
	}
}
