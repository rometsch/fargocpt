...
void check_free_space(t_data &data)
{
	...
	logging::print_master(LOG_WARNING
			      "Couldn't stat filesystem. You have to check "
	...
	if (space_needed > free_space) {
	    logging::print_master(
		LOG_WARNING "There is not enough space for all outputs! The "
		      ...
		      unsigned int nTimeStep, bool force_update)
{
	...
	fprintf(fd,
		...
		totalAngularMomentum, totalEnergy, internalEnergy,
		kinematicEnergy, gravitationalEnergy, radialKinematicEnergy,
		azimuthalKinematicEnergy, disk_eccentricity, disk_periastron,
		qplus, qminus, pdivv_total, InnerPositive, InnerNegative,
...
void write_torques(t_data &data, unsigned int timestep, bool force_update)
{
	...
	for (unsigned int n_radial = 0;
	     n_radial <= data[t_data::TORQUE].get_max_radial(); ++n_radial) {
		...
		if (sqrt(pow2(dx) + pow2(dy)) < 4.0 * r_taper) {
		    taper = 1.0 / (exp(-(sqrt(pow2(dx) + pow2(dy)) - r_taper) /
				       (0.1 * r_taper)) +
				   1.0);
		}

		double F_x =
		    -dx * m * planet.get_mass() * pow(distance2, -1.5) * taper;
		double F_y =
		    -dy * m * planet.get_mass() * pow(distance2, -1.5) * taper;
...
void write_1D_info(t_data &data)
{
	...
	if (data[t_data::t_polargrid_type(i)].get_write_1D()) {
	    ...
	    info_ofs << "# " << data[t_data::t_polargrid_type(i)].get_name()
