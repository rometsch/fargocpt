...
void update_kernel(t_data &data)
{
	...
	if (since_last_calculated >= (update_every_nth_step - 1)) {
		...
		lambda_sq =
		    pow2(0.4571 * aspect_ratio + 0.6737 * sqrt(aspect_ratio));
...
void init(t_data &data)
{
	...
	if (parameters::radial_grid_type != parameters::logarithmic_spacing) {
		logging::print_master(
		    LOG_ERROR "A logarithmic grid is needed to compute "
			      "self-gravity with polar method. Try again!\n");
		PersonalExit(1);
	}
	...
	fftplan_forward_K_radial = fftw_mpi_plan_dft_r2c_2d(
	    2 * GlobalNRadial, NAzimuthal, K_radial, FFT_K_radial,
	...
	fftplan_forward_S_radial = fftw_mpi_plan_dft_r2c_2d(
	    2 * GlobalNRadial, NAzimuthal, S_radial, FFT_S_radial,
	    MPI_COMM_WORLD, FFTW_MEASURE | FFTW_MPI_TRANSPOSED_OUT);
...
void compute(t_data &data, double dt, bool update)
{
	...
	if (update) {
		// Computes polar components of acceleration and updates values
		// of vrad, vtheta at each step
	...
	if (CPU_Rank != CPU_NoFriend) {
		if (CPU_Rank >= (CPU_Number + one_if_odd) / 2) {
			...
			MPI_Isend(&dens[Zero_or_active * NAzimuthal],
				  active_hydro_totalsize, MPI_DOUBLE,
				  CPU_Friend, 30, MPI_COMM_WORLD, &req);
		} else {
			/* all lower cpus recieve data */
			MPI_Irecv(&dens_friend[0],
				  active_hydro_totalsize_friend, MPI_DOUBLE,
				  CPU_Friend, 30, MPI_COMM_WORLD, &req);
		}
	...
	if ((CPU_Rank < CPU_Number / 2) && (CPU_Rank != CPU_NoFriend)) {
		for (i = 0; i < ifront + 1; i++) {
			for (j = 0; j < NAzimuthal; j++) {
				...
				S_radial[l] =
				    dens[lh] * sqrt(Rmed[ih] / GlobalRmed[0]);
				/* S_t = sigma(u,phi) exp(3*u/2) */
				S_azimuthal[l] =
				    S_radial[l] * Rmed[ih] / GlobalRmed[0];
			}
		}
...
void compute_FFT_kernel()
{
	...
	for (unsigned int i = 0; i < (unsigned int)local_Nx; i++) {
		if (i + local_i_start < GlobalNRadial) {
			...
			u = -log(
			    Radii[2 * GlobalNRadial - (i + local_i_start)] /
			    Radii[0]);
		}
...
void compute_acceleration(t_polargrid &density)
{
	...
	// First we compute sg_acc as a convolution product of reduced density
	// and kernel arrays. In fact, all bufffttabs are transposed arrays,
	// since we used the flag FFTW_TRANSPOSED_ORDER before. However, this is
	// not a problem: we compute double and imaginary parts of 1-dimension
	// convolution arrays buffer_FFT_acc_radial(t)
	for (unsigned int i = 0; i < total_local_size; i++) {
		...
		FFT_acc_azimuthal[i][0] =
		    -constants::G *
		    (FFT_K_azimuthal[i][0] * FFT_S_azimuthal[i][0] -
		     FFT_K_azimuthal[i][1] * FFT_S_azimuthal[i][1]);
		FFT_acc_azimuthal[i][1] =
		    -constants::G *
		    (FFT_K_azimuthal[i][0] * FFT_S_azimuthal[i][1] +
	...
	// fourier transforms ensures that arrays are not transposed anymore.
	// Then we transfer the exact necessary quantity of sg_acceleration
	// arrays from the fftw d.d. to the hydro mesh
	if (CPU_Rank != CPU_NoFriend) {
		if (CPU_Rank < CPU_Number / 2) {
			for (i = 0; i < transfer_size; i++) {
				if (i < transfer_size / 2)
					...
					ffttohydro_transfer[i] = acc_azimuthal
					    [(ifront + 1 - CPUOVERLAP) *
						 stride +
					     i - transfer_size / 2];
			}
			MPI_Isend(ffttohydro_transfer, transfer_size,
				  MPI_DOUBLE, CPU_Friend, 40, MPI_COMM_WORLD,
				  &req1);
			MPI_Wait(&req1, &global_MPI_Status);
		} else {
			MPI_Irecv(ffttohydro_transfer_friend,
				  transfer_size_friend, MPI_DOUBLE, CPU_Friend,
				  40, MPI_COMM_WORLD, &req1);
			MPI_Wait(&req1, &global_MPI_Status);
		}
	}
		...
		if (CPU_Rank == 0) {
			for (i = 0; i < nr; i++) {
				for (j = 0; j < NAzimuthal; j++) {
					l = i * NAzimuthal + j;
					g_radial[l] =
					    acc_radial[i * stride + j];
					g_azimuthal[l] =
					    acc_azimuthal[i * stride + j];
				}
			}
		} else {
			for (i = Zero_or_active; i < nr; i++) {
				for (j = 0; j < NAzimuthal; j++) {
					l = i * NAzimuthal + j;
					g_radial[l] =
					    acc_radial[(i - Zero_or_active) *
							   stride +
						       j];
					g_azimuthal[l] =
					    acc_azimuthal[(i - Zero_or_active) *
							      stride +
							  j];
				}
			}
	...
	if (CPU_Rank >= (CPU_Number + one_if_odd) / 2) {
		if (CPU_Rank == CPU_Highest) {
			for (i = 0; i < nr; i++) {
				for (j = 0; j < NAzimuthal; j++) {
					l = i * NAzimuthal + j;
					g_radial[l] = ffttohydro_transfer_friend
					    [i * stride + j];
					g_azimuthal[l] =
					    ffttohydro_transfer_friend
						[transfer_size_friend / 2 +
						 i * stride + j];
				}
			}
		} else {
	...
	if (CPU_Number > 1) {
		if ((CPU_Rank > 0) &&
		    (CPU_Rank < (CPU_Number + one_if_odd) / 2)) {
			MPI_Isend(&g_azimuthal[Zero_or_active * NAzimuthal],
				  ghost_size, MPI_DOUBLE, CPU_Prev, 60,
				  MPI_COMM_WORLD, &req3);
			MPI_Wait(&req3, &global_MPI_Status);
			MPI_Irecv(&g_azimuthal[0], ghost_size, MPI_DOUBLE,
				  CPU_Prev, 61, MPI_COMM_WORLD, &req4);
			MPI_Wait(&req4, &global_MPI_Status);
		}
		if ((CPU_Rank >= (CPU_Number + one_if_odd) / 2) &&
		    (CPU_Rank != CPU_Highest)) {
			MPI_Irecv(&g_azimuthal[Max_or_active * NAzimuthal],
				  ghost_size, MPI_DOUBLE, CPU_Next, 60,
				  MPI_COMM_WORLD, &req3);
			MPI_Wait(&req3, &global_MPI_Status);
			MPI_Isend(&g_azimuthal[(Max_or_active - CPUOVERLAP) *
					       NAzimuthal],
				  ghost_size, MPI_DOUBLE, CPU_Next, 61,
				  MPI_COMM_WORLD, &req4);
			MPI_Wait(&req4, &global_MPI_Status);
		}
		if ((CPU_Rank > 0) &&
		    (CPU_Rank < (CPU_Number + one_if_odd) / 2)) {
			MPI_Isend(&g_radial[Zero_or_active * NAzimuthal],
				  ghost_size, MPI_DOUBLE, CPU_Prev, 50,
				  MPI_COMM_WORLD, &req5);
			MPI_Wait(&req5, &global_MPI_Status);
			MPI_Irecv(&g_radial[0], ghost_size, MPI_DOUBLE,
				  CPU_Prev, 51, MPI_COMM_WORLD, &req6);
			MPI_Wait(&req6, &global_MPI_Status);
		}
		if ((CPU_Rank >= (CPU_Number + one_if_odd) / 2) &&
		    (CPU_Rank != CPU_Highest)) {
			MPI_Irecv(&g_radial[Max_or_active * NAzimuthal],
				  ghost_size, MPI_DOUBLE, CPU_Next, 50,
				  MPI_COMM_WORLD, &req5);
			MPI_Wait(&req5, &global_MPI_Status);
			MPI_Isend(&g_radial[(Max_or_active - CPUOVERLAP) *
					    NAzimuthal],
				  ghost_size, MPI_DOUBLE, CPU_Next, 51,
				  MPI_COMM_WORLD, &req6);
			MPI_Wait(&req6, &global_MPI_Status);
		}
	}
	...
	for (i = 0; i < nr; i++) {
		// g_r(u,phi) normalized with exp(-u/2)*Δu*Δphi/(2*N_r*N_phi)
		// (3.43 page 57) g_phi(u,phi) normalized with exp(-3*u/2)*Δu
		// Δphi/(2*N_r*N_phi) (3.44 page 57)
		normaccr = r_step * t_step /
			   ((double)(2 * GlobalNRadial) * (double)NAzimuthal);
		normacct = normaccr;
		normaccr /= sqrt(Rmed[i] / GlobalRmed[0]);
		normacct /=
		    (Rmed[i] / GlobalRmed[0] * sqrt(Rmed[i] / GlobalRmed[0]));
		for (j = 0; j < NAzimuthal; j++) {
			l = i * NAzimuthal + j;
			g_radial[l] *= normaccr;
		       ...
		       double dt)
{
	...
	for (unsigned int n_radial = 0; n_radial < v_radial.get_max_radial();
		...
		for (unsigned int n_azimuthal = 0; n_azimuthal < NAzimuthal;
			...
			// We compute VAzimuthal - half-centered in radius -
			// from centered-in-cell azimutal sg acceleration
			if (n_azimuthal == 0)
				jm1 = NAzimuthal - 1;
			else
				jm1 = n_azimuthal - 1;
...
void init_azimuthal_velocity(t_polargrid &v_azimuthal)
{
	...
	for (unsigned int n_radial = 0;
	     n_radial <= v_azimuthal.get_max_radial() - GHOSTCELLS_B;
	     ++n_radial) {
		...
		if (temp < 0) {
			logging::print(
			    "Radicand %lg < 0 in init_azimuthal_velocity! "
			    "Maybe ThicknessSmoothingSG (%lg) is too small!\n",
...
void init_planetary_system(t_data &data)
{
	...
	// Planetary system initialization in self-gravity cases: planets are
	// put in a fixed circular orbit, we need to know radial sg acceleration
	// felt by planets
	...
	for (unsigned int k = 0;
		...
		if (data.get_planetary_system().get_planet(k).get_feeldisk()) {
			dist = data.get_planetary_system()
				   .get_planet(k)
				   .get_semi_major_axis();
			...
			if (dist < RMAX) {
				...
				dr = rip1 - ri;
				sgacc =
				    (dist - ri) * GLOBAL_AxiSGAccr[ipl + 1] +
				    (rip1 - dist) * GLOBAL_AxiSGAccr[ipl];
				sgacc /= dr;
				/* sgacc is the radial sg acc. at the planet's
				 * semi-major axis */
				double new_vy =
