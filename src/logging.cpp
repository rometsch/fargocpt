	...
	if (current_level <= print_level) {
		...
		if (!time_format) {
			fprintf(current_level <= error_level ? stderr : stdout,
				"[%0*i] %s",
				(int)(log(CPU_Number) / log(10) + 1), CPU_Rank,
				buf);
		} else {
			fprintf(current_level <= error_level ? stderr : stdout,
				"[%0*i %s] %s",
...
void print_runtime_final()
{
	...
	logging::print_master(
	    LOG_INFO
	    "-- Final: Total Hyrdosteps %d, Physical Time %.2f, realtime %.2f "
	    "seconds, time per step: %.2f milliseconds\n",
			...
			unsigned int time_step_coarse, double dt)
{
	...
	// average runtime, ... depending on whether enough real time or number
	// of hydro steps have passed since the last log
	...
	bool log_bc_time = parameters::log_after_real_seconds > 0 &&
			   realtime_since_last / 1000000.0 >
			       parameters::log_after_real_seconds;
	if (log_bc_steps || log_bc_time) {
		if (log_bc_steps) {
			...
			realtime_since_last =
				...
				std::chrono::microseconds>(realtime_now -
							   realtime_last_log)
				.count();
		}
		realtime =
		...
		if ((N_iter - n_last_log) != 0) {
			time_per_step_ms = realtime_since_last /
					   (1000.0 * (N_iter - n_last_log));
		}
		logging::print_master(
		    LOG_INFO
		    "output %d, timestep %d, hyrdostep %d, physicaltime %f, dt "
		    "%.3e, realtime %.2f s, timeperstep %.2f ms\n",
