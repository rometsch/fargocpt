...
double gas_aspect_ratio(t_data &data)
{
	...
	// synchronize threads
	MPI_Allreduce(&local_aspect_ratio, &aspect_ratio, 1, MPI_DOUBLE,
...
double gas_disk_radius(t_data &data, const double total_mass)
{
	...
	for (unsigned int n_radial = local_array_start;
	     n_radial < local_array_end; ++n_radial) {
	...
	MPI_Gatherv(local_mass, send_size, MPI_DOUBLE, GLOBAL_bufarray,
		    RootNradialLocalSizes, RootNradialDisplacements, MPI_DOUBLE,
		    0, MPI_COMM_WORLD);
		...
		for (int rank = 0; rank < CPU_Number; ++rank) {
			...
			for (int i = RootIMIN[id]; i <= RootIMAX[id]; ++i) {
				...
				if (current_mass > 0.99 * total_mass) {
					radius = GlobalRmed[j];
					goto found_radius; // break out of
							   // nested loop
				}
			}
		}
...
double gas_internal_energy(t_data &data)
{
	...
	MPI_Reduce(&local_internal_energy, &global_internal_energy, 1,
...
double gas_kinematic_energy(t_data &data)
{
	...
	for (unsigned int n_radial = (CPU_Rank == 0) ? GHOSTCELLS_B
		...
		for (unsigned int n_azimuthal = 0;
			...
			v_azimuthal_center =
			    0.5 * (data[t_data::V_AZIMUTHAL](n_radial,
							     n_azimuthal) +
	...
	MPI_Reduce(&local_kinematic_energy, &global_kinematic_energy, 1,
		   MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
			       ...
			       bool force_update)
{
	...
	for (unsigned int n_radial = 0;
		...
		for (unsigned int n_azimuthal = 0;
		     ...
		     ++n_azimuthal) {
			total_mass =
			    M + data[t_data::DENSITY](n_radial, n_azimuthal) *
				    Surf[n_radial];
			...
			e_x = j * v_ymed / (constants::G * total_mass) -
			      r_x / Rmed[n_radial];
			...
			if (FrameAngle != 0.0) {
				// periastron grid is rotated to non-rotating
				// coordinate system to prevent phase jumps of
				// atan2 in later transformations like you would
				// have had if you back-transform the output
				// periastron values
				data[t_data::PERIASTRON](n_radial,
							 n_azimuthal) =
				    atan2(e_y * cosFrameAngle +
					      e_x * sinFrameAngle,
					  e_x * cosFrameAngle -
					      e_y * sinFrameAngle);
			} else {
				data[t_data::PERIASTRON](
				    n_radial, n_azimuthal) = atan2(e_y, e_x);
			}
		}
	}
}
			  ...
			  bool force_update)
{
	...
	for (unsigned int n_radial = 0;
	     n_radial <= data[t_data::ALPHA_GRAV].get_max_radial();
	     ++n_radial) {
		for (unsigned int n_azimuthal = 0;
		     n_azimuthal <=
		     data[t_data::ALPHA_GRAV].get_max_azimuthal();
		     ++n_azimuthal) {
			...
			data[t_data::ALPHA_GRAV](n_radial, n_azimuthal) =
			    2.0 / 3.0 *
			    data[t_data::T_GRAVITATIONAL](n_radial,
							  n_azimuthal) /
			    (data[t_data::DENSITY](n_radial, n_azimuthal) *
				 ...
				 bool force_update)
{
	...
	for (unsigned int n_radial = 0;
	     n_radial <= data[t_data::ALPHA_GRAV_1D].get_max_radial();
	     ++n_radial) {
				     ...
				     double dt)
{
	...
	for (unsigned int n_radial = 0;
		...
		for (unsigned int n_azimuthal = 0;
		     n_azimuthal <=
		     data[t_data::ALPHA_GRAV_MEAN].get_max_azimuthal();
		     ++n_azimuthal) {
				     ...
				     bool force_update)
{
	...
	for (unsigned int n_radial = 0;
		...
		for (unsigned int n_azimuthal = 0;
		     n_azimuthal <=
		     data[t_data::ALPHA_REYNOLDS].get_max_azimuthal();
		     ++n_azimuthal) {
					 ...
					 double dt)
{
	...
	for (unsigned int n_radial = 0;
		...
		for (unsigned int n_azimuthal = 0;
		     ...
		     ++n_azimuthal) {
			data[t_data::ALPHA_REYNOLDS_MEAN](n_radial,
							  n_azimuthal) +=
			    data[t_data::ALPHA_REYNOLDS](n_radial,
							 n_azimuthal) *
			    dt;
		}
	}
		      ...
		      bool /* force_update */)
{
	...
	for (unsigned int n_radial = 1;
		...
		for (unsigned int n_azimuthal = 0;
		     ...
		     ++n_azimuthal) {
			// kappa^2 = 1/r^3 d((r^2 Omega)^2)/dr = 1/r^3
			// d((r*v_phi)^2)/dr
			kappa = sqrt(fabs(pow3(InvRmed[n_radial]) *
					  (pow2(data[t_data::V_AZIMUTHAL](
			     ...
			     bool /* force_update */)
{
	...
	for (unsigned int n_radial = 1;
		...
		kappa =
		    sqrt(fabs(pow3(InvRmed[n_radial]) *
			      (pow2(data[t_data::V_AZIMUTHAL_1D](n_radial) *
				    Rmed[n_radial]) -
				 ...
				 bool force_update)
{
	...
	for (unsigned int n_radial = 0;
		...
		for (unsigned int n_azimuthal = 0;
			...
			double dphi =
			    2.0 * PI /
			    (double)data[t_data::QMINUS].get_size_azimuthal();
			data[t_data::LUMINOSITY_1D](n_radial) +=
			    data[t_data::QMINUS](n_radial, n_azimuthal) *
			    Rmed[n_radial] * dr * dphi;
		}
	}
}
				  ...
				  bool force_update)
{
	...
	for (unsigned int n_radial = 0;
		...
		for (unsigned int n_azimuthal = 0;
			...
			double dphi =
			    2.0 * PI /
			    (double)data[t_data::QPLUS].get_size_azimuthal();
