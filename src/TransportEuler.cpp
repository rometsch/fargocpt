...
void ComputeConstantResidual(PolarGrid *VAzimuthal, double dt)
{
	...
	for (i = 0; i < nr; i++) {
		Ntilde = v_azimuthal_mean(i) * InvRmed[i] * dt * (double)ns /
			 2.0 / PI;
		  ...
		  PolarGrid *Energy, double dt)
{
	...
	ComputeConstantResidual(
	    VAzimuthal,
	    dt); /* Constant residual is in VAzimuthal from now on */
	UniformTransport = NO;
			 ...
			 t_polargrid *QStar, double dt)
{
	/*	// calculate all dq
	   ...
	   base.get_max_radial(); ++n_radial) { for (unsigned int n_azimuthal =
	   0; n_azimuthal <= base.get_max_azimuthal(); ++n_azimuthal) {
				// inner and outer most cells need dq = 0
				if ((n_radial == 0) || (n_radial ==
	   base.get_max_radial())) { dq[n_radial] = 0.0; } else { double
	   dq_minus = base(n_radial,
	   n_azimuthal)-base(n_azimuthal-1,n_azimuthal)*InvDiffRmed[nRadial];
					double dq_plus = base(n_radial+1,
	   n_azimuthal)-base(n_radial,n_azimuthal)*InvDiffRmed[nRadial+1];

					if (dq_minus * dq_plus > 0.0) {
						dq[n_radial]
	   = 2.0*dq_minus*dq_plus/(dq_minus+dq_plus); } else { dq[n_radial] =
	   0.0;
					}
				}
			}
		...
		for (unsigned int n_radial = 0; n_radial <=
	   base.get_max_radial(); ++n_radial) { for (unsigned int n_azimuthal =
	   0; n_azimuthal <= base.get_max_azimuthal(); ++n_azimuthal) { if
	   (v_radial(n_radial, n_azimuthal) > 0.0) { star(n_radial, n_azimuthal)
	   = base(n_radial-1,
	   n_azimuthal)+(Rmed[nRadial]-Rmed[nRadial-1]-v_radial(n_radial,n_azimuthal)*dt)*0.5*dq[nRadial-1];
				} else {
					star(n_radial, n_azimuthal) =
	   base(n_radial,
	...
	for (nAzimuthal = 0; nAzimuthal < Qbase->Nsec; ++nAzimuthal) {
		...
		for (nRadial = 0; nRadial < Qbase->Nrad; ++nRadial) {
			unsigned int cell =
			    CELL(nRadial, nAzimuthal, Qbase->Nsec);
			...
			if ((nRadial == 0) || (nRadial == Qbase->Nrad - 1)) {
				...
				dqm = (Qbase->Field[cell] -
				       Qbase->Field[cellPrevRadial]) *
				      InvDiffRmed[nRadial];
				dqp = (Qbase->Field[cellNextRadial] -
				       Qbase->Field[cell]) *
		...
		// TODO: potential problem: Using Rmed[nRadial] -
		// Rmed[nRadial-1] for a-mesh Qties (v_rad,..) as well as for
		// b-mesh Qties (Density,...)
		      ...
		      double dt)
{
	...
	for (nRadial = 0; nRadial < Qbase->Nrad; ++nRadial) {
		...
		for (nAzimuthal = 0; nAzimuthal < Qbase->Nsec; ++nAzimuthal) {
			...
			if (ksi > 0.0)
				QStar->Field[cell] = Qbase->Field[ljm] +
						     (dxtheta - ksi) * dq[ljm];
			else
				QStar->Field[cell] = Qbase->Field[cell] -
						     (dxtheta + ksi) * dq[cell];
		}
	}
}
				     ...
				     t_polargrid &v_azimuthal)
{
	for (unsigned int n_radial = 0; n_radial <= density.get_max_radial();
	     ++n_radial) {
		for (unsigned int n_azimuthal = 0;
		     n_azimuthal <= density.get_max_azimuthal();
		     ++n_azimuthal) {
	...
	for (unsigned int n_radial = 0; n_radial <= density.get_max_radial();
		...
		for (unsigned int n_azimuthal = 0;
			...
			if (n_radial == 0) {
				...
				v_radial(n_radial, n_azimuthal) =
				    (radial_momentum_plus(n_radial - 1,
							  n_azimuthal) +
				     radial_momentum_minus(n_radial,
							   n_azimuthal)) /
			...
			v_azimuthal(n_radial, n_azimuthal) =
			    (angular_momentum_plus(n_radial,
						   n_azimuthal_minus) +
			     angular_momentum_minus(n_radial, n_azimuthal)) /
		   ...
		   double dt)
{
	...
	if (var_name != NULL) {
		is_density =
		    strcmp(var_name, data[t_data::DENSITY_INT].get_name());
	}
	...
	for (nRadial = 0; nRadial <= Qbase->get_max_radial(); ++nRadial) {
		for (nAzimuthal = 0; nAzimuthal <= Qbase->get_max_azimuthal();
			...
			// update density
			Qbase->Field[cell] +=
			    (varq_inf - varq_sup) * InvSurf[nRadial];

			if (is_density) {
				// TODO: boundary
				// if ((nRadial == 0) &&
				// (parameters::boundary_inner ==
				// parameters::boundary_condition_open)) if
				// ((nRadial == 0) && (OpenInner))
				...
				if (parameters::write_massflow) {
					...
					if (CPU_Rank == CPU_Highest &&
					    nRadial ==
						Qbase->get_max_radial()) {
						data[t_data::MASSFLOW_1D](
						    nRadial) +=
						    varq_sup; // TODO this part
							      // is always zero
							      // (tested with
							      // open
							      // boundaries)
		  ...
		  double dt)
{
	...
	for (nRadial = 0; nRadial < Qbase->Nrad; ++nRadial) {
		...
		if ((UniformTransport == NO) ||
		    (NoSplitAdvection[nRadial] == NO)) {
			for (nAzimuthal = 0; nAzimuthal < Qbase->Nsec;
			     ++nAzimuthal) {
				double varq;
				...
				varq = dxrad * QRStar->Field[cell] *
				       DensityStar->Field[cell] *
				       VAzimuthal->Field[cell];
				varq -= dxrad * QRStar->Field[ljp] *
					DensityStar->Field[ljp] *
...
void boundary_layer_mass_influx(PolarGrid *QStar, PolarGrid *VRadial)
{
	...
	// printf("QStar->get_max_radial() = %d, VRadial->get_max_radial() =
