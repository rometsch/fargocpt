...
void update_viscosity(t_data &data)
{
	...
	if (ViscosityAlpha) {
		...
		if (!calculated) {
			for (unsigned int n_radial = 0;
			     n_radial <=
			     data[t_data::VISCOSITY].get_max_radial();
			     ++n_radial) {
				for (unsigned int n_azimuthal = 0;
				     n_azimuthal <= data[t_data::VISCOSITY]
							.get_max_azimuthal();
				     ++n_azimuthal) {
					data[t_data::VISCOSITY](
					    n_radial, n_azimuthal) = VISCOSITY;
				}
			}
		}
...
void compute_viscous_terms(t_data &data, bool include_artifical_viscosity)
{
		...
		for (unsigned int n_azimuthal = 0;
			...
			data[t_data::DIV_V](n_radial, n_azimuthal) =
			    ...
			    (data[t_data::V_AZIMUTHAL](
				 n_radial,
				 n_azimuthal ==
					 data[t_data::DIV_V].get_max_azimuthal()
				     ? 0
				     : n_azimuthal + 1) -
	...
	for (unsigned int n_radial = 0;
		...
		for (unsigned int n_azimuthal = 0;
			...
			// tau_r_r = 2*nu*Sigma*( d(v_r)/dr - 1/3 div(v) + eta2
			// div(v))
	...
	for (unsigned int n_radial = 0;
	     n_radial <= data[t_data::TAU_PHI_PHI].get_max_radial();
	     ++n_radial) {
		for (unsigned int n_azimuthal = 0;
		     n_azimuthal <=
			...
			dpp =
			    (data[t_data::V_AZIMUTHAL](
				 n_radial,
				 n_azimuthal == data[t_data::TAU_PHI_PHI]
						    .get_max_azimuthal()
				     ? 0
				     : n_azimuthal + 1) -
			     data[t_data::V_AZIMUTHAL](n_radial, n_azimuthal)) *
			...
			// tau_phi_phi = 2*nu*Sigma*( 1/r d(v_phi)/dphi + v_r/r
			// - 1/3 div(v) )
	...
	for (unsigned int n_radial = 1;
	     n_radial <= data[t_data::TAU_R_PHI].get_max_radial() - 1;
	     ++n_radial) {
		for (unsigned int n_azimuthal = 0;
			...
			drp =
			    Ra[n_radial] * dvphirdr + dvrdphi * InvRa[n_radial];
			...
			double nu =
			    0.25 *
			    (data[t_data::VISCOSITY](n_radial, n_azimuthal) +
			     data[t_data::VISCOSITY](n_radial - 1,
						     n_azimuthal) +
			     data[t_data::VISCOSITY](n_radial,
						     n_azimuthal_minus) +
			     data[t_data::VISCOSITY](n_radial - 1,
						     n_azimuthal_minus));
			...
			// tau_r_phi = nu*Sigma*( r*d(v_phi/r)/dr + 1/r
			// d(v_r)/dphi )
	...
	if (include_artifical_viscosity) {
		for (unsigned int n_radial = 0;
		     n_radial <= data[t_data::DIV_V].get_max_radial();
		     ++n_radial) {
			for (unsigned int n_azimuthal = 0;
			     n_azimuthal <=
			     data[t_data::DIV_V].get_max_azimuthal();
				...
				if (data[t_data::DIV_V](n_radial, n_azimuthal) <
				    0) {
					nu_art =
					    parameters::
				      ...
				      t_polargrid &v_azimuthal, double dt)
{
	...
	for (unsigned int n_radial = 1;
	     n_radial <= v_radial.get_max_radial() - 1; ++n_radial) {
		for (unsigned int n_azimuthal = 0;
			...
			// a_phi = 1/(r*Sigma) ( d(r*tau_r_phi)/dr +
			// d(tau_phi_phi)/dphi + tau_r_phi )
			...
			*/
			...
			// a_r = 1/(r*Sigma) ( d(r*tau_r_r)/dr +
			// d(tau_r_phi)/dphi - tau_phi_phi )
			sigma_avg =
			    0.5 *
			    (data[t_data::DENSITY](n_radial, n_azimuthal) +
			     data[t_data::DENSITY](n_radial - 1, n_azimuthal));
			v_radial(n_radial, n_azimuthal) +=
			    ...
			    ((Ra[n_radial] *
				  data[t_data::TAU_R_R](n_radial, n_azimuthal) -
			      Ra[n_radial - 1] *
				  data[t_data::TAU_R_R](n_radial - 1,
							n_azimuthal)) *
				 InvDiffRmed[n_radial] +
			     (data[t_data::TAU_R_PHI](n_radial,
						      n_azimuthal_plus) -
			      data[t_data::TAU_R_PHI](n_radial, n_azimuthal)) *
				 invdphi -
			     0.5 * (data[t_data::TAU_PHI_PHI](n_radial,
							      n_azimuthal) +
				    data[t_data::TAU_PHI_PHI](n_radial - 1,
							      n_azimuthal)));
		}
	}
}
