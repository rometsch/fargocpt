...
void compute_viscous_terms(t_data &data, bool include_artifical_viscosity)
{
	...
	for (unsigned int n_azimuthal = 0;
	    ...
	    data[t_data::VISCOSITY_SIGMA](n_radial, n_azimuthal) =
	...
	for (unsigned int n_azimuthal = 0;
	    ...
	    const double correction_helper_value = nu * sigma;
	    data[t_data::VISCOSITY_SIGMA_RP](n_radial, n_azimuthal) =
		correction_helper_value;
	}
    }
	...
	for (unsigned int n_radial = 1;
		...
		const double NuSig_rp_ip =
		    data[t_data::VISCOSITY_SIGMA_RP](n_radial + 1, n_azimuthal);
		...
		const double NuSigma_jm =
		    data[t_data::VISCOSITY_SIGMA](n_radial, n_azimuthal_minus);
		const double NuSigma_im =
		    data[t_data::VISCOSITY_SIGMA](n_radial - 1, n_azimuthal);
		...
		const double Ra3NuSigmaInvDiffRmed = NuSig_rp *
						     std::pow(Ra[n_radial], 3) *
						     InvDiffRmed[n_radial];
		const double Ra3NuSigmaInvDiffRmed_p =
		    NuSig_rp_ip * std::pow(Ra[n_radial + 1], 3) *
		    InvDiffRmed[n_radial + 1];

		const double cphi_rp =
		    -InvRmed[n_radial] * TwoDiffRaSq[n_radial] *
		...
		const double c1_phi =
		    (cphi_rp + cphi_pp) / (sigma_avg_phi * Rmed[n_radial]);
		data[t_data::VISCOSITY_CORRECTION_FACTOR_PHI](
		    n_radial, n_azimuthal) = c1_phi;
		...
		const double cr_rp =
		    -(NuSig_rp_jp + NuSig_rp) / (dphi * dphi * Ra[n_radial]);

		double cr_pp_1 =
		    2.0 * NuSigma *
		    (0.5 * InvRmed[n_radial] +
		     1.0 / 3.0 * Ra[n_radial] * InvDiffRsupRb[n_radial]);
		double cr_pp_2 =
		    2.0 * NuSigma_im *
		...
		double cr_rr_1 =
		    Rmed[n_radial] * 2.0 * NuSigma *
		    (-InvDiffRsup[n_radial] +
		     1.0 / 3.0 * Ra[n_radial] * InvDiffRsupRb[n_radial]);
		double cr_rr_2 =
		    -1.0 * Rmed[n_radial - 1] * 2.0 * NuSigma_im *
				      ...
				      t_polargrid &v_azimuthal, double dt)
{
	...
	for (unsigned int naz = 0; naz <= v_radial.get_max_azimuthal(); ++naz) {
	    ...
	    const int naz_minus =
		(naz == 0 ? Sigma.get_max_azimuthal() : naz - 1);
	    ...
	    if (StabilizeViscosity == 1) {
		const double cr =
		    data[t_data::VISCOSITY_CORRECTION_FACTOR_R](nr, naz);
		const double corr =
...
static void get_tau_rp(t_data &data, double &tau_rp_1, double &tau_rp_2,
		       const int n_radial, const int n_azimuthal, bool r_id_p)
{
	...
	drp_2 = Ra[n_radial] * (-InvRb[n_radial - 1]) * InvDiffRmed[n_radial];
				  ...
				  const double dt)
{
	...
	for (unsigned int n_azimuthal = 0;
	    ...
	    // Rmed[n_radial - 1] * TAU_RR_im_1) * 		InvDiffRmed[n_radial]);
