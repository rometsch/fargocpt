...
class t_data
{
	public:
		...
		enum t_polargrid_type {
			...
			POTENTIAL,	// the gravitational potential felt by any fluid elements (introduced by Fargo-ADSG)
			V_RADIAL_SOURCETERMS,	// v_radial with sourceterms (after substep 1)
			V_AZIMUTHAL_SOURCETERMS,	// v_azimuthal with sourceterms (after substep 1)
		...
		inline t_polargrid& operator[](t_polargrid_type polargrid_type) { return m_polargrids[polargrid_type]; }
		inline t_radialgrid& operator[](t_radialgrid_type radialgrid_type) { return m_radialgrids[radialgrid_type]; }

		void set_size(unsigned int global_n_radial, unsigned int global_n_azimiuthal, unsigned int n_radial, unsigned int n_azimuthal);
		inline t_planetary_system& get_planetary_system() { return m_planetary_system; }
		void print_memory_usage(unsigned int global_n_radial, unsigned int global_n_azimiuthal, unsigned int n_radial, unsigned int n_azimuthal);
		...
		inline ptrdiff_t get_global_n_azimuthal(void) { return m_global_n_azimuthal; };
