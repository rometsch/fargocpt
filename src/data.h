...
class t_data
{
	...
	enum t_polargrid_type {
		...
		POTENTIAL, // the gravitational potential felt by any fluid
			   // elements (introduced by Fargo-ADSG)
		V_RADIAL_SOURCETERMS,    // v_radial with sourceterms (after
					 // substep 1)
		V_AZIMUTHAL_SOURCETERMS, // v_azimuthal with sourceterms (after
					 // substep 1)
	...
	void print_memory_usage(unsigned int n_radial,
				unsigned int n_azimuthal);

	double pdivv_total;
	inline ptrdiff_t get_n_radial(void) { return m_n_radial; };
	inline ptrdiff_t get_n_azimuthal(void) { return m_n_azimuthal; };
	inline ptrdiff_t get_global_n_radial(void)
	{
		return m_global_n_radial;
	};
