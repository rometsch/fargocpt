...
void get_polytropic_constants(char *filename, double &K, double &gamma)
{
	...
	strcat(command, " 0");
	// Python script fits the parameters ADIABATICINDEX and
	// POLYTROPIC_CONSTANT,
...
void ReadVariables(char *filename, t_data &data, int argc, char **argv)
{
	...
	IMPOSEDDISKDRIFT =
	    config::value_as_double_default("IMPOSEDDISKDRIFT", 0.0);
	...
	if (asprintf(&OUTPUTDIR, "%s",
			 ...
			 "OUTPUTDIR", par_file_name.c_str())) < 0) {
	...
	if (CPU_Master) {
		...
		if (PLANETCONFIG != NULL) {
			...
			append_new_planet_file.open(planet_filename.c_str(),
							...
							std::ios::out);
			std::copy(
			    std::istreambuf_iterator<char>(&old_planet_file),
			    {},
			    std::ostreambuf_iterator<char>(
				&append_new_planet_file));
	...
	switch (
	    tolower(*config::value_as_string_default("TRANSPORT", "Fast"))) {
	case 'f':
		FastTransport = 1;
		break;
	case 's':
	...
	if ((parameters::radial_grid_type == parameters::logarithmic_spacing) ||
		...
		double optimal_N_azimuthal = M_PI / ((exp(c / NRadial) - 1.0) /
						     (exp(c / NRadial) + 1.0));

		// check if optimal azimuthal cell number differs from actual
		// azimuthal cell number by more than 10%
		if (fabs(((double)NAzimuthal - optimal_N_azimuthal) /
			 (double)NAzimuthal) > 0.1) {
			logging::print_master(
			    LOG_WARNING
			    "You have %u cells in azimuthal direction. This "
			    "should be %u cells to have quadratic cells!\n",
	...
	if (!config::key_exists("OuterBoundary")) {
		logging::print_master(
		    LOG_ERROR "OuterBoundary doesn't exist. Old .par file?\n");
	...
	parameters::exitOnDeprecatedSetting(
	    "IndirectTerm",
	    "Indirect terms are now handled automatically to avoid unphysical "
	    "settings.",
	...
	if (Adiabatic_deprecated == 'n') {
		logging::print_master(
		    LOG_INFO
		    "Warning : Setting the isothermal equation of state with "
		    "the flag 'Adiabatic   NO' is deprecated. Use "
		    "'EquationOfState   Isothermal' instead.\n");
	}
	if (Adiabatic_deprecated == 'y') {
		parameters::Adiabatic = true;
		logging::print_master(
		    LOG_INFO "Warning : Setting the ideal equation of state "
			     "with the flag 'Adiabatic    YES' is deprecated. "
			     "Use 'EquationOfState   Adiabatic' instead.\n");
		...
		if ((parameters::Adiabatic) && (ADIABATICINDEX == 1)) {
			logging::print_master(
			    LOG_WARNING
			    "You cannot have Adiabatic=true and AdiabatcIndex "
			    "= 1. I decided to put Adiabatic=false, to  "
			    "simulate a locally isothermal equation of state. "
			    "Please check that it what you really wanted to "
			    "do!\n");
		...
		char eos_string[512];
		strncpy(eos_string,
			config::value_as_string_default("EquationOfState",
							"Isothermal"),
		...
		if (strcmp(eos_string, "isothermal") == 0 ||
			...
			logging::print_master(
			    LOG_INFO "Using isothermal equation of state.\n");
		}
		if (strcmp(eos_string, "adiabatic") == 0 ||
			...
			if (strcmp(ADIABATICINDEX_string, "fit_isothermal") ==
				0 ||
			    strcmp(ADIABATICINDEX_string, "fit isothermal") ==
				0) {
				logging::print_master(
				    LOG_ERROR
				    "Automatic AdiabatcIndex determination "
				    "only available for polytropic equation of "
				    "state\n");
				PersonalExit(1);
			} else {
				ADIABATICINDEX =
				    config::value_as_double_default(
					"AdiabaticIndex", 7.0 / 5.0);
			}
			...
			if ((parameters::Adiabatic) && (ADIABATICINDEX == 1)) {
				logging::print_master(
				    LOG_WARNING
				    "You cannot have Adiabatic=true and "
				    "AdiabatcIndex = 1. I decided to put "
				    "Adiabatic=false, to simulate a locally "
				    "isothermal equation of state. Please "
				    "check that it what you really wanted to "
				    "do!\n");
				parameters::Adiabatic = false;
			}
			logging::print_master(
			    LOG_INFO "Using ideal equation of state.\n");
		}
		...
		if (strcmp(eos_string, "polytropic") == 0 ||
			...
			strncpy(POLYTROPIC_CONSTANT_string,
				config::value_as_string_default(
				    "PolytropicConstant", "12.753"),
				256); // same as MAXNAME from config.cpp
			for (char *t = POLYTROPIC_CONSTANT_string; *t != '\0';
			     ++t) {
				*t = tolower(*t);
			}

			if (strcmp(POLYTROPIC_CONSTANT_string,
				   "fit_isothermal") == 0 ||
			    strcmp(POLYTROPIC_CONSTANT_string,
				   "fit isothermal") == 0) {
				if (K == 0.0) // Call script only if needed
				{
					get_polytropic_constants(filename, K,
								 gamma);
				}
			...
			if ((parameters::Polytropic) && (ADIABATICINDEX == 1)) {
				logging::print_master(
				    LOG_WARNING
				    "You cannot have Polytropic=true and "
				    "AdiabatcIndex = 1. I decided to put "
				    "Polytropic=false, to simulate a locally "
				    "isothermal equation of state. Please "
				    "check that it what you really wanted to "
				    "do!\n");
				parameters::Polytropic = false;
			...
			logging::print_master(
			    LOG_INFO "Using polytropic equation of state.\n");
		}
	...
	if (!parameters::Adiabatic) // if energy is not needed, delete the
				    // energy damping boundary conditions
	{
		parameters::damping_vector.erase(
		    parameters::damping_vector.begin() +
	...
	if ((ALPHAVISCOSITY != 0.0) && (VISCOSITY != 0.0)) {
		...
		logging::print_master(
		    LOG_ERROR "Edit the parameter file so as to remove\n");
		logging::print_master(
		    LOG_ERROR "one of these variables and run again.\n");
		PersonalExit(1);
	}
	...
	if ((parameters::thickness_smoothing != 0.0) &&
		...
		logging::print_master(LOG_ERROR
				      "You cannot use at the same time\n");
		logging::print_master(
		    LOG_ERROR "`ThicknessSmoothing' and `RocheSmoothing'.\n");
		logging::print_master(
		    LOG_ERROR "Edit the parameter file so as to remove\n");
		logging::print_master(
		    LOG_ERROR "one of these variables and run again.\n");
	...
	if ((parameters::thickness_smoothing <= 0.0) &&
	    (ROCHESMOOTHING <= 0.0)) {
		logging::print_master(LOG_ERROR
				      "A non-vanishing potential smoothing "
				      "length is required.\n");
		logging::print_master(
		    LOG_ERROR
		    "Please use either of the following variables:\n");
		logging::print_master(
		    LOG_ERROR "`ThicknessSmoothing' *or* `RocheSmoothing'.\n");
		logging::print_master(LOG_ERROR
	...
	if (ROCHESMOOTHING != 0.0) {
		...
		logging::print_master(LOG_INFO
				      "Planet potential smoothing scales with "
				      "their Hill sphere.\n");
	} else if (config::value_as_bool_default("ThicknessSmoothingAtPlanet",
						 0)) {
		ThicknessSmoothingAtCell = NO;
		ThicknessSmoothingAtPlanet = YES;
		logging::print_master(
		    LOG_INFO "Planet potential smoothing uses disk scale "
			     "height at planet location (bad choice!).\n");
	} else {
		ThicknessSmoothingAtCell = YES;
		ThicknessSmoothingAtPlanet = NO;
		logging::print_master(LOG_INFO
				      "Planet potential smoothing uses disk "
				      "scale height at gas cell location.\n");
	}
	...
	// units::calculate_unit_factors() so it is not possible. Moving the
	// read() call causes an error.
...
{
	logging::print_master(LOG_ERROR
			      "Usage : %s [-abcdeimnptvz] [-(0-9)] [-s number] "
			      "[-f scaling] parameters file\n",
			      execname);
	logging::print_master(
	    LOG_ERROR
	...
	logging::print_master(LOG_ERROR "-c : Sloppy CFL condition (checked at "
					"each DT, not at each timestep)\n");
	logging::print_master(LOG_ERROR "-d : Print some debugging information "
					"on 'stdout' at each timestep\n");
	logging::print_master(
	    LOG_ERROR "-e : Activate EU test problem torque file output\n");
	logging::print_master(LOG_ERROR
			      "-f : Scale density array by 'scaling'. Useful "
			      "to increase/decrease\n");
	logging::print_master(LOG_ERROR "     disk surface density after a "
					"restart, for instance.            \n");
	logging::print_master(
	    LOG_ERROR
	    "-i : tabulate Sigma profile as given by restart files\n");
	logging::print_master(LOG_ERROR "-n : Disable simulation. The program "
					"just reads parameters file\n");
	logging::print_master(
	    LOG_ERROR "-o : Overrides output directory of input file.\n");
	logging::print_master(
	    LOG_ERROR "-p : Give profiling information at each time step\n");
	logging::print_master(LOG_ERROR
			      "-s : Restart simulation, taking #'number' files "
	...
	logging::print_master(LOG_ERROR
			      "     This option must stand alone on one switch "
			      "(-va -4 is legal, -v4a is not)\n");
	PersonalExit(1);
}
...
void TellEverything()
{
	...
	*/
	...
	// temp=2.0*PI*parameters::sigma0/(2.0-SIGMASLOPE)*(pow(RMAX,2.0-SIGMASLOPE)
	// - 1.0); logging::print_master(LOG_VERBOSE "Initial Mass outer to
	// r=1.0 : %g \n", temp);
	logging::print_master(LOG_VERBOSE
