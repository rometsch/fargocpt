...
void ReadVariables(char *filename, t_data &data, int argc, char **argv)
{
	...
	if (fabs(((double)NAzimuthal - optimal_N_azimuthal) /
		...
		LOG_WARNING "You have %u cells in azimuthal direction. This "
	...
	logging::print_master(
	    LOG_INFO "Warning : Setting the isothermal equation of state with "
	...
	logging::print_master(LOG_INFO
			      "Warning : Setting the ideal equation of state "
	...
	if ((parameters::Adiabatic) && (ADIABATICINDEX == 1)) {
	    logging::print_master(
		LOG_WARNING "You cannot have Adiabatic=true and AdiabatcIndex "
	...
	if (strcmp(eos_string, "adiabatic") == 0 ||
		...
		*t = tolower(*t);
		...
		logging::print_master(
		    LOG_ERROR "Automatic AdiabatcIndex determination "
    ...
    if ((parameters::thickness_smoothing <= 0.0) && (ROCHESMOOTHING <= 0.0)) {
	logging::print_master(LOG_ERROR "A non-vanishing potential smoothing "
					"length is required.\n");
	logging::print_master(
	...
	RocheSmoothing = YES;
	logging::print_master(LOG_INFO "Planet potential smoothing scales with "
				       "their Hill sphere.\n");
	...
	logging::print_master(LOG_INFO
			      "Planet potential smoothing uses disk scale "
			      "height at planet location (bad choice!).\n");
    } else {
	ThicknessSmoothingAtCell = YES;
	ThicknessSmoothingAtPlanet = NO;
	logging::print_master(LOG_INFO "Planet potential smoothing uses disk "
