...
void init_radialarrays()
{
	...
	if (asprintf(&fd_input_filename, "%s%s", OUTPUTDIR, "radii.dat") ==
	    -1) {
	...
	if (fd_input == NULL) {
		...
		case parameters::arithmetic_spacing:
			...
			for (nRadial = 0; nRadial <= GlobalNRadial; ++nRadial) {
				Radii[nRadial] =
				    RMIN + interval * (double)(nRadial - 1.0);
			}
			break;
		case parameters::exponential_spacing:
			slope = (.011 * 5. - .01) * 100. /
				((double)(GlobalNRadial - 2));
			for (nRadial = 0; nRadial <= GlobalNRadial; ++nRadial) {
				Radii[nRadial] =
				    (RMIN - RMAX) /
					(exp(slope) -
					 exp(slope *
					     ((double)GlobalNRadial - 1))) *
		...
		for (nRadial = 0; nRadial <= GlobalNRadial; ++nRadial) {
			...
			if (fscanf(fd_input, "%lf", &temp) == 1) {
				...
				logging::print_master(
				    LOG_ERROR "Reading 'radii.dat' file: No "
					      "data left to read :(\n");
				PersonalExit(1);
			}
		}
	...
	for (nRadial = 0; nRadial < GlobalNRadial; ++nRadial) {
		...
		// Rmed = 1/2 * [ (4/3 Pi r_sup^3) - (4/3 Pi r_inf^3) ] / [ (Pi
		// r_sup^2) - (Pi r_inf^2) ]
		GlobalRmed[nRadial] =
		    2.0 / 3.0 *
		    (Radii[nRadial + 1] * Radii[nRadial + 1] *
			 Radii[nRadial + 1] -
		     Radii[nRadial] * Radii[nRadial] * Radii[nRadial]);
		GlobalRmed[nRadial] = GlobalRmed[nRadial] /
				      (Radii[nRadial + 1] * Radii[nRadial + 1] -
				       Radii[nRadial] * Radii[nRadial]);
	}

	logging::print_master(
	    LOG_VERBOSE "Active %s grid is ranging from %g to %g. Total grid "
			"is range from %g to %g.\n",
	    parameters::radial_grid_names[parameters::radial_grid_type],
	    Radii[1], Radii[GlobalNRadial - 1], Radii[0], Radii[GlobalNRadial]);
	...
	for (nRadial = 0; nRadial < NRadial; ++nRadial) {
		...
		// TODO: Is already calculated a few lines above. assert should
		// check this
		assert((Rmed[nRadial] - GlobalRmed[nRadial + IMIN]) <
		       DBL_EPSILON);

		Surf[nRadial] = PI *
				(pow2(Rsup[nRadial]) - pow2(Rinf[nRadial])) /
				(double)NAzimuthal;
	...
	for (nRadial = 1; nRadial < NRadial; ++nRadial) {
		InvDiffRmed[nRadial] =
		    1.0 / (Rmed[nRadial] - Rmed[nRadial - 1]);
	}
	...
	if (CPU_Master) {
		...
		if (asprintf(&fd_output_filename, "%s%s", OUTPUTDIR,
			     "used_rad.dat") == -1) {
			logging::print_master(
...
void init_physics(t_data &data)
{
	if ((parameters::sigma_initialize_condition ==
		...
		return;
		   ...
		   (parameters::energy_initialize_condition ==
		    parameters::initialize_condition_shakura_sunyaev)) {
		die("Both Sigma and Energy have to be initialised by Shakura & "
		    "Sunyaev Standard-Solution. Other initialisation not yet "
		    "implemented!");
	}
	...
	if (Corotating) {
		OmegaFrame =
		    data.get_planetary_system().get_planet(0).get_omega();
	}
...
void init_shakura_sunyaev(t_data &data)
{
	...
	if (!parameters::Adiabatic)
		die("Isothermal equation of state and Shakura & Sunyaev "
		    "starting conditions has not yet been implemented!");

	for (unsigned int n_radial = 0;
	     n_radial < data[t_data::TEMPERATURE].Nrad; ++n_radial) {
		for (unsigned int n_azimuthal = 0;
		     n_azimuthal < data[t_data::TEMPERATURE].Nsec;
		     ++n_azimuthal) {

			factor =
			    pow(1. - sqrt(parameters::star_radius /
					  (Rb[n_radial] +
			...
			data[t_data::DENSITY](n_radial, n_azimuthal) =
			    (5.2 * pow(ALPHAVISCOSITY, -4. / 5.) *
			     pow(parameters::mass_accretion_rate *
				     units::mass_accretion_rate
					 .get_cgs_factor() /
				     1.e16,
				 7. / 10.) *
			     pow(parameters::M0, 0.25) *
			     pow(Rb[n_radial] * units::length.get_cgs_factor() /
				     1.e10,
				 -0.75) *
			     pow(factor, 14. / 5.)) /
			...
			data[t_data::V_RADIAL](n_radial, n_azimuthal) =
			    -(2.7e4 * pow(ALPHAVISCOSITY, 4. / 5.) *
			      ...
			      pow(parameters::M0, -0.25) *
			      pow(Rb[n_radial] *
				      units::length.get_cgs_factor() / 1.e10,
				  -0.25) *
			      pow(factor, -14. / 5.)) /
			    units::velocity.get_cgs_factor();

			data[t_data::SOUNDSPEED](n_radial, n_azimuthal) = sqrt(
			    constants::R / parameters::MU * ADIABATICINDEX *
			    data[t_data::TEMPERATURE](n_radial, n_azimuthal));
			data[t_data::ENERGY](n_radial, n_azimuthal) =
			    constants::R / parameters::MU * 1. /
			    (ADIABATICINDEX - 1.) *
			    data[t_data::DENSITY](n_radial, n_azimuthal) *
			    data[t_data::TEMPERATURE](n_radial, n_azimuthal);
			...
			// 	data[t_data::TEMPERATURE](n_radial, n_azimuthal)
			// =
			// pow( 3.*constants::G*M*parameters::mass_accretion_rate
			// /
			// (8.*PI*pow(Rb[n_radial],3)*constants::sigma.get_code_value()
			// ) * (1. -
			// pow(parameters::star_radius/Rb[n_radial+1],.5)),
			// 0.25); 	data[t_data::SOUNDSPEED](n_radial,
			// n_azimuthal) = sqrt( constants::R/parameters::MU *
			// ADIABATICINDEX * data[t_data::TEMPERATURE](n_radial,
			// n_azimuthal) );
			// data[t_data::ASPECTRATIO](n_radial, n_azimuthal) =
			...
			// n_azimuthal)*data[t_data::TEMPERATURE](n_radial,
			// n_azimuthal); 	data[t_data::PRESSURE](n_radial,
			// n_azimuthal) =
			...
			// (8.*PI*pow(Rb[n_radial],3)*constants::sigma.get_code_value()
			// ) * (1. -
			// pow(parameters::star_radius/Rb[n_radial],.5)), 0.25);
	...
	if (parameters::self_gravity)
		die("Self-gravity and Shakura-Sunyaev starting values has not "
	...
	if (CentrifugalBalance)
		die("CentrifugalBalance and Shakura-Sunyaev starting values "
...
void init_gas_density(t_data &data)
{
	...
	case parameters::initialize_condition_profile:
		logging::print_master(
		    LOG_INFO
		    "Initializing Sigma(r) = %g = %g %s * [r/(%g AU)]^(%g)\n",
		    parameters::sigma0,
		    parameters::sigma0 *
			units::surface_density.get_cgs_factor(),
		...
		for (unsigned int n_radial = 0;
		     n_radial < data[t_data::DENSITY].Nrad; ++n_radial) {
			for (unsigned int n_azimuthal = 0;
			     n_azimuthal < data[t_data::DENSITY].Nsec;
			     ++n_azimuthal) {
				data[t_data::DENSITY](n_radial, n_azimuthal) =
				    parameters::sigma0 *
				    pow(Rmed[n_radial], -SIGMASLOPE);
			}
		}
		break;
	...
	case parameters::initialize_condition_shakura_sunyaev:
		...
		// parameters::initialize_condition_shakura_sunyaev:
		// 			logging::print_master(LOG_INFO
		// "Initializing Sigma from Shakura and Sunyaev 1973 standard
		// solution (cf. A&A, 24, 337)");
		//
		// 			for (unsigned int n_radial = 0; n_radial
		// <
		// data[t_data::DENSITY].Nrad; ++n_radial) {
	...
	if (parameters::sigma_randomize) {
		...
		if (open_simplex_noise(parameters::random_seed + 325582,
				       &osn) !=
		    0) // add random number to seed to keep seed unique, because
		       // it is reused several times in the code.
		{
			die("Bad open simplex noise!");
		}

		logging::print_master(LOG_INFO
				      "Randomizing Sigma by %.2f %%.\n",
		...
		for (unsigned int n_radial = 0;
			...
			for (unsigned int n_azimuthal = 0;
				...
				double angle = (double)n_azimuthal /
					       (double)data[t_data::V_RADIAL]
						   .get_size_azimuthal() *
					       2.0 * PI;
				...
				for (int i = 0; i < simplex_order; ++i) {
					...
					double weight_factor = double(
					    1 << (simplex_order - i - 1));

					noise +=
					    open_simplex_noise2(
						osn, feature_factor * x / f,
						feature_factor * y / f) *
					    weight_factor;
				}
				...
				data[t_data::DENSITY](n_radial, n_azimuthal) *=
				    (1 +
				     parameters::sigma_random_factor * noise);
			}
		}
	...
	if (parameters::profile_damping) {
		logging::print_master(
		    LOG_INFO
		    "Damping Sigma for r > %g %s over a range from %g %s\n",
		    parameters::profile_damping_point *
			units::length.get_cgs_factor(),
		    units::length.get_cgs_symbol(),
		    parameters::profile_damping_width *
			units::length.get_cgs_factor(),
		    units::length.get_cgs_symbol());
		for (unsigned int n_radial = 0;
		     n_radial <= data[t_data::DENSITY].get_max_radial();
	...
	if (parameters::sigma_adjust) {
		...
		logging::print_master(
		    LOG_INFO
		    "Setting Sigma0=%g %s to set disc mass of %g to %g.\n",
		    parameters::sigma0 *
			units::surface_density.get_cgs_factor(),
...
void init_gas_energy(t_data &data)
{
	if (ADIABATICINDEX == 1.0) {
		logging::print_master(
		    LOG_ERROR
		    "The adiabatic index must differ from unity to initialize "
	...
	case parameters::initialize_condition_profile:
		logging::print_master(
		    LOG_INFO
		    "Initializing Energy=%g %s * [r/(%.1f AU)]^(%g). Flaring "
		    "index is %g. T=%g %s * [r/(%.1f AU)]^(%g).\n",
		...
		for (unsigned int n_radial = 0;
			...
			for (unsigned int n_azimuthal = 0;
			     ...
			     ++n_azimuthal) {
				data[t_data::ENERGY](n_radial, n_azimuthal) =
				    1.0 / (ADIABATICINDEX - 1.0) *
				    parameters::sigma0 * pow2(ASPECTRATIO_REF) *
				    pow(Rmed[n_radial],
	...
	if (parameters::profile_damping) {
		logging::print_master(
		    LOG_INFO
		    "Damping Energy for r > %g %s over a range of %g %s\n",
		    parameters::profile_damping_point *
			units::length.get_cgs_factor(),
		    units::length.get_cgs_symbol(),
		    parameters::profile_damping_width *
...
void init_gas_velocities(t_data &data)
{
	...
	if (parameters::initialize_pure_keplerian) {
		for (unsigned int n_radial = 0;
		     ...
		     ++n_radial) {
			// TODO: This should be Rinf but seems to produce
			// incorrect data
	...
	if (CentrifugalBalance) {
		...
		/* vt_int \equiv rOmega� = grad(P)/sigma +
		 * \partial(phi)/\partial(r)  -  acc_sg_radial */
		mpi_make1Dprofile(data[t_data::PRESSURE].Field,
				  GLOBAL_bufarray);
		...
		for (int i = 0; i < CPU_Number; ++i) {
			if (i == CPU_Rank) {
				...
				if (CPU_Rank > 0) {
					MPI_Irecv(&SigmaMedGlobal[0],
						  IMIN + CPUOVERLAP - 1,
						  MPI_DOUBLE, CPU_Prev, 0,
				...
				for (unsigned int j =
				     ...
				     j <= IMAX - (CPU_Rank == CPU_Highest
						      ? 0
						      : CPUOVERLAP);
				     ++j) {
					SigmaMedGlobal[j] = SigmaMed
					    [Zero_or_active + j - IMIN -
					     (CPU_Rank == 0 ? 0 : CPUOVERLAP)];
				}
				// send data to next CPU
				if (CPU_Rank < CPU_Highest) {
					MPI_Isend(&SigmaMedGlobal[0],
						  IMAX - CPUOVERLAP, MPI_DOUBLE,
						  CPU_Next, 0, MPI_COMM_WORLD,
						  &request);
					MPI_Wait(&request, &status);
				}
			}
		...
		MPI_Bcast(&SigmaMedGlobal[0], GlobalNRadial, MPI_DOUBLE,
			  CPU_Highest, MPI_COMM_WORLD);

		/* global axisymmetric pressure field, known by all cpus */
		for (unsigned int i = 1; i < GlobalNRadial; i++) {
			vt_int[i] =
			    (GLOBAL_bufarray[i] - GLOBAL_bufarray[i - 1]) /
				(.5 *
				 (SigmaMedGlobal[i] + SigmaMedGlobal[i - 1])) /
		...
		if (parameters::self_gravity) { // Better test with CL rigid!
			...
			mpi_make1Dprofile(selfgravity::g_radial,
					  GLOBAL_AxiSGAccr);

			for (unsigned int i = 1; i < GlobalNRadial; i++) {
				vt_int[i] -=
				    ((Radii[i] - GlobalRmed[i - 1]) *
					 GLOBAL_AxiSGAccr[i] +
				     (GlobalRmed[i] - Radii[i]) *
					 GLOBAL_AxiSGAccr[i - 1]) /
				    (GlobalRmed[i] - GlobalRmed[i - 1]);
			}
		...
		for (unsigned int i = 1; i < GlobalNRadial; i++)
			vt_int[i] =
			    sqrt(vt_int[i] * Radii[i]) - Radii[i] * OmegaFrame;
	...
	for (unsigned int n_radial = 0;
		...
		for (unsigned int n_azimuthal = 0;
			...
			if (n_radial == data[t_data::V_RADIAL].Nrad) {
				...
				data[t_data::V_RADIAL](n_radial, n_azimuthal) =
				    IMPOSEDDISKDRIFT * parameters::sigma0 /
				    SigmaInf[n_radial] / ri;
	...
	for (unsigned int n_azimuthal = 0;
		...
		data[t_data::V_RADIAL](data[t_data::V_RADIAL].Nrad,
				       n_azimuthal) = 0.0;
	}
}
