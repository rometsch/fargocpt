/*
	...
	Total mass and angular momentum monitoring, and boundary conditions. In addition, this file contains a few low-level functions that manipulate PolarGrid 's or initialize the forces evaluation.
...
{
	static double totalStartAngularMomentum, gasStartAngularMomentum, planetsStartAngularMomentum;
	static int firstStart = 1;

	unsigned int nPlanet;

	double totalAngularMomentum, gasAngularMomentum, planetsAngularMomentum = 0.0, planetAngularMomentum = 0.0;
	...
	for (nPlanet = 0; nPlanet < data.get_planetary_system().get_number_of_planets(); ++nPlanet) {
	...
	if (firstStart) {
		...
		if ( PhysicalTime > 1e-10 ) {
			logging::print_master(LOG_INFO "CheckAngularMomentumConservation is called for the first time very late: t=%f\n",PhysicalTime);
		}
		...
		logging::print_master(LOG_INFO "time = %lg, Hp0 = %lg, Hg0 = %lg et Ht0 = %lg\n", PhysicalTime, planetsStartAngularMomentum, gasStartAngularMomentum, totalStartAngularMomentum);
	}
	...
	if (fd == NULL) {
		logging::print_master(LOG_ERROR "Can't write 'Momentum.dat' file. Aborting.\n");
		PersonalExit(1);
	}
	...
	planetsAngularMomentum = fabs(planetsAngularMomentum - planetsStartAngularMomentum);
	gasAngularMomentum = fabs(gasAngularMomentum - gasStartAngularMomentum);
	totalAngularMomentum = fabs(totalAngularMomentum - totalStartAngularMomentum);

	// print to logfile
	fprintf(fd, "%#.18g\t%#.18g\t%#.18g\t%#.18g\t%#.18g\n", PhysicalTime, planetsAngularMomentum, gasAngularMomentum, totalAngularMomentum, totalAngularMomentum / totalStartAngularMomentum);
...
{
	for (unsigned int n_radial = 0; n_radial <= result.get_max_radial(); ++n_radial) {
		for (unsigned int n_azimuthal = 0; n_azimuthal <= result.get_max_azimuthal(); ++n_azimuthal) {
			result(n_radial, n_azimuthal) = num(n_radial, n_azimuthal)/(denom(n_radial, n_azimuthal)+DBL_EPSILON);
		}
	}
}
...
void InitComputeAccel()
{
	...
	for (nRadial = 0; nRadial < CellAbscissa->Nrad; ++nRadial) {
		for (nAzimuthal = 0; nAzimuthal < CellAbscissa->Nsec; ++nAzimuthal) {
			...
			CellAbscissa->Field[cell] = Rmed[nRadial] * cos(2.0*PI*(double)nAzimuthal/(double)(CellAbscissa->Nsec));
			CellOrdinate->Field[cell] = Rmed[nRadial] * sin(2.0*PI*(double)nAzimuthal/(double)(CellAbscissa->Nsec));
		}
	}
}
...
void NonReflectingBoundary_inner(t_data &data, t_polargrid* VRadial, t_polargrid* Density, t_polargrid* Energy)
{
	...
	if (CPU_Rank == 0) {
		...
		for (nAzimuthal = 0; nAzimuthal < Density->Nsec; ++nAzimuthal) {
			...
			Energy->Field[lp] = Energy->Field[cell];   // copy first ring into ghost ring
			vr_med = -data[t_data::SOUNDSPEED].Field[cell]*(rho[cell]-SigmaMed[1])/SigmaMed[1];
...
void NonReflectingBoundary_outer(t_data &data, t_polargrid* VRadial, t_polargrid* Density, t_polargrid* Energy)
{
	...
	if (CPU_Rank == CPU_Highest) {
		...
		csnrm1 = 0.0;
		for (nAzimuthal = 0; nAzimuthal < Density->Nsec; ++nAzimuthal) {
			csnrm2 += data[t_data::SOUNDSPEED].Field[(Density->Nrad-2)*Density->Nsec+nAzimuthal];
			csnrm1 += data[t_data::SOUNDSPEED].Field[(Density->Nrad-1)*Density->Nsec+nAzimuthal];
		}
		csnrm1 /= (double)(Density->Nsec);
		csnrm2 /= (double)(Density->Nsec);
		nRadial = Density->Nrad-1;		 // The expression below should be refined
		// We need to know the orbital frequency of the nearest planet
		dangle = (pow(Rinf[Density->Nrad-2],-1.5)-1.0)/(.5*(csnrm1+csnrm2));
		...
		for (nAzimuthal = 0; nAzimuthal < Density->Nsec; ++nAzimuthal) {
			...
			Energy->Field[cell] = Energy->Field[lp];	// copy first ring into ghost ring
			vr_med = data[t_data::SOUNDSPEED].Field[cell]*(rho[cell-Density->Nsec]-SigmaMed[Density->Nrad-2])/SigmaMed[Density->Nrad-2];
			vr[cell] = 2.*vr_med-vr[cell-Density->Nsec];
		}
		mean = 0.0;
		for (nAzimuthal = 0; nAzimuthal < Density->Nsec; ++nAzimuthal) {
		...
		mean /= (double)(Density->Nsec);
		for (nAzimuthal = 0; nAzimuthal < Density->Nsec; ++nAzimuthal) {
			rho[nAzimuthal+(Density->Nrad-1)*Density->Nsec] += SigmaMed[Density->Nrad-1]-mean;
		}
		mean = 0.0;
		for (nAzimuthal = 0; nAzimuthal < Density->Nsec; ++nAzimuthal) {
			mean += Energy->Field[nAzimuthal+Density->Nsec*(Density->Nrad-1)];
		}
		mean /= (double)(Density->Nsec);
		for (nAzimuthal = 0; nAzimuthal < Density->Nsec; ++nAzimuthal) {
			Energy->Field[nAzimuthal+(Density->Nrad-1)*Density->Nsec] += EnergyMed[Density->Nrad-1]-mean;
		}
	}
}
...
void EvanescentBoundary(t_data &data, double step)
{
	...
	DRMAX = GlobalRmed[GlobalNRadial-1] - 0.2667*(GlobalRmed[GlobalNRadial-1]-rp);
	...
	for (nRadial = Zero_or_active; nRadial < Max_or_active; ++nRadial) {
		if ( (Rmed[nRadial] < DRMIN) || (Rmed[nRadial] > DRMAX) ) {
			...
			if (Rmed[nRadial] > DRMAX) {
				damping = (Rmed[nRadial]-DRMAX)/(GlobalRmed[GlobalNRadial-1]-DRMAX);
			...
			if (!ViscosityAlpha && (VISCOSITY == 0.0) )
				viscosity = 0.0;
			if (!SelfGravity) {
				vtheta0 = sqrt ( G*1.0/Rmed[i] * ( 1.0 - (1.0+SIGMASLOPE-2.0*FLARINGINDEX)* pow(AspectRatio(Rmed[i]),2.0)*pow(Rmed[i],2.0*FLARINGINDEX) ) );
			}
			if (SelfGravity) {
				vtheta0 = sqrt ( G*1.0/Rmed[i] * ( 1.0 - (1.0+SIGMASLOPE-2.0*FLARINGINDEX)* pow(AspectRatio(Rmed[i]),2.0)*pow(Rmed[i],2.0*FLARINGINDEX) ) - Rmed[i]*GLOBAL_AxiSGAccr[i+IMIN] );
			}
			...
			*/
			...
			dens0 = 0.0;
			energ0 = 0.0;
			for (nAzimuthal = 0; nAzimuthal < data[t_data::DENSITY].Nsec; ++nAzimuthal) {
			...
			for (nAzimuthal = 0; nAzimuthal < data[t_data::DENSITY].Nsec; ++nAzimuthal) {
				...
				vtheta[cell] = (vtheta[cell]+lambda*vtheta0)/(1.0+lambda);
				dens[cell] = (dens[cell]+lambda*dens0)/(1.0+lambda);
				if (parameters::Adiabatic)
					energ[cell] = (energ[cell]+lambda*energ0)/(1.0+lambda);
			}
		}
	}
}
...
void ApplyOuterSourceMass(t_polargrid* Density, t_polargrid* VRadial)
{
	...
	penul_vr = IMPOSEDDISKDRIFT*pow((Rinf[Density->Nrad-1]/1.0),-SIGMASLOPE);
	for (nAzimuthal = 0; nAzimuthal < Density->Nsec; ++nAzimuthal) {
		cell = nAzimuthal+nRadial*Density->Nsec;
...
void ApplySubKeplerianBoundary(t_polargrid& v_azimuthal)
{
	...
	if (!parameters::self_gravity) {
		/* (3.4) on page 44 */
		VKepIn = sqrt ( constants::G*hydro_center_mass/Rb[0] * ( 1.0 - (1.0+SIGMASLOPE-2.0*FLARINGINDEX) * pow(ASPECTRATIO_REF,2.0)*pow(Rb[0],2.0*FLARINGINDEX) ) );
		VKepOut = sqrt ( constants::G*hydro_center_mass/Rb[v_azimuthal.get_max_radial()] * ( 1.0 - (1.0+SIGMASLOPE-2.0*FLARINGINDEX) * pow(ASPECTRATIO_REF,2.0)*pow(Rb[v_azimuthal.get_max_radial()],2.0*FLARINGINDEX) ) );
	} else {
		...
		if ( CPU_Rank == 0 ) {
			...
			VKepIn = sqrt ( constants::G*hydro_center_mass/Rb[0] * ( 1.0 - (1.0+SIGMASLOPE-2.0*FLARINGINDEX) * pow(ASPECTRATIO_REF,2.0)*pow(Rb[0],2.0*FLARINGINDEX) ) - Rb[0]*GLOBAL_AxiSGAccr[0] );
		}

		/* VKepOut is only needed on outermost CPU */
		if ( CPU_Rank == CPU_Highest ) {
			// viscosity::aspect_ratio(Rmed[VAzimuthal->Nrad-1])
			VKepOut = sqrt ( constants::G*hydro_center_mass/Rb[v_azimuthal.get_max_radial()] * ( 1.0 - (1.0+SIGMASLOPE-2.0*FLARINGINDEX) * pow(ASPECTRATIO_REF,2.0)*pow(Rb[v_azimuthal.get_max_radial()],2.0*FLARINGINDEX) ) - Rb[v_azimuthal.get_max_radial()]*GLOBAL_AxiSGAccr[v_azimuthal.get_max_radial()+IMIN] );
		}
	...
	if (CPU_Rank == CPU_Highest) {
		for (unsigned int n_azimuthal = 0; n_azimuthal <= v_azimuthal.get_max_azimuthal(); n_azimuthal++) {
			v_azimuthal(v_azimuthal.get_max_radial(), n_azimuthal) = VKepOut-Rb[v_azimuthal.get_max_radial()]*OmegaFrame;
		}
	}
}
...
{
	// TODO: maybe think about max-1 here as this might be alread set in boundary condition or change in ApplySubKeplerianBoundary
