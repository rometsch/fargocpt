...
void CheckAngularMomentumConservation(t_data &data)
{
	...
	for (nPlanet = 0;
	     ...
	     ++nPlanet) {
		xplanet =
		    data.get_planetary_system().get_planet(nPlanet).get_x();
		yplanet =
		    data.get_planetary_system().get_planet(nPlanet).get_y();
		rpl = sqrt(xplanet * xplanet + yplanet * yplanet);
		thetapl = atan2(yplanet, xplanet);
		vxplanet =
		    data.get_planetary_system().get_planet(nPlanet).get_vx();
		vyplanet =
		    data.get_planetary_system().get_planet(nPlanet).get_vy();
		vazimpl = -vxplanet * sin(thetapl) + vyplanet * cos(thetapl);
		masspl =
		    data.get_planetary_system().get_planet(nPlanet).get_mass();
	...
	if (firstStart) {
		...
		// PhysicalTime < 1e-10 was the "old" condition for saving start
		// values
		if (PhysicalTime > 1e-10) {
			logging::print_master(
			    LOG_INFO
			    "CheckAngularMomentumConservation is called for "
			    "the first time very late: t=%f\n",
			    PhysicalTime);
		}
		...
		logging::print_master(
		    ...
		    PhysicalTime, planetsStartAngularMomentum,
		    gasStartAngularMomentum, totalStartAngularMomentum);
	}
	...
	if (fd == NULL) {
		logging::print_master(
		    LOG_ERROR "Can't write 'Momentum.dat' file. Aborting.\n");
		PersonalExit(1);
	}
	...
	fprintf(fd, "%#.18g\t%#.18g\t%#.18g\t%#.18g\t%#.18g\n", PhysicalTime,
		planetsAngularMomentum, gasAngularMomentum,
		totalAngularMomentum,
...
void InitComputeAccel()
{
	...
	for (nRadial = 0; nRadial < CellAbscissa->Nrad; ++nRadial) {
		for (nAzimuthal = 0; nAzimuthal < CellAbscissa->Nsec;
		     ++nAzimuthal) {
				 ...
				 t_polargrid *Density, t_polargrid *Energy)
{
	...
	if (CPU_Rank == 0) {
		...
		for (nAzimuthal = 0; nAzimuthal < Density->Nsec; ++nAzimuthal) {
			...
			cs1 += data[t_data::SOUNDSPEED]
				   .Field[Density->Nsec + nAzimuthal];
		}
		...
		for (nAzimuthal = 0; nAzimuthal < Density->Nsec; ++nAzimuthal) {
			...
			Energy->Field[lp] =
			    Energy->Field[cell]; // copy first ring into ghost
						 // ring
	...
	if (CPU_Rank == CPU_Highest) {
		...
		for (nAzimuthal = 0; nAzimuthal < Density->Nsec; ++nAzimuthal) {
			csnrm2 +=
				...
				.Field[(Density->Nrad - 2) * Density->Nsec +
				       nAzimuthal];
			csnrm1 +=
			    data[t_data::SOUNDSPEED]
				.Field[(Density->Nrad - 1) * Density->Nsec +
				       nAzimuthal];
		}
		csnrm1 /= (double)(Density->Nsec);
		csnrm2 /= (double)(Density->Nsec);
		nRadial =
		    Density->Nrad - 1; // The expression below should be refined
		// We need to know the orbital frequency of the nearest planet
		...
		for (nAzimuthal = 0; nAzimuthal < Density->Nsec; ++nAzimuthal) {
			...
			Energy->Field[cell] =
			    Energy
				->Field[lp]; // copy first ring into ghost ring
			vr_med = data[t_data::SOUNDSPEED].Field[cell] *
				 (rho[cell - Density->Nsec] -
		...
		for (nAzimuthal = 0; nAzimuthal < Density->Nsec; ++nAzimuthal) {
			mean +=
			    Energy->Field[nAzimuthal +
					  Density->Nsec * (Density->Nrad - 1)];
		}
		mean /= (double)(Density->Nsec);
		for (nAzimuthal = 0; nAzimuthal < Density->Nsec; ++nAzimuthal) {
			Energy->Field[nAzimuthal +
				      (Density->Nrad - 1) * Density->Nsec] +=
...
void EvanescentBoundary(t_data &data, double step)
{
	...
	for (nRadial = Zero_or_active; nRadial < Max_or_active; ++nRadial) {
		if ((Rmed[nRadial] < DRMIN) || (Rmed[nRadial] > DRMAX)) {
			...
			// this could be refined if CentrifugalBalance is
			used... vtheta0 -= Rmed[i]*OmegaFrame; vrad0 =
			-3.0*viscosity/Rmed[i]*(-SIGMASLOPE+.5); dens0 =
			SigmaMed[i]; energ0 = EnergyMed[i];
			*/
			vrad0 = 0.0;
			vtheta0 = 0.0;
			dens0 = 0.0;
			energ0 = 0.0;
			for (nAzimuthal = 0;
			     nAzimuthal < data[t_data::DENSITY].Nsec;
			     ++nAzimuthal) {
				cell = nRadial * data[t_data::DENSITY].Nsec +
				       nAzimuthal;
				vrad0 += vrad[cell];
				vtheta0 += vtheta[cell];
				dens0 += dens[cell];
			...
			for (nAzimuthal = 0;
				...
				cell = nRadial * data[t_data::DENSITY].Nsec +
				       nAzimuthal;
				vrad[cell] = (vrad[cell] + lambda * vrad0) /
					     (1.0 + lambda);
				vtheta[cell] =
				    (vtheta[cell] + lambda * vtheta0) /
				    (1.0 + lambda);
				dens[cell] = (dens[cell] + lambda * dens0) /
					     (1.0 + lambda);
				if (parameters::Adiabatic)
					energ[cell] =
					    (energ[cell] + lambda * energ0) /
					    (1.0 + lambda);
			}
		}
	}
...
void ApplyOuterSourceMass(t_polargrid *Density, t_polargrid *VRadial)
{
	...
	penul_vr = IMPOSEDDISKDRIFT *
		   pow((Rinf[Density->Nrad - 1] / 1.0), -SIGMASLOPE);
	for (nAzimuthal = 0; nAzimuthal < Density->Nsec; ++nAzimuthal) {
		cell = nAzimuthal + nRadial * Density->Nsec;
			...
			VKepOut = sqrt(
				...
				(1.0 - (1.0 + SIGMASLOPE - 2.0 * FLARINGINDEX) *
					   ...
					   pow(Rb[v_azimuthal.get_max_radial()],
					       2.0 * FLARINGINDEX)) -
			    Rb[v_azimuthal.get_max_radial()] *
				GLOBAL_AxiSGAccr[v_azimuthal.get_max_radial() +
						 IMIN]);
		}
	...
	if (CPU_Rank == CPU_Highest) {
		for (unsigned int n_azimuthal = 0;
			...
			v_azimuthal(v_azimuthal.get_max_radial(), n_azimuthal) =
			    VKepOut -
			    Rb[v_azimuthal.get_max_radial()] * OmegaFrame;
		}
	}
}
