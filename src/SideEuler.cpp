...
void ApplySubKeplerianBoundary(t_polargrid& v_azimuthal)
{
	...
	if (!parameters::self_gravity) {
		/* (3.4) on page 44 */
		VKepIn = sqrt ( constants::G*1.0/Rb[0] * ( 1.0 - (1.0+SIGMASLOPE-2.0*FLARINGINDEX) * pow(ASPECTRATIO_REF,2.0)*pow(Rb[0],2.0*FLARINGINDEX) ) );
		VKepOut = sqrt ( constants::G*1.0/Rb[v_azimuthal.get_max_radial()] * ( 1.0 - (1.0+SIGMASLOPE-2.0*FLARINGINDEX) * pow(ASPECTRATIO_REF,2.0)*pow(Rb[v_azimuthal.get_max_radial()],2.0*FLARINGINDEX) ) );
	} else {
		...
		if ( CPU_Rank == 0 ) {
			...
			VKepIn = sqrt ( constants::G*1.0/Rb[0] * ( 1.0 - (1.0+SIGMASLOPE-2.0*FLARINGINDEX) * pow(ASPECTRATIO_REF,2.0)*pow(Rb[0],2.0*FLARINGINDEX) ) - Rb[0]*GLOBAL_AxiSGAccr[0] );
		}

		/* VKepOut is only needed on outermost CPU */
		if ( CPU_Rank == CPU_Highest ) {
			// viscosity::aspect_ratio(Rmed[VAzimuthal->Nrad-1])
			VKepOut = sqrt ( constants::G*1.0/Rb[v_azimuthal.get_max_radial()] * ( 1.0 - (1.0+SIGMASLOPE-2.0*FLARINGINDEX) * pow(ASPECTRATIO_REF,2.0)*pow(Rb[v_azimuthal.get_max_radial()],2.0*FLARINGINDEX) ) - Rb[v_azimuthal.get_max_radial()]*GLOBAL_AxiSGAccr[v_azimuthal.get_max_radial()+IMIN] );
		}
