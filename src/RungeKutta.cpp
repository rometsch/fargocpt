		    ...
		    unsigned int n, double dt)
{
    ...
    for (unsigned int i = 0; i < parameters::n_bodies_for_hydroframe_center;
	 i++) {
...
void TranslatePlanetRK5(double *qold, double c1, double c2, double c3,
			double c4, double c5, double *qnew, unsigned int n)
{
    for (unsigned int i = 0; i < 4 * n; i++)
	qnew[i] = qold[i] + c1 * k1[i] + c2 * k2[i] + c3 * k3[i] + c4 * k4[i] +
		  c5 * k5[i];
}
