...
void TranslatePlanetRK5(double *qold, const double c1, const double c2, const double c3,
			const double c4, const double c5, double *qnew, const unsigned int n, const double dt)
{
    for (unsigned int i = 0; i < 4 * n; i++)
	qnew[i] = qold[i] + dt*(c1 * k1[i] + c2 * k2[i] + c3 * k3[i] + c4 * k4[i] +
		  c5 * k5[i]);
}
