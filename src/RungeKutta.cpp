...
void DerivMotionRK5(double *q_init, double *masses, double *deriv, int n,
		    double dt, int *feelothers)
{
	...
	for (i = 0; i < n; i++) {
		...
		derivy[i] = vy[i];
		derivvx[i] =
		    -constants::G * 1.0 / Dist[i] / Dist[i] / Dist[i] * x[i];
		derivvy[i] =
		    -constants::G * 1.0 / Dist[i] / Dist[i] / Dist[i] * y[i];
		for (j = 0; j < n; j++) {
			// apply correction term caused by non barycenter
			// systems
			derivvx[i] -=
			    constants::G * masses[j] / pow3(Dist[j]) * x[j];
			derivvy[i] -=
			    constants::G * masses[j] / pow3(Dist[j]) * y[j];
			if ((j != i) && (feelothers[i])) {
				dist = pow2(x[i] - x[j]) + pow2(y[i] - y[j]);
				dist = sqrt(dist);
				derivvx[i] += constants::G * masses[j] /
					      pow3(dist) * (x[j] - x[i]);
				derivvy[i] += constants::G * masses[j] /
					      pow3(dist) * (y[j] - y[i]);
			}
		}
	}
...
void TranslatePlanetRK5(double *qold, double c1, double c2, double c3,
			double c4, double c5, double *qnew, int n)
{
	int i;
...
void RungeKutta(double *q0, double dt, double *masses, double *q1, int n,
		int *feelothers)
{
	int i;
	double timestep;
