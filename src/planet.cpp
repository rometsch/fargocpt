...
*/
double t_planet::get_distance() { return sqrt(pow2(get_x()) + pow2(get_y())); }
...
{
	return pow2(get_angular_momentum() / get_mass()) /
	       (constants::G * (M + get_mass())) /
	       (1.0 - pow2(get_eccentricity()));
}
...
{
	// j = r x p = r x mv
	return get_mass() * get_x() * get_vy() -
	       get_mass() * get_y() * get_vx();
}
...
double t_planet::get_eccentricity()
{
	...
	double A_x =
	    get_angular_momentum() / get_mass() * (M + get_mass()) * get_vy() -
	    constants::G * M * pow2(M + get_mass()) * get_x() / d;
	double A_y =
	    -get_angular_momentum() / get_mass() * (M + get_mass()) * get_vx() -
	    constants::G * M * pow2(M + get_mass()) * get_y() / d;
	// eccentricity
	return sqrt(pow2(A_x) + pow2(A_y)) /
	       (constants::G * M * pow2(M + get_mass()));
}
...
void t_planet::create_planet_file()
{
	...
	// create normal file
	if (asprintf(&filename, "%splanet%u.dat", OUTPUTDIR,
		     get_planet_number()) == -1) {
...
void t_planet::write(unsigned int timestep, bool big_file)
{
	...
	if (asprintf(&filename,
		     big_file ? "%sbigplanet%u.dat" : "%splanet%u.dat",
				     ...
				     std::string variable_name)
{
	...
	if (column == -1) {
		std::cerr << "Something went wring in obtaining the column for "
			  << variable_name << " for the planet data file."
			  << std::endl;
		PersonalExit(1);
	}
