...
void t_polargrid::write2D(unsigned int number) const
{
	...
	if (m_unit) {
		...
		// add unit factor
		memcpy(buffer, from,
		       (count) * (get_size_azimuthal()) * sizeof(double));
		for (unsigned int i = 0; i < (count * get_size_azimuthal());
		     ++i) {
			buffer[i] *= m_unit->get_cgs_factor();
		}

		// write data from temporary buffer
		MPI_File_write(fh, buffer, (count) * (get_size_azimuthal()),
			       MPI_DOUBLE, &status);
		...
		MPI_File_write(fh, from, (count) * (get_size_azimuthal()),
			       MPI_DOUBLE, &status);
	}
...
void t_polargrid::write1D(unsigned int number) const
{
	...
	if (asprintf(&filename, "%s/gas%s1D%i.dat", OUTPUTDIR, get_name(),
		     number) < 0) {
		die("Not enough memory!");
	}
	...
	if (m_write_max_max_1D) {
		for (unsigned int n_radial = 0; n_radial < count; ++n_radial) {
			...
			for (unsigned int n_azimuthal = 0;
			     n_azimuthal <= get_max_azimuthal();
			     ++n_azimuthal) {
				buffer[number_of_values * n_radial + 2] = min(
				    buffer[number_of_values * n_radial + 2],
				    operator()(from + n_radial, n_azimuthal));
				buffer[number_of_values * n_radial + 3] = max(
...
void t_polargrid::read2D(const char *_filename)
{
	...
	for (unsigned int n_radial = 0; n_radial <= get_max_radial();
	     ++n_radial) {
		for (unsigned int n_azimuthal = 0; n_azimuthal < Nsec;
		     ++n_azimuthal) {
