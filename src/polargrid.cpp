...
void t_polargrid::set_size(ptrdiff_t size_radial, ptrdiff_t size_azimuthal)
{
	...
	Field = new double[(m_scalar ? size_radial : size_radial+1)*(size_azimuthal)];
...
*/
void t_polargrid::set_unit(units::t_unit &unit)
{
	m_unit = &unit;
}

void t_polargrid::set_vector(bool value)
{
	m_scalar = !value;
}

void t_polargrid::set_scalar(bool value)
{
	m_scalar = value;
}
...
void t_polargrid::clear()
{
	for (unsigned int n_radial = 0; n_radial <= get_max_radial(); ++n_radial) {
		for (unsigned int n_azimuthal = 0; n_azimuthal <= get_max_azimuthal(); ++n_azimuthal) {
...
void t_polargrid::write2D(unsigned int number) const
{
	...
	// if unit is set, copy values to temporary buffer and multiply them by factor
	if (m_unit) {
		double *buffer;
		...
		MPI_File_write(fh, buffer, (count)*(get_size_azimuthal()), MPI_DOUBLE, &status);

		// delete temporary buffer
		delete[] buffer;
	} else {
		// write data from buffer
		MPI_File_write(fh, from, (count)*(get_size_azimuthal()), MPI_DOUBLE, &status);
	}
...
void t_polargrid::write1D(unsigned int number) const
{
	...
	if (asprintf(&filename, "%s/gas%s1D%i.dat",OUTPUTDIR,get_name(),number) <0) {
		die("Not enough memory!");
	}
	...
	for (unsigned int n_radial = 0; n_radial < count; ++n_radial) {
		...
		for (unsigned int n_azimuthal = 0; n_azimuthal <= get_max_azimuthal(); ++n_azimuthal) {
			buffer[number_of_values*n_radial+1] += operator()(from+n_radial,n_azimuthal);
		}
	...
	if (m_write_max_max_1D) {
		for (unsigned int n_radial = 0; n_radial < count; ++n_radial) {
			...
			for (unsigned int n_azimuthal = 0; n_azimuthal <= get_max_azimuthal(); ++n_azimuthal) {
				buffer[number_of_values*n_radial+2] = min(buffer[number_of_values*n_radial+2], operator()(from+n_radial,n_azimuthal));
				buffer[number_of_values*n_radial+3] = max(buffer[number_of_values*n_radial+3], operator()(from+n_radial,n_azimuthal));
			}
		}
	}
	...
	if (m_unit) {
		for (unsigned int n_radial = 0; n_radial < count; ++n_radial) {
			...
			if (m_write_max_max_1D) {
				buffer[number_of_values*n_radial+2] *= m_unit->get_cgs_factor();
				buffer[number_of_values*n_radial+3] *= m_unit->get_cgs_factor();
			}
		}
	}
...
void t_polargrid::read2D(const char* _filename)
{
	...
	if ( count != size ) {
		die("Filename '%s' has %u bytes but has to be %u bytes.",_filename,size,count);
	}
	...
	for (unsigned int n_radial = 0; n_radial <= get_max_radial(); ++n_radial) {
		for (unsigned int n_azimuthal = 0; n_azimuthal < Nsec; ++n_azimuthal) {
			operator()(n_radial,n_azimuthal) = buffer_file[(IMIN+n_radial)*Nsec+n_azimuthal];
		}
	}
...
void t_polargrid::read1D(const char* _filename, bool skip_min_max)
{
	...
	for (unsigned int n_radial = 0; n_radial <= get_max_radial(); ++n_radial) {
		for (unsigned int n_azimuthal = 0; n_azimuthal <= get_max_azimuthal(); ++n_azimuthal) {
			operator()(n_radial,n_azimuthal) = gsl_spline_eval(spline, radius[n_radial] ,acc);
		}
	}
...
{
	// TODO: Speed up by using only one loop and so skipping calculation of cells. This should be done if backward-compability bug (size = (Nrad+1)*(Nsec+1) ) is fixed.
...
unsigned int t_polargrid::get_memory_usage(ptrdiff_t size_radial, ptrdiff_t size_azimuthal)
{
	return (m_scalar ? size_radial : size_radial+1)*(size_azimuthal)*sizeof(double);
}
