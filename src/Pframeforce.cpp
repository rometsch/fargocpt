...
void FillForcesArrays(t_data &data)
{
	...
	if (data.get_planetary_system().get_planet(k).get_rampuptime() > 0) {
	    double ramping = 1.0;
	    if (PhysicalTime <
		data.get_planetary_system().get_planet(k).get_rampuptime() *
		    DT) {
		ramping = 1.0 - pow2(cos(PhysicalTime * PI / 2.0 /
	...
	for (unsigned int n_radial = 0;
		...
		angle = (double)n_azimuthal /
			(double)data[t_data::POTENTIAL].get_size_azimuthal() *
			2.0 * PI;
		x = Rmed[n_radial] * cos(angle);
		y = Rmed[n_radial] * sin(angle);
		double distance2 =
		    pow2(x - planet.get_x()) + pow2(y - planet.get_y());
		...
		pot += constants::G * mplanet * InvPlanetDistance3 *
...
void AdvanceSystemFromDisk(Force *force, t_data &data, double dt)
{
	...
	if (data.get_planetary_system().get_planet(k).get_feeldisk()) {
	    ...
	    gamma = ComputeAccel(force, data, planet.get_x(), planet.get_y(),
				 planet.get_mass());
	    double new_vx =
		planet.get_vx() + dt * gamma.x + dt * IndirectTerm.x;
	    double new_vy =
		planet.get_vy() + dt * gamma.y + dt * IndirectTerm.y;
...
void SolveOrbits(t_data &data)
{
	...
	FindOrbitalElements(
	    x, y, vx, vy,
	    1.0 + data.get_planetary_system().get_planet(i).get_mass(), i);
    }
}
...
void AccreteOntoPlanets(t_data &data, double dt)
{
	...
	if (data.get_planetary_system().get_planet(k).get_acc() > 1e-10) {
	    ...
	    Mplanet += dMplanet;
	    if (data.get_planetary_system().get_planet(k).get_feeldisk()) {
		data.get_planetary_system().get_planet(k).set_vx(PxPlanet /
								 Mplanet);
		data.get_planetary_system().get_planet(k).set_vy(PyPlanet /
								 Mplanet);
	    }
