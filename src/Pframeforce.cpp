/**
	...
	Functions that evaluate the %force between the planets and the disk. The
   FillForcesArrays() function is ill-named: it rather fill an array of the
   potential, that is later use to derive the force acting on the disk at every
   zone. The name of this file is due to the fact that we work in the frame
   centered on the primary (which is therefore not inertial). Old versions of
   fargo also featured the file Gframeforce.c, in which we worked in the frame
   centered on the center of gravity of the system.  The present file also
   contains the functions necessary to update the planets' position and
   velocities (taking into account, or not, the indirect term, ie the term
   arising from the fact that the frame is not inertial), as well as the
   function that initializes the hydrodynamics fields with analytic
   prescription.
*/
...
void FillForcesArrays(t_data &data)
{
	...
	for (unsigned int k = 0;
		...
		if (data.get_planetary_system().get_planet(k).get_rampuptime() >
		    0) {
			double ramping = 1.0;
			if (PhysicalTime < data.get_planetary_system()
						   .get_planet(k)
						   .get_rampuptime() *
					       DT) {
				ramping =
		...
		// calculate smoothing length only once if not dependend on
		// radius
		if (RocheSmoothing) {
			smooth = pow2(r_hill * ROCHESMOOTHING);
		} else {
			smooth = pow2(compute_smoothing_isothermal(
			    planet.get_distance()));
		}

		for (unsigned int n_radial = 0;
		     n_radial <= data[t_data::POTENTIAL].get_max_radial();
		     ++n_radial) {
			...
			for (unsigned int n_azimuthal = 0;
			     ...
			     ++n_azimuthal) {
				// calculate smoothing length if dependend on
				// radius i.e. for thickness smoothing with
				// scale height at cell location
				if (ThicknessSmoothingAtCell &&
				    (!parameters::Locally_Isothermal)) {
					smooth = pow2(compute_smoothing(
					    Rmed[n_radial], data, n_radial,
					    n_azimuthal));
				}
				angle = (double)n_azimuthal /
					(double)data[t_data::POTENTIAL]
					    .get_size_azimuthal() *
					2.0 * PI;
				x = Rmed[n_radial] * cos(angle);
				y = Rmed[n_radial] * sin(angle);
				double distance2 = pow2(x - planet.get_x()) +
						   pow2(y - planet.get_y());
				...
				pot +=
				    constants::G * mplanet *
				    InvPlanetDistance3 *
				    (x * planet.get_x() + y * planet.get_y());

				data[t_data::POTENTIAL](n_radial,
							n_azimuthal) += pot;
			}
		}
	}
...
void AdvanceSystemFromDisk(Force *force, t_data &data, double dt)
{
	...
	for (unsigned int k = 0;
		...
		if (data.get_planetary_system().get_planet(k).get_feeldisk()) {
			...
			gamma = ComputeAccel(force, data, planet.get_x(),
					     planet.get_y(), planet.get_mass());
			double new_vx = planet.get_vx() + dt * gamma.x +
					dt * IndirectTerm.x;
			double new_vy = planet.get_vy() + dt * gamma.y +
					dt * IndirectTerm.y;
...
void AdvanceSystemRK5(t_data &data, double dt)
{
	...
	if (parameters::integrate_planets) {
		for (unsigned int i = 0;
		     ...
		     i++) {
			q0[i + 0 * n] =
			    data.get_planetary_system().get_planet(i).get_x();
			q0[i + 1 * n] =
			    data.get_planetary_system().get_planet(i).get_y();
			q0[i + 2 * n] =
			    data.get_planetary_system().get_planet(i).get_vx();
			q0[i + 3 * n] =
			    data.get_planetary_system().get_planet(i).get_vy();
			PlanetMasses[i] = data.get_planetary_system()
					      .get_planet(i)
					      .get_mass();
			FeelOthers[i] = data.get_planetary_system()
					    .get_planet(i)
					    .get_feelother();
		}
		...
		for (unsigned int i = 0;
		     i < data.get_planetary_system().get_number_of_planets();
		     i++) {
			data.get_planetary_system().get_planet(i).set_x(
			    q1[i + 0 * n]);
			data.get_planetary_system().get_planet(i).set_y(
			    q1[i + 1 * n]);
			data.get_planetary_system().get_planet(i).set_vx(
			    q1[i + 2 * n]);
			data.get_planetary_system().get_planet(i).set_vy(
			    q1[i + 3 * n]);
		}
	}
}
...
void SolveOrbits(t_data &data)
{
	...
	for (unsigned int i = 0;
		...
		FindOrbitalElements(
		    x, y, vx, vy,
		    1.0 + data.get_planetary_system().get_planet(i).get_mass(),
		    i);
	}
}
...
void AccreteOntoPlanets(t_data &data, double dt)
{
	...
	for (unsigned int k = 0;
	     k < data.get_planetary_system().get_number_of_planets(); k++) {
		if (data.get_planetary_system().get_planet(k).get_acc() >
		    1e-10) {
			dMplanet = dPxPlanet = dPyPlanet = 0.0;
			// Hereafter : initialization of W. Kley's parameters
			facc =
			    dt *
			    data.get_planetary_system().get_planet(k).get_acc();
			facc1 = 1.0 / 3.0 * facc;
			facc2 = 2.0 / 3.0 * facc;
			frac1 = 0.75;
			frac2 = 0.45;
			...
			Xplanet =
			    data.get_planetary_system().get_planet(k).get_x();
			Yplanet =
			    data.get_planetary_system().get_planet(k).get_y();
			VXplanet =
			    data.get_planetary_system().get_planet(k).get_vx();
			VYplanet =
			    data.get_planetary_system().get_planet(k).get_vy();
			Mplanet = data.get_planetary_system()
				      .get_planet(k)
				      .get_mass();
			Rplanet = sqrt(Xplanet * Xplanet + Yplanet * Yplanet);
			RRoche =
			    pow((1.0 / 3.0 * Mplanet), (1.0 / 3.0)) * Rplanet;
			...
			j_min = (int)((double)ns / 2.0 / PI *
				      (angle - 2.0 * RRoche / Rplanet));
			j_max = (int)((double)ns / 2.0 / PI *
				      (angle + 2.0 * RRoche / Rplanet));
			...
			for (i = i_min; i <= i_max; i++) {
				for (j = j_min; j <= j_max; j++) {
					...
					vtcell =
					    0.5 * (vtheta[l] + vtheta[ljp]) +
					    Rmed[i] * OmegaFrame;
					vrcell = 0.5 * (vrad[l] + vrad[lip]);
					vxcell = (vrcell * xc - vtcell * yc) /
						 Rmed[i];
					vycell = (vrcell * yc + vtcell * xc) /
						 Rmed[i];
			...
			PyPlanet += dPyPlanet;
			Mplanet += dMplanet;
			if (data.get_planetary_system()
				.get_planet(k)
				.get_feeldisk()) {
				data.get_planetary_system()
				    .get_planet(k)
				    .set_vx(PxPlanet / Mplanet);
				data.get_planetary_system()
				    .get_planet(k)
				    .set_vy(PyPlanet / Mplanet);
			}
			data.get_planetary_system().get_planet(k).set_mass(
			    Mplanet);
		}
	}
}

void FindOrbitalElements(double x, double y, double vx, double vy, double m,
			 int n)
{
	// m is mstar + mplanet
