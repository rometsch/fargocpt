...
void ComputeForce(t_data &data, Force *force, double x, double y, double mass)
{
	...
	// consider calculation of force on primary, don't need to recalculate
	// rsmoothing then
	...
	for (unsigned int n_radial = Zero_or_active; n_radial < Max_or_active;
		...
		for (unsigned int n_azimuthal = 0;
			...
			if (Rmed[n_radial] < a) {
				...
				fxhi += constants::G * cellmass * dx *
					InvDist3 * hill_cut;
				fyhi += constants::G * cellmass * dy *
					InvDist3 * hill_cut;
			} else {
				fxo += constants::G * cellmass * dx * InvDist3;
				fyo += constants::G * cellmass * dy * InvDist3;
				fxho += constants::G * cellmass * dx *
					InvDist3 * hill_cut;
				fyho += constants::G * cellmass * dy *
					InvDist3 * hill_cut;
			}
		}
	}
	...
	for (unsigned int i = 0;
		...
		if (CPU_Rank == CPU_Number - 1) {
			...
			fprintf(out,
				...
				vx * fc->fx_ex_outer + vy * fc->fy_ex_outer,
				time);
			fclose(out);
		}
	}
