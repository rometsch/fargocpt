...
void usage(int argc, char **argv)
{
	...
	logging::print_master(
	    ...
	    "-d | --debug        Print some debugging information on 'stdout' "
	    "at each timestep\n"
	    "-r | --restart <n>  Restart calculation at timestep <n>\n"
	    "-v | --verbose      Verbose mode. Tells everything about "
	    "parameters file\n"
	    "-q | --quiet        Only print errors and warnings\n"
	    "-b |                Adjust azimuthal velocity to impose strict "
	    "centrifugal balance at t=0\n"
	    "-c |                Sloppy CFL condition (checked at each DT, not "
	    "at each timestep)\n"
	    "-e |                Activate EU test problem torque file output\n"
	    "-n |                Disable simulation. The program just reads "
	    "parameters file\n"
...
void parse(int argc, char **argv)
{
	for (;;) {
		...
		case 'd':
#ifdef NDEBUG
			die("option -d for debugging is only valid if not "
		...
		case 'r':
			...
			logging::print_master(
			    LOG_INFO "Restarting simulation at timestep %u\n",
			    restart_from);
			break;
		case 'v':
