...
static void correct_for_self_gravity(t_data &data, const unsigned int i)
{
    ...
    double v = sqrt(constants::G * (M + particles[i].mass) / semi_major_axis -
...
void restart(unsigned int timestep)
{
    ...
    fd = fopen(filename, "r");
    if (fd == nullptr) {
	logging::print_master(LOG_INFO "Can't find file particles%d.dat. Using "
				       "generated particles.\n",
			      timestep);
	return;
    }
	...
	logging::print_master(LOG_ERROR
			      "Error while reading to file '%s'. Check file "
    ...
    const double phi, const double phi_dot, t_data &data)
{
	...
	aphi += constants::G * planet.get_mass() / (pow2(r_planet) * r) *
		sin_delta_phi;
    }
}
						 ...
						 const double phi, t_data &data)
{
	...
	minus_grav_l_dot +=
	    constants::G * planet_mass * r / (pow2(r_planet)) * sin_delta_phi;
    }
}
...
void check_tstop(t_data &data)
{
	...
	if (tstop < 10.0 * dt && parameters::particle_gas_drag_enabled &&
	    ...
	    logging::print_master(
		LOG_ERROR "Particle stopping time too small for explicit "
	...
	if (tstop < 0.005 * dt && parameters::particle_gas_drag_enabled &&
	    ...
	    logging::print_master(
		LOG_ERROR "Particle stopping time too small for semiimplicit "
...
void write(unsigned int timestep)
{
    ...
    if (error != MPI_SUCCESS) {
	logging::print_master(LOG_ERROR
			      "Error while writing to file '%s'. Check file "
			      "permissions and IO support of MPI library\n",
			      filename);
