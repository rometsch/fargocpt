...
static double f(double x, double n)
{
	...
	if (n == -1) {
		...
		return exp(log(x * (-pow(rmin, n + 1) + pow(rmax, n + 1)) +
			       pow(rmin, n + 1)) /
			   (n + 1));
	}
}
				      ...
				      const double r, double phi)
{
	...
	if (n_azimuthal_minus == last_index) {
		if (phi < PI) {
			...
			// previous cell is at N_azimuthal_max - 1, but measure
			// distance from -0.5*dphi
			phim = -0.5 * dphi;
			phip = 0.5 * dphi;
		} else // phi > PI
		{
			// particle is inside cell with n_azimuthal =
			// N_azimuthal_max - 1 next cell is at N_azimuthal = 0,
			// but measure distance from 2*PI+0.5*dphi
	...
	double Qm =
	    ((rm * phip - rm * phi) * Qmm + (rm * phi - rm * phim) * Qmp) /
	    (rm * phip - rm * phim);
	double Qp =
	    ((rp * phip - rp * phi) * Qpm + (rp * phi - rp * phim) * Qpp) /
	    (rp * phip - rp * phim);
...
static double get_particle_eccentricity(int particle_index)
{
	...
	const double reduced_mass = M*particles[i].mass / (M +
particles[i].mass); const double A_r = reduced_mass * pow3(particles[i].r) *
...
static void init_particle_timestep(t_data &data)
{
	...
	for (unsigned int i = 0; i < local_number_of_particles; ++i) {
		...
		// cartesian coordinates are written inside the polar
		// coordinates
		...
		temp_r_dot =
		    particles[i].r_dot + particles[i].timestep * k1_r_dot;
		temp_phi_dot =
		...
		// step size is computed such that h**iord *
		// max_d(norm(f0),norm(der2)) = 0.01
...
static void correct_for_self_gravity(t_data &data, const unsigned int i)
{
	...
	const double eccentricity =
	    particles[i]
		.phi_ddot; // reused variable for storage, will be initialized
			   // correctly in this function to 0.0
	...
	if (parameters::particle_disk_gravity_enabled) {
		...
		find_nearest(data[t_data::DENSITY], n_radial_a_minus,
			     n_radial_a_plus, n_azimuthal_a_minus,
			     n_azimuthal_a_plus, n_radial_b_minus,
			     n_radial_b_plus, n_azimuthal_b_minus,
			     n_azimuthal_b_plus, r, phi);
	...
	double v =
	    sqrt(constants::G * (M + particles[i].mass) / semi_major_axis -
	...
	if (CartesianParticles) {
		// Beware: cartesian particles still use the names of polar
		// coordinates vx = r_dot vy = phi_dot
	      ...
	      std::uniform_real_distribution<double> &dis_eccentricity)
{
	double semi_major_axis =
	    f(dis_one(generator), parameters::particle_slope);
	...
	phi = 2.0*PI / num_particles_per_ring *
	(global_id/num_particles_per_ring); semi_major_axis = (rmax - rmin) *
	...
	*/
	...
	if (CartesianParticles) {
		// Beware: cartesian particles still use the names of polar
		// coordinates x = r y = phi vx = r_dot vy = phi_dot
...
void init(t_data &data)
{
	...
	// get number of local particles from all nodes to compute correct
	// offsets
	unsigned int nodes_number_of_particles[CPU_Number];
	MPI_Allgather(&local_number_of_particles, 1, MPI_UNSIGNED,
		      nodes_number_of_particles, 1, MPI_UNSIGNED,
		      MPI_COMM_WORLD);
	...
	for (unsigned int i = 0; i < local_offset; ++i) {
		init_particle(0, local_offset, generator, dis_one, dis_twoPi,
			      dis_eccentricity); // bring random generator to
						 // correct position
	}
	...
	MPI_Datatype mpi_particle_types[11] = {
	...
	// particles might be created on the wrong node, so move them to the
	// correct one :)
...
void restart(unsigned int timestep)
{
	...
	logging::print_master(
	    LOG_WARNING
	    "Beware: when restarting particles, the user is responsible that "
	    "the loaded particle file is written with the same coordinate "
	    "system as the simulation is running on!\n\n");
	...
	if (fd == nullptr) {
		logging::print_master(LOG_INFO
				      "Can't find file particles%d.dat. Using "
				      "generated particles.\n",
				      timestep);
		return;
	}
	...
	if (num_particles_in_file < parameters::number_of_particles) {
		logging::print_master(
		    LOG_ERROR "Warning: Simulation runs with %d particles but "
			      "only %d particles can be loaded from file!\n\n",
		    parameters::number_of_particles, num_particles_in_file);
		global_number_of_particles = num_particles_in_file;
		parameters::number_of_particles = num_particles_in_file;
		local_number_of_particles =
		    global_number_of_particles / CPU_Number;
		free(particles);
		...
		local_r_min = Ra[CPU_Rank == 0 ? GHOSTCELLS_A - 1 : CPUOVERLAP];
		local_r_max =
		    Ra[CPU_Rank == CPU_Highest ? NRadial - GHOSTCELLS_A + 1
					       : NRadial - CPUOVERLAP];

		// create storage
		particles_size = local_number_of_particles;
		particles =
		    (t_particle *)malloc(sizeof(t_particle) * particles_size);
	}
	if (num_particles_in_file > parameters::number_of_particles) {
		logging::print_master(
		    LOG_WARNING
		    "Warning: particle File contains %d particles but only %d "
		    "particles can be loaded into Simulation!\n\n",
	...
	if (error != MPI_SUCCESS) {
		logging::print_master(
		    LOG_ERROR "Error while reading to file '%s'. Check file "
	...
	MPI_File_read_all(fh, particles, local_number_of_particles,
			  mpi_particle, &status);

	// close file
	MPI_File_close(&fh);

	// particles might be loaded to the wrong node, so move them to the
	// correct one
	for (int i = 0; i < CPU_Number; ++i)
		move();
}
    ...
    const double phi, const double phi_dot, t_data &data)
{
	...
	for (unsigned int k = 0;
		...
		const t_planet &planet =
		    data.get_planetary_system().get_planet(k);
		...
		const double distance_to_planet =
		    sqrt(r * r + r_planet * r_planet -
			 2 * r * r_planet * cos_delta_phi);
		...
		// indirect term
		ar -= constants::G * planet.get_mass() / pow2(r_planet) *
		      cos_delta_phi;
		aphi += constants::G * planet.get_mass() /
			(pow2(r_planet) * r) * sin_delta_phi;
	}
}
							...
							t_data &data)
{
	...
	for (unsigned int k = 0;
		...
		r2 += epsilon_sq;
		const double factor =
		    constants::G * planet.get_mass() / (r * r2);
		...
		const double factor2 =
		    -constants::G * planet.get_mass() / dist3;
						 ...
						 const double phi, t_data &data)
{
	...
	for (unsigned int k = 0;
		...
		const double distance_to_planet =
		    sqrt(r * r + r_planet * r_planet -
			 2.0 * r * r_planet * cos_delta_phi);
		const double distance_to_planet_smoothed_pow2 =
		    distance_to_planet * distance_to_planet + epsilon_sq;

		// direct term
		grav_r_ddot -=
		    constants::G * planet_mass *
		...
		// indirect term
		grav_r_ddot -=
		    constants::G * planet_mass / pow2(r_planet) * cos_delta_phi;
		minus_grav_l_dot += constants::G * planet_mass * r /
				    (pow2(r_planet)) * sin_delta_phi;
	}
}
    ...
    const double phi, t_data &data)
{
	...
	for (unsigned int k = 0;
		...
		// direct term
		acart[0] +=
		    -constants::G * planet_mass * x_dist / (dist * dist2);
		acart[1] +=
		    -constants::G * planet_mass * y_dist / (dist * dist2);

		// indirect term
		const double r_planet = planet.get_r();
		acart[0] += -constants::G * planet_mass * x_planet /
			    (r_planet * r_planet * r_planet);
		acart[1] += -constants::G * planet_mass * y_planet /
			    (r_planet * r_planet * r_planet);
	}
		     ...
		     unsigned int n_azimuthal_plus, double r, double phi)
{
	...
	if (azimuthal_a_grid) {
		...
		// next cells is at n_azimuthal = 0, but measure distance from
		// 2*PI instead
		...
		if (n_azimuthal_minus == last_index) {
			if (phi < PI) {
				// particle is inside cell with n_azimuthal = 0
				// previous cell is at N_azimuthal_max - 1, but
				// measure distance from -0.5*dphi
				phim = -0.5 * dphi;
				phip = 0.5 * dphi;
			} else {
				// particle is inside cell with n_azimuthal =
				// N_azimuthal_max - 1 next cell is at
				// N_azimuthal = 0, but measure distance from
				// 2*PI+0.5*dphi
				phim = (n_azimuthal_minus + 0.5) * dphi;
				phip = (n_azimuthal_minus + 1.5) * dphi;
			}
	...
	// double Qm =
	// ((rm*phip-rm*phi)*Qmm+(rm*phi-rm*phim)*Qmp)/(rm*phip-rm*phim); double
	// Qp = ((rp*phip-rp*phi)*Qpm+(rp*phi-rp*phim)*Qpp)/(rp*phip-rp*phim);
			     ...
			     double &tstop, const double r0, const double l0)
{
	...
	// particle is still inside the domain the gas values are determined at
	// the edge of the domain if particle is outside the domain
	...
	const double temperature = interpolate_bilinear(
	    data[t_data::TEMPERATURE], false, false, n_radial_b_minus,
	    n_radial_b_plus, n_azimuthal_b_minus, n_azimuthal_b_plus, r_tmp,
	    phi);
	const double vg_radial = interpolate_bilinear(
	    data[t_data::V_RADIAL], true, false, n_radial_a_minus,
	    n_radial_a_plus, n_azimuthal_b_minus, n_azimuthal_b_plus, r_tmp,
	    phi);
	const double vg_azimuthal = interpolate_bilinear(
	    data[t_data::V_AZIMUTHAL], false, true, n_radial_b_minus,
	    n_radial_b_plus, n_azimuthal_a_minus, n_azimuthal_a_plus, r_tmp,
	    phi);
	const double m0 = parameters::MU * constants::m_u.get_code_value();
	const double vthermal = sqrt(8.0 * constants::k_B.get_code_value() *
				     temperature / (PI * m0));
...
void check_tstop(t_data &data)
{
	...
	CommunicateBoundaries(&data[t_data::DENSITY], &data[t_data::V_RADIAL],
			      ...
			      &data[t_data::ENERGY]);
	local_gas_time_step_cfl = condition_cfl(data, data[t_data::V_RADIAL],
						data[t_data::V_AZIMUTHAL],
	...
	for (unsigned int i = 0; i < local_number_of_particles; ++i) {
		...
		// calculate gas quantities at the particle location
		const double rho = interpolate_bilinear(
		    data[t_data::RHO], false, false, n_radial_b_minus,
		...
		const double vg_radial = interpolate_bilinear(
		    data[t_data::V_RADIAL], true, false, n_radial_a_minus,
		    n_radial_a_plus, n_azimuthal_b_minus, n_azimuthal_b_plus, r,
		    phi);
		const double vg_azimuthal = interpolate_bilinear(
		    data[t_data::V_AZIMUTHAL], false, true, n_radial_b_minus,
		    n_radial_b_plus, n_azimuthal_a_minus, n_azimuthal_a_plus, r,
		    phi);
		const double m0 =
		    parameters::MU * constants::m_u.get_code_value();
		const double vthermal =
		    sqrt(8.0 * constants::k_B.get_code_value() * temperature /
			 (PI * m0));
		...
		const double vrel =
		    sqrt(pow2(minus_r_dotel_r) + pow2(vrel_phi));
		...
		// mean free path for molecular hydrogen (see Haghighipour &
		// Boss, 2003 eq. 20)
		double l = 4.72e-9 / rho;
		double f = radius / (radius + l);

		// ***************************************************************************
		// Combined Epstein + Stokes drag regimes (see Haghighipour &
		// Boss, 2003 eq. 8)
		double term =
		    rho * ((1.0 - f) * vthermal + 3.0 / 8.0 * f * Cd * vrel);
		// ***************************************************************************
		// Epstein only drag regime
		// double term = rho*vthermal;
		// ***************************************************************************
		...
		if (tstop < 10.0 * dt &&
		    parameters::particle_gas_drag_enabled &&
		    (parameters::integrator ==
			 parameters::integrator_explicit ||
		     parameters::integrator ==
			 parameters::integrator_adaptive)) {
			logging::print_master(
			    LOG_ERROR
			    "Particle stopping time too small for explicit "
			    "integrator! Use the semiimplicit or implicit "
			    "integrator instead!");
			PersonalExit(1);
		}
		...
		if (tstop < 0.005 * dt &&
		    parameters::particle_gas_drag_enabled &&
		    parameters::integrator ==
			parameters::integrator_semiimplicit) {
			logging::print_master(
			    LOG_ERROR
			    "Particle stopping time too small for semiimplicit "
			    "integrator! Use the implicit integrator instead!");
			PersonalExit(1);
		}
	}
}
...
void update_velocities_from_indirect_term(const double dt)
{
	...
	for (unsigned int i = 0; i < local_number_of_particles; ++i) {
		...
		if (CartesianParticles) {
			indirect_q1_dot =
			    fma(IndirectTerm.x, dt, particles[i].r_dot);
			indirect_q2_dot =
			    fma(IndirectTerm.y, dt, particles[i].phi_dot);
		} else {
			...
			indirect_q1_dot =
			    r_dot + dt * (IndirectTerm.x * cos(phi) +
					  IndirectTerm.y * sin(phi));
			indirect_q2_dot =
			    phi_dot + dt *
					  (-IndirectTerm.x * sin(phi) +
					   IndirectTerm.y * cos(phi)) /
					  r;
		}
...
void update_velocities_from_gas_drag_cart(t_data &data, double dt)
{
	...
	for (unsigned int i = 0; i < local_number_of_particles; ++i) {
		...
		double vg_x = cos(phi) * vg_radial -
			      sin(phi) * (vg_azimuthal + OmegaFrame * r);
		double vg_y = sin(phi) * vg_radial +
			      cos(phi) * (vg_azimuthal + OmegaFrame * r);
		...
		if (parameters::particle_disk_gravity_enabled) {
			update_velocity_from_disk_gravity_cart(
			    n_radial_a_minus, n_radial_a_plus,
			    n_azimuthal_b_minus, n_azimuthal_b_plus, r, phi, i,
			    dt);
		}
	}
...
void update_velocities_from_gas_drag(t_data &data, double dt)
{
	...
	for (unsigned int i = 0; i < local_number_of_particles; ++i) {
		...
		const double reynolds =
		    2.0 * rho * particles[i].radius * vrel / nu;
		...
		const double fdrag_temp = -0.5 * Cd * PI *
					  pow2(particles[i].radius) * rho *
...
void update_velocity_from_disk_gravity_cart_old(t_data &data, double dt)
{
	...
	t_particle *all_particles = (t_particle *)malloc(
	    sizeof(t_particle) * global_number_of_particles);
	double *force_x =
	...
	MPI_Allgather(&local_number_of_particles, 1, MPI_INT,
		      number_of_particles, 1, MPI_INT, MPI_COMM_WORLD);
	...
	double dphi =
	    2.0 * PI / (double)data[t_data::DENSITY].get_size_azimuthal();
	for (unsigned int n_radial = Zero_or_active; n_radial < Max_or_active;
		...
		for (unsigned int n_azimuthal = 0;
			...
			for (unsigned int i = 0; i < global_number_of_particles;
			     ++i) {
				...
				double d_y = cell_y - y;
				double invdist3 =
				    pow(pow2(d_x) + pow2(d_y) + pow2(smoothing),
					-3.0 / 2.0);

				force_x[i] +=
				    constants::G * cell_mass * d_x * invdist3;
				force_y[i] +=
				    constants::G * cell_mass * d_y * invdist3;
			}
		}
	}

	// send force back
	MPI_Allreduce(MPI_IN_PLACE, force_x, global_number_of_particles,
		      MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	MPI_Allreduce(MPI_IN_PLACE, force_y, global_number_of_particles,
		      MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
...
void integrate_semiimplicit(t_data &data, const double dt)
{
	...
	// Semi implicit integrator in cylindrical coordinates (see Zhu et al.
	// 2014, eqs. A4-A12)
	...
	for (unsigned int i = 0; i < local_number_of_particles; ++i) {
		...
		double phi1 =
		    phi0 + 0.5 * (l0 / pow2(r0) + l0 / pow2(r1)) * hfdt;
		check_angle(phi1);
		...
		if (parameters::particle_gas_drag_enabled) {
			calculate_tstop2(r1, phi1, r_dot1, phi_dot1, data,
					 particles[i].radius, vrel_r,
					 minus_l_rel, tstop, r0, l0);
		...
		const double phi3 =
		    phi2 + 0.5 * (l2 / pow2(r2) + l2 / pow2(r3)) * hfdt;

		particles[i].r_dot = r_dot2;
		particles[i].r = r3;
		// particles[i].phi  = phi1 + 0.5*(phi_dot0 +
		// l2/pow2(particles[i].r)) * hfdt; particles[i].phi  = phi1 +
...
void integrate_implicit(t_data &data, const double dt)
{
	...
	double r_ddot0, minus_l_dot0, r_ddot1, minus_l_dot1, hfdt,
	    minus_r_dot_rel0, minus_l_rel0, minus_r_dot_rel1, minus_l_rel1;
	...
	for (unsigned int i = 0; i < local_number_of_particles; ++i) {
		...
		if (parameters::particle_gas_drag_enabled) {
			calculate_tstop(r0, phi0, r_dot0, phi_dot0, data,
					particles[i].radius, minus_r_dot_rel0,
					minus_l_rel0, tstop0);
			dt0 = 1.0 + dt / tstop0;
		}
		...
		if (parameters::particle_gas_drag_enabled) {
			l1 += (minus_l_rel0 / tstop0 +
			       minus_l_rel1 / tstop1 * dt0) *
			      dt1;
		}
		...
		r_dot1 +=
		    (pow2(l0) / pow3(r0) + pow2(l1) / pow3(r1) * dt0) * dt1;
		if (parameters::particle_gas_drag_enabled) {
			r_dot1 += (minus_r_dot_rel0 / tstop0 +
				   minus_r_dot_rel1 / tstop1 * dt0) *
				  dt1;
		}
		...
		const double phi2 =
		    phi0 + (l0 / pow2(r0) + l1 / pow2(r2)) * hfdt;
...
void integrate_explicit_adaptive(t_data &data, const double dt)
{
	...
	// as particles move independent of each other, we can integrate one
	// after one
	for (unsigned int i = 0; i < local_number_of_particles; ++i) {
		...
		while (!last) {
			...
			if (CartesianParticles) {
				...
				calculate_accelerations_from_star_and_planets(
				    temp_ar, temp_aphi, temp_r, temp_r_dot,
				    temp_phi, temp_phi_dot, data);
			}
			...
			temp_r_dot =
			    particles[i].r_dot + timestep * 0.2 * k1_r_dot;
			temp_phi_dot =
			    particles[i].phi_dot + timestep * 0.2 * k1_phi_dot;
			...
			temp_r = particles[i].r +
				 timestep * (0.075 * k1_r + 0.225 * k2_r);
			temp_phi = particles[i].phi +
				   timestep * (0.075 * k1_phi + 0.225 * k2_phi);

			temp_r_dot =
			    particles[i].r_dot +
			    timestep * (0.075 * k1_r_dot + 0.225 * k2_r_dot);
			temp_phi_dot = particles[i].phi_dot +
				       timestep * (0.075 * k1_phi_dot +
						   0.225 * k2_phi_dot);
			...
			temp_phi = particles[i].phi +
				   timestep * (0.3 * k1_phi - 0.9 * k2_phi +
					       1.2 * k3_phi);

			temp_r_dot =
			    particles[i].r_dot +
			    timestep * (0.3 * k1_r_dot - 0.9 * k2_r_dot +
					1.2 * k3_r_dot);
			temp_phi_dot =
			    particles[i].phi_dot +
			...
			temp_r = particles[i].r +
				 timestep *
				     (-11.0 / 54.0 * k1_r + 2.5 * k2_r -
				      70.0 / 27.0 * k3_r + 35.0 / 27.0 * k4_r);
			temp_phi =
			    particles[i].phi +
			    timestep *
				(-11.0 / 54.0 * k1_phi + 2.5 * k2_phi -
				 70.0 / 27.0 * k3_phi + 35.0 / 27.0 * k4_phi);

			temp_r_dot = particles[i].r_dot +
				     timestep * (-11.0 / 54.0 * k1_r_dot +
						 2.5 * k2_r_dot -
						 70.0 / 27.0 * k3_r_dot +
						 35.0 / 27.0 * k4_r_dot);
			temp_phi_dot = particles[i].phi_dot +
				       timestep * (-11.0 / 54.0 * k1_phi_dot +
						   2.5 * k2_phi_dot -
						   70.0 / 27.0 * k3_phi_dot +
						   35.0 / 27.0 * k4_phi_dot);
			...
			temp_r = particles[i].r +
				 timestep * (1631.0 / 55296.0 * k1_r +
					     175.0 / 512.0 * k2_r +
					     575.0 / 13824.0 * k3_r +
					     44275.0 / 110592.0 * k4_r +
					     253.0 / 4096.0 * k5_r);
			temp_phi = particles[i].phi +
				   timestep * (1631.0 / 55296.0 * k1_phi +
					       175.0 / 512.0 * k2_phi +
					       575.0 / 13824.0 * k3_phi +
					       44275.0 / 110592.0 * k4_phi +
					       253.0 / 4096.0 * k5_phi);

			temp_r_dot = particles[i].r_dot +
				     timestep * (1631.0 / 55296.0 * k1_r_dot +
						 175.0 / 512.0 * k2_r_dot +
						 575.0 / 13824.0 * k3_r_dot +
						 44275.0 / 110592.0 * k4_r_dot +
			...
			const double r_new =
			    particles[i].r +
			    timestep *
				(37.0 / 378.0 * k1_r + 250.0 / 621.0 * k3_r +
				 125.0 / 594.0 * k4_r + 512.0 / 1771.0 * k6_r);
			double phi_new = particles[i].phi +
					 timestep * (37.0 / 378.0 * k1_phi +
						     250.0 / 621.0 * k3_phi +
						     125.0 / 594.0 * k4_phi +
						     512.0 / 1771.0 * k6_phi);
			...
			const double r_ddot_new = (37.0 / 378.0 * k1_r_dot +
						   250.0 / 621.0 * k3_r_dot +
						   125.0 / 594.0 * k4_r_dot +
						   512.0 / 1771.0 * k6_r_dot);
			const double phi_ddot_new =
			    (37.0 / 378.0 * k1_phi_dot +
			     250.0 / 621.0 * k3_phi_dot +
			     125.0 / 594.0 * k4_phi_dot +
			     512.0 / 1771.0 * k6_phi_dot);

			const double r_dot_new =
			    particles[i].r_dot + timestep * r_ddot_new;
			const double phi_dot_new =
			    particles[i].phi_dot + timestep * phi_ddot_new;

			// error estimates, reuse k2 variables
			k2_r = timestep * (e1 * k1_r + e3 * k3_r + e4 * k4_r +
					   e5 * k5_r + e6 * k6_r);
			k2_phi = timestep *
				 (e1 * k1_phi + e3 * k3_phi + e4 * k4_phi +
				  e5 * k5_phi + e6 * k6_phi);
			k2_r_dot = timestep * (e1 * k1_r_dot + e3 * k3_r_dot +
					       e4 * k4_r_dot + e5 * k5_r_dot +
					       e6 * k6_r_dot);
			k2_phi_dot =
			    timestep * (e1 * k1_phi_dot + e3 * k3_phi_dot +
					e4 * k4_phi_dot + e5 * k5_phi_dot +
					e6 * k6_phi_dot);
			...
			sk = atoli +
			     rtoli * fmax(fabs(particles[i].r), fabs(r_new));
			sqr = k2_r / sk;
			err += sqr * sqr;

			sk = atoli + rtoli * fmax(fabs(particles[i].phi),
						  fabs(phi_new));
			sqr = k2_phi / sk;
			err += sqr * sqr;

			sk = atoli + rtoli * fmax(fabs(particles[i].r_dot),
						  fabs(r_dot_new));
			sqr = k2_r_dot / sk;
			err += sqr * sqr;

			sk = atoli + rtoli * fmax(fabs(particles[i].phi_dot),
						  fabs(phi_dot_new));
			sqr = k2_phi_dot / sk;
			err += sqr * sqr;
			...
			// if timestep is reduced to reach full dt, don't make
			// it larger
			if (!update_timestep)
				fac = fmax(fac, 1.0);
			...
			if (err <= 1.0) {
				...
				if (reject) // last try was rejected
				{
					particles[i].timestep =
					    fmin(fabs(particles[i].timestep),
						 fabs(old_timestep));
				}
				reject = false;
...
void integrate_explicit(t_data &data, const double dt)
{
	...
	for (unsigned int i = 0; i < local_number_of_particles; ++i) {
		...
		temp_phi =
		    particles[i].phi + dt * (0.075 * k1_phi + 0.225 * k2_phi);

		temp_r_dot = particles[i].r_dot +
			     dt * (0.075 * k1_r_dot + 0.225 * k2_r_dot);
		temp_phi_dot = particles[i].phi_dot +
		...
		temp_phi_dot = particles[i].phi_dot +
			       dt * (0.3 * k1_phi_dot - 0.9 * k2_phi_dot +
				     1.2 * k3_phi_dot);
		...
		temp_r_dot =
		    particles[i].r_dot + dt * (1631.0 / 55296.0 * k1_r_dot +
					       175.0 / 512.0 * k2_r_dot +
					       575.0 / 13824.0 * k3_r_dot +
					       44275.0 / 110592.0 * k4_r_dot +
					       253.0 / 4096.0 * k5_r_dot);
		temp_phi_dot = particles[i].phi_dot +
			       dt * (1631.0 / 55296.0 * k1_phi_dot +
				     175.0 / 512.0 * k2_phi_dot +
				     575.0 / 13824.0 * k3_phi_dot +
				     44275.0 / 110592.0 * k4_phi_dot +
				     253.0 / 4096.0 * k5_phi_dot);
		...
		// update position & velocity
		particles[i].r +=
		    dt * (37.0 / 378.0 * k1_r + 250.0 / 621.0 * k3_r +
			  125.0 / 594.0 * k4_r + 512.0 / 1771.0 * k6_r);
		particles[i].phi +=
...
void move(void)
{
	...
	if (CPU_Rank < CPU_Highest) {
		...
		if (particles_size < local_number_of_particles + number) {
			...
			particles = (t_particle *)realloc(
			    particles, sizeof(t_particle) * particles_size);
		}

		MPI_Recv(particles + local_number_of_particles, number,
			 mpi_particle, CPU_Next, 0, MPI_COMM_WORLD, &status);
	...
	if (CPU_Rank > 0) {
		...
		MPI_Type_indexed(inward_count, inward_size, inward_offset,
				 mpi_particle, &inward_type);
		MPI_Type_commit(&inward_type);
		MPI_Send(particles, 1, inward_type, CPU_Prev, 0,
			 MPI_COMM_WORLD);
	...
	for (int i = 0; i < inward_count; ++i) {
		...
		particles[inward_offset[i]] =
		    particles[local_number_of_particles - 1];

		// check if the particle we switched is also in inward_offset
		for (int j = i + 1; j < inward_count; ++j) {
			if (inward_offset[j] ==
			    (int)local_number_of_particles - 1)
	...
	if (CPU_Rank > 0) {
		...
		MPI_Recv(particles + local_number_of_particles, number,
			 mpi_particle, CPU_Prev, 0, MPI_COMM_WORLD, &status);
	...
	if (CPU_Rank < CPU_Highest) {
		...
		MPI_Send(particles, 1, outward_type, CPU_Next, 0,
			 MPI_COMM_WORLD);
	...
	MPI_Allreduce(&local_number_of_particles, &global_number_of_particles,
...
void write(unsigned int timestep)
{
	...
	if (asprintf(&filename, "%s/particles%i.dat", OUTPUTDIR, timestep) <
	    0) {
		die("Not enough memory!");
	}

	// try to open file
	error = MPI_File_open(MPI_COMM_WORLD, filename,
