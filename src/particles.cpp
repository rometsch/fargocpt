...
static void init_particle_timestep(t_data &data)
{
	...
	for (unsigned int i = 0; i < local_number_of_particles; ++i) {
		...
		// cartesian coordinates are written inside the polar coordinates
		if(CartesianParticles)
		{
			calculate_accelerations_from_star_and_planets_cart(temp_ar, temp_aphi, temp_r, temp_phi, data);
		}
		else
		{
			calculate_accelerations_from_star_and_planets(temp_ar, temp_aphi, temp_r, temp_r_dot, temp_phi, temp_phi_dot, data);
		}
		...
		temp_phi_dot = particles[i].phi_dot + particles[i].timestep * k1_phi_dot;
...
void init(t_data &data) {
	...
	MPI_Datatype mpi_particle_types[11] = {MPI_UNSIGNED, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE};
...
void restart(unsigned int timestep) {
  ...
  logging::print_master(LOG_WARNING "Beware: when restarting particles, the user is responsible that the loaded particle file is written with the same coordinate system as the simulation is running on!\n\n");
  ...
  {
	  logging::print_master(LOG_ERROR "Warning: Simulation runs with %d particles but only %d particles can be loaded from file!\n\n", parameters::number_of_particles, num_particles_in_file);
  ...
  if(num_particles_in_file > parameters::number_of_particles)
  {
	  logging::print_master(LOG_WARNING "Warning: particle File contains %d particles but only %d particles can be loaded into Simulation!\n\n", num_particles_in_file, parameters::number_of_particles);
  }
  ...
  if (error != MPI_SUCCESS) {
	  logging::print_master(LOG_ERROR "Error while reading to file '%s'. Check file permissions and IO support of MPI library\n", filename);
  ...
  MPI_File_read_all(fh, particles, local_number_of_particles, mpi_particle, &status);
...
static void calculate_tstop(const double r, const double phi, const double r_dot, const double phi_dot,
  ...
  // mean free path for molecular hydrogen (see Haghighipour & Boss, 2003 eq. 20)
  double l = 4.72e-9/rho;
  double f = radius/(radius+l);

  // ***************************************************************************
  // Combined Epstein + Stokes drag regimes (see Haghighipour & Boss, 2003 eq. 8)
...
static void calculate_tstop2(const double r, const double phi, const double r_dot, const double phi_dot,
					 t_data &data, const double radius, double &minus_r_dotel_r, double &minus_l_rel,
...
void check_tstop(t_data &data)
{
	...
	for (unsigned int i = 0; i < local_number_of_particles; ++i)
	{
		...
		if(tstop < 10.0*dt && parameters::particle_gas_drag_enabled && (parameters::integrator == parameters::integrator_explicit || parameters::integrator == parameters::integrator_adaptive))
		{
			logging::print_master(LOG_ERROR "Particle stopping time too small for explicit integrator! Use the semiimplicit or implicit integrator instead!");
			PersonalExit(1);
		}
...
void update_velocities_from_gas_drag(t_data &data, double dt) {
	...
	for (unsigned int i = 0; i < local_number_of_particles; ++i) {
		...
		const double vrel_phi = r*(particles[i].phi_dot - OmegaFrame) - vg_azimuthal;
		const double vrel = sqrt(pow2(vrel_r)+pow2(vrel_phi));

		const double m0 = parameters::MU * constants::m_u.get_code_value();
		const double vthermal = sqrt(8.0*constants::k_B.get_code_value()*temperature/(PI*m0));
		// a0 = 1.5e-8 cm for molecular hydrogen
		const double sigma = (PI*pow2(1.5e-8/units::length.get_cgs_factor()));
		...
		const double fdrag_temp = -0.5*Cd*PI*pow2(particles[i].radius)*rho*vrel/particles[i].mass;
	...
	for (unsigned int n_radial = Zero_or_active; n_radial < Max_or_active; ++n_radial) {
		for (unsigned int n_azimuthal = 0; n_azimuthal <= data[t_data::DENSITY].get_max_azimuthal(); ++n_azimuthal) {
			...
			for (unsigned int i = 0; i < global_number_of_particles; ++i) {
				...
				double smoothing = compute_smoothing_isothermal(all_particles[i].get_distance_to_star());
				double d_x = cell_x - x;
				double d_y = cell_y - y;
...
void integrate_semiimplicit(t_data &data, const double dt)
{
	...
	for (unsigned int i = 0; i < local_number_of_particles; ++i) {
		...
		if (parameters::particle_gas_drag_enabled)
		{
			calculate_tstop2(r1, phi1, r_dot1, phi_dot1, data, particles[i].radius, vrel_r, minus_l_rel, tstop, r0, l0);
		...
		particles[i].r = r3;
		//particles[i].phi  = phi1 + 0.5*(phi_dot0 + l2/pow2(particles[i].r)) * hfdt;
		//particles[i].phi  = phi1 + 0.5*(l1/pow2(r1)+l2/pow2(particles[i].r)) * hfdt;
...
void integrate_implicit(t_data &data, const double dt)
{
  ...
  // initialize with failsave values to suppress compiler warning  "-Wmaybe-uninitialized"
  minus_l_rel0 = 0.0;
  ...
  if (parameters::particle_gas_drag_enabled)
  {
	...
	for (unsigned int i = 0; i < local_number_of_particles; ++i) {
		...
		if (parameters::particle_gas_drag_enabled)
		{
		  calculate_tstop(r0, phi0, r_dot0, phi_dot0, data, particles[i].radius, minus_r_dot_rel0, minus_l_rel0, tstop0);
		  dt0 = 1.0+dt/tstop0;
		}
		...
		if (parameters::particle_gas_drag_enabled)
		{
			calculate_tstop2(r1, phi1, r_dot0, phi_dot0, data, particles[i].radius, minus_r_dot_rel1, minus_l_rel1, tstop1, r0, l0);
		...
		if (parameters::particle_gas_drag_enabled)
		{
			r_dot1 += (minus_r_dot_rel0/tstop0+minus_r_dot_rel1/tstop1*dt0)*dt1;
		}
...
void integrate_explicit_adaptive(t_data &data, const double dt)
{
	...
	for (unsigned int i = 0; i < local_number_of_particles; ++i)
	{
		...
		while(!last)
		{
			...
			temp_phi = particles[i].phi + timestep * (0.075 * k1_phi + 0.225 * k2_phi);

			temp_r_dot = particles[i].r_dot + timestep * (0.075 * k1_r_dot + 0.225 * k2_r_dot);
			temp_phi_dot = particles[i].phi_dot + timestep * (0.075 * k1_phi_dot + 0.225 * k2_phi_dot);
			...
			temp_r = particles[i].r + timestep * (0.3 * k1_r - 0.9 * k2_r + 1.2 * k3_r);
			temp_phi = particles[i].phi + timestep * (0.3 * k1_phi - 0.9 * k2_phi + 1.2 * k3_phi);

			temp_r_dot = particles[i].r_dot + timestep * (0.3 * k1_r_dot - 0.9 * k2_r_dot + 1.2 * k3_r_dot);
			temp_phi_dot = particles[i].phi_dot + timestep * (0.3 * k1_phi_dot - 0.9 * k2_phi_dot + 1.2 * k3_phi_dot);
			...
			temp_r = particles[i].r  + timestep * (- 11.0/54.0 * k1_r + 2.5 * k2_r - 70.0/27.0 * k3_r + 35.0/27.0 * k4_r);
			temp_phi = particles[i].phi + timestep * (- 11.0/54.0 * k1_phi + 2.5 * k2_phi - 70.0/27.0 * k3_phi + 35.0/27.0 * k4_phi);

			temp_r_dot = particles[i].r_dot  + timestep * (- 11.0/54.0 * k1_r_dot + 2.5 * k2_r_dot - 70.0/27.0 * k3_r_dot + 35.0/27.0 * k4_r_dot);
			temp_phi_dot = particles[i].phi_dot + timestep * (- 11.0/54.0 * k1_phi_dot + 2.5 * k2_phi_dot - 70.0/27.0 * k3_phi_dot + 35.0/27.0 * k4_phi_dot);
			...
			temp_r = particles[i].r + timestep * (1631.0/55296.0 * k1_r + 175.0/512.0 * k2_r + 575.0/13824.0 * k3_r + 44275.0/110592.0 * k4_r + 253.0/4096.0 * k5_r);
			temp_phi = particles[i].phi + timestep * (1631.0/55296.0 * k1_phi + 175.0/512.0 * k2_phi + 575.0/13824.0 * k3_phi + 44275.0/110592.0 * k4_phi + 253.0/4096.0 * k5_phi);

			temp_r_dot = particles[i].r_dot + timestep * (1631.0/55296.0 * k1_r_dot + 175.0/512.0 * k2_r_dot + 575.0/13824.0 * k3_r_dot + 44275.0/110592.0 * k4_r_dot + 253.0/4096.0 * k5_r_dot);
			temp_phi_dot = particles[i].phi_dot + timestep * (1631.0/55296.0 * k1_phi_dot + 175.0/512.0 * k2_phi_dot + 575.0/13824.0 * k3_phi_dot + 44275.0/110592.0 * k4_phi_dot + 253.0/4096.0 * k5_phi_dot);
			...
			const double phi_dot_new = particles[i].phi_dot + timestep * phi_ddot_new;


			// error estimates, reuse k2 variables
			k2_r = timestep * (e1*k1_r + e3*k3_r + e4*k4_r + e5*k5_r + e6*k6_r);
			k2_phi = timestep * (e1*k1_phi + e3*k3_phi + e4*k4_phi + e5*k5_phi + e6*k6_phi);
			k2_r_dot = timestep * (e1*k1_r_dot + e3*k3_r_dot + e4*k4_r_dot + e5*k5_r_dot + e6*k6_r_dot);
			k2_phi_dot = timestep * (e1*k1_phi_dot + e3*k3_phi_dot + e4*k4_phi_dot + e5*k5_phi_dot + e6*k6_phi_dot);
			...
			sk = atoli + rtoli * fmax (fabs(particles[i].r_dot), fabs(r_dot_new));
			sqr = k2_r_dot / sk;
			err += sqr*sqr;

			sk = atoli + rtoli * fmax (fabs(particles[i].phi_dot), fabs(phi_dot_new));
			sqr = k2_phi_dot / sk;
			err += sqr*sqr;
...
void integrate_explicit(t_data &data, const double dt) {
	...
	for (unsigned int i = 0; i < local_number_of_particles; ++i)
	{
		...
		temp_r_dot = particles[i].r_dot + dt * (0.075 * k1_r_dot + 0.225 * k2_r_dot);
		temp_phi_dot = particles[i].phi_dot + dt * (0.075 * k1_phi_dot + 0.225 * k2_phi_dot);
		...
		temp_phi = particles[i].phi + dt * (0.3 * k1_phi - 0.9 * k2_phi + 1.2 * k3_phi);

		temp_r_dot = particles[i].r_dot + dt * (0.3 * k1_r_dot - 0.9 * k2_r_dot + 1.2 * k3_r_dot);
		temp_phi_dot = particles[i].phi_dot + dt * (0.3 * k1_phi_dot - 0.9 * k2_phi_dot + 1.2 * k3_phi_dot);
		...
		temp_r = particles[i].r  + dt * (- 11.0/54.0 * k1_r + 2.5 * k2_r - 70.0/27.0 * k3_r + 35.0/27.0 * k4_r);
		temp_phi = particles[i].phi + dt * (- 11.0/54.0 * k1_phi + 2.5 * k2_phi - 70.0/27.0 * k3_phi + 35.0/27.0 * k4_phi);

		temp_r_dot = particles[i].r_dot  + dt * (- 11.0/54.0 * k1_r_dot + 2.5 * k2_r_dot - 70.0/27.0 * k3_r_dot + 35.0/27.0 * k4_r_dot);
		temp_phi_dot = particles[i].phi_dot + dt * (- 11.0/54.0 * k1_phi_dot + 2.5 * k2_phi_dot - 70.0/27.0 * k3_phi_dot + 35.0/27.0 * k4_phi_dot);
		...
		temp_r = particles[i].r + dt * (1631.0/55296.0 * k1_r + 175.0/512.0 * k2_r + 575.0/13824.0 * k3_r + 44275.0/110592.0 * k4_r + 253.0/4096.0 * k5_r);
		temp_phi = particles[i].phi + dt * (1631.0/55296.0 * k1_phi + 175.0/512.0 * k2_phi + 575.0/13824.0 * k3_phi + 44275.0/110592.0 * k4_phi + 253.0/4096.0 * k5_phi);

		temp_r_dot = particles[i].r_dot + dt * (1631.0/55296.0 * k1_r_dot + 175.0/512.0 * k2_r_dot + 575.0/13824.0 * k3_r_dot + 44275.0/110592.0 * k4_r_dot + 253.0/4096.0 * k5_r_dot);
		temp_phi_dot = particles[i].phi_dot + dt * (1631.0/55296.0 * k1_phi_dot + 175.0/512.0 * k2_phi_dot + 575.0/13824.0 * k3_phi_dot + 44275.0/110592.0 * k4_phi_dot + 253.0/4096.0 * k5_phi_dot);
