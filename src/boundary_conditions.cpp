...
void apply_boundary_condition(t_data &data, double dt, bool final)
{
	...
	case parameters::boundary_condition_viscous_outflow:
		...
		break;
	case parameters::boundary_condition_evanescent: // evanescent works only
							// for inner and outer
							// together until now
		if (parameters::boundary_outer ==
			...
			logging::print_master(LOG_ERROR
					      "Different EvanescentBoundary "
					      "Parameters. Old .par File?\n");
			die("inner/outer evanescent boundary not implemented "
			    "yet");
		}
		break;
	...
	case parameters::boundary_condition_evanescent:
		// EvanescentBoundary_outer(VRadial, VAzimuthal, Density,
		// Energy, dt, sys);
		break;
	case parameters::boundary_condition_keplerian:
	...
	if (CPU_Rank == CPU_Highest) {
		if ((parameters::domegadr_zero) &&
			...
			for (unsigned int n_azimuthal = 0;
			     ...
			     ++n_azimuthal) {
				// this is a work around as long as V_AZIMUTHAL
	...
	if (OuterSourceMass)
		ApplyOuterSourceMass(&data[t_data::DENSITY],
...
void open_boundary_outer(t_data &data)
{
	...
	for (unsigned int n_azimuthal = 0;
		...
		data[t_data::DENSITY](data[t_data::DENSITY].get_max_radial(),
				      n_azimuthal) =
		    data[t_data::DENSITY](
			data[t_data::DENSITY].get_max_radial() - 1,
			n_azimuthal);
		...
		// set velocity to min(v[+1],0) (allow only outflow)
		if (data[t_data::V_RADIAL](
			data[t_data::V_RADIAL].get_max_radial() - 2,
			n_azimuthal) < 0.0) {
			data[t_data::V_RADIAL](
			    data[t_data::V_RADIAL].get_max_radial() - 1,
			    n_azimuthal) = 0.0;
			data[t_data::V_RADIAL](
			    data[t_data::V_RADIAL].get_max_radial(),
			    n_azimuthal) = 0.0;
		} else {
			data[t_data::V_RADIAL](
			    data[t_data::V_RADIAL].get_max_radial() - 1,
	...
	if (CPU_Rank == CPU_Highest) {
		for (unsigned int n_azimuthal = 0;
			...
			data[t_data::DENSITY](
				...
				n_azimuthal);
			data[t_data::ENERGY](
			    data[t_data::ENERGY].get_max_radial(),
			    n_azimuthal) =
			    data[t_data::ENERGY](
				data[t_data::ENERGY].get_max_radial() - 1,
				n_azimuthal);
...
void viscous_outflow_boundary_inner(t_data &data)
{
	if (CPU_Rank == 0) {
		...
		for (unsigned int n_azimuthal = 0;
		     ...
		     ++n_azimuthal) {
			// V_rad =  - 1.5 / r * Nu (Kley, Papaloizou and
			// Ogilvie, 2008)
			data[t_data::V_RADIAL](1, n_azimuthal) =
			    -1.5 / Rinf[1] * nu_med;
			data[t_data::V_RADIAL](0, n_azimuthal) =
			  ...
			  double dt)
{
	...
	if ((parameters::damping_inner_limit > 1.0) &&
		...
		while (
		    (radius[limit] < RMIN * parameters::damping_inner_limit) &&
			  ...
			  double dt)
{
	...
	if ((parameters::damping_outer_limit < 1.0) &&
		...
		while (
		    (radius[limit] > RMAX * parameters::damping_outer_limit) &&
		    (limit > 0)) {
			limit--;
		}
		limit++;
...
void damping(t_data &data, double dt)
{
	if (parameters::damping) {
		...
		for (unsigned int i = 0; i < number_of_quantities_to_damp;
		     ++i) {
			...
			if (damper->inner_damping_function != nullptr)
				(damper->inner_damping_function)(
				    ...
				    data[damper->array_with_damping_values],
				    dt);
...
void mass_overflow(t_data &data)
{
	...
	if (parameters::mof_planet + 1 >
		...
		logging::print_master(LOG_ERROR
				      "Wrong Planet/Star for Mass Overflow "
				      "specified! Old .par File?\n");
		die("Wrong Planet/Star for Mass Overflow specified! Old .par "
		    "File?");
	}
	double xplanet = data.get_planetary_system()
			     .get_planet(parameters::mof_planet)
			     .get_x();
	double yplanet = data.get_planetary_system()
			     .get_planet(parameters::mof_planet)
			     .get_y();
	...
	int nearest_grid_cell =
	    maxcells * angle; // nearest gridcell to binary star
	...
	double dist = data.get_planetary_system()
			  .get_planet(parameters::mof_planet)
			  .get_distance();
	...
	for (int i = -number_of_cells; i <= number_of_cells; i++) {
		...
		check += weight_factor;
		mass_stream =
		    weight_factor * parameters::mof_value / 2.0 / PI * DT;
...
#ifndef NDEBUG
		logging::print(
		    LOG_VERBOSE "dens %lE, WF %lE , angle %lf, "
				"nearest_grid_cell %i, mass_stream %lE, "
				"gridcell %i, maxcells %i , noc %i , i %i \n",
		    data[t_data::DENSITY](maxradial - 1, gridcell),
		    weight_factor, angle, nearest_grid_cell, mass_stream,
		    gridcell, maxcells, number_of_cells * 2 + 1, i);
#endif
	}
	...
	if (!(check > 0.99) || !(check < 1.01)) {
		logging::print_master(LOG_ERROR
				      "weight_factor %lf should be 0.997 \n",
				      weight_factor);
	}
}
...
{
	if (CPU_Rank == 0) {
		for (unsigned int n_radial = 0; n_radial <= GHOSTCELLS_B;
		     ++n_radial) {
			for (unsigned int n_azimuthal = 0;
			     n_azimuthal <=
		...
		default:
			logging::print_master(
			    LOG_ERROR "Boundary condition at inner boundary "
	...
	if (CPU_Rank == CPU_Highest) {
		...
		default:
			logging::print_master(
			    LOG_ERROR "Boundary condition at outer boundary "
				      "not supported for temperature!\n");
			break;
		}
...
void boundary_layer_inner_boundary(t_data &data)
{
	...
	for (unsigned int n_azimuthal = 0;
		...
		data[t_data::V_RADIAL](1, n_azimuthal) =
		    -1. * parameters::vrad_fraction_of_kepler *
		    sqrt(1. / Ra[1]);
		data[t_data::V_RADIAL](0, n_azimuthal) =
		    data[t_data::V_RADIAL](1, n_azimuthal);
...
void keplerian2d_boundary_inner(t_data &data)
{
	for (unsigned int n_azimuthal = 0;
		...
		data[t_data::V_AZIMUTHAL](1, n_azimuthal) =
		    sqrt(constants::G * M / Rmed[1]);
		data[t_data::V_AZIMUTHAL](0, n_azimuthal) =
		    sqrt(constants::G * M / Rmed[0]);
	}
}
...
void keplerian2d_boundary_outer(t_data &data)
{
	for (unsigned int n_azimuthal = 0;
		...
		data[t_data::TEMPERATURE](
		    data[t_data::TEMPERATURE].get_max_radial(), n_azimuthal) =
		    data[t_data::ENERGY](data[t_data::ENERGY].get_max_radial(),
					 n_azimuthal) /
		    data[t_data::DENSITY](
			data[t_data::DENSITY].get_max_radial(), n_azimuthal) *
		    (ADIABATICINDEX - 1.0) * parameters::MU * constants::R;
		data[t_data::TEMPERATURE](
		    data[t_data::TEMPERATURE].get_max_radial() - 1,
		    n_azimuthal) =
		    data[t_data::ENERGY](data[t_data::ENERGY].get_max_radial() -
					     1,
					 n_azimuthal) /
