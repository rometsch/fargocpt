...
void apply_boundary_condition(t_data &data, double dt, bool final)
{
	...
	if (parameters::boundary_outer ==
	    ...
	    logging::print_master(LOG_ERROR "Different EvanescentBoundary "
...
void mass_overflow(t_data &data)
{
	...
	logging::print_master(LOG_ERROR "Wrong Planet/Star for Mass Overflow "
...
#ifndef NDEBUG
	logging::print(LOG_VERBOSE
		       "dens %lE, WF %lE , angle %lf, "
...
void apply_boundary_condition_temperature(t_data &data)
{
	...
	default:
	    logging::print_master(LOG_ERROR
				  "Boundary condition at inner boundary "
	...
	default:
	    logging::print_master(LOG_ERROR
				  "Boundary condition at outer boundary "
				  "not supported for temperature!\n");
	    break;
	}
