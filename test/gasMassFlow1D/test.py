...
mass_flow_reader = gasMassFlow1D(out_folder, "solMass/yr")
...
rs, DT, data, num_outputs = mass_flow_reader.read()
...
for n in N_output:
    ...
    ax.plot(rs[1:-1], np.abs((data[N_DT[n]])[1:-1]), color=colors[ind], label="Simulation " + str(n))
