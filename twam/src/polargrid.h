...
class t_polargrid
{
	private:
		...
		/// scalar or vector quantity (scalar uses grid from 0 to Nrad-1, vector from 0 to Nrad)
		bool m_scalar;
	...
	public:
		...
		inline void set_write(bool value) { set_write_1D(value); set_write_2D(value); }
		inline void set_do_before_write(void (*value)(t_data &, unsigned int, bool)) { m_do_before_write = value; }

		// getter
		inline ptrdiff_t get_max_radial() const { return m_scalar ? Nrad-1 : Nrad; }
		...
		unsigned int get_memory_usage(ptrdiff_t  size_radial, ptrdiff_t  size_azimuthal);

		inline unsigned int cell(ptrdiff_t nRadial, ptrdiff_t nAzimuthal) const { return nAzimuthal+(nRadial*Nsec); }

		inline double& operator()(ptrdiff_t nRadial, ptrdiff_t nAzimuthal) { return Field[cell(nRadial,nAzimuthal)]; }
		inline double operator()(ptrdiff_t nRadial, ptrdiff_t nAzimuthal) const { return Field[cell(nRadial,nAzimuthal)]; }
