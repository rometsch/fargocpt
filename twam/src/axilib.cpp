...
/**
	Make an 1D profile of a 2D grid by calculating mean values over azimuthal cells
...
void mpi_make1Dprofile(double* grid, double* array)
{
	...
	if (tempArray == NULL) {
		logging::print(LOG_ERROR "Not enough memory in axisglib.c ; suspect...");
		PersonalExit(1);
	}
	...
	/* if more than 1 cpu... */
		...
		MPI_Bcast(array, GlobalNRadial, MPI_DOUBLE, CPU_Highest, MPI_COMM_WORLD);
	}
